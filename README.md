...
**Note about LLVM 13 and TBB 2021:** Hyrise can currently not be built with LLVM 13. We hope to get LLVM 13 running soon. For TBB, please use a `2020*` version until https://github.com/oneapi-src/oneTBB/issues/378 is resolved. On MacOS with brew, LLVM 12 can be installed as follows: `brew install llvm@12`. Keep in mind that this package version is an alternate version and, thus, not symlinked into `/usr/local`. `brew link llvm@12` symlinks the package.
