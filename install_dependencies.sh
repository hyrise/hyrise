...
if echo $REPLY | grep -E '^[Yy]$' > /dev/null; then
    ...
    if [[ "$unamestr" == 'Darwin' ]]; then
        ...
        if brew update >/dev/null; then
            ...
            for formula in autoconf boost cmake coreutils dos2unix graphviz libpq ncurses parallel pkg-config postgresql readline sqlite3; do
        ...
        if [ -f /etc/lsb-release ] && cat /etc/lsb-release | grep DISTRIB_ID | grep Ubuntu >/dev/null; then
            ...
            if sudo apt-get update >/dev/null; then
                ...
                sudo update-alternatives --install /usr/bin/clang clang /usr/bin/clang-9 90 --slave /usr/bin/clang++ clang++ /usr/bin/clang++-10 --slave /usr/bin/clang-tidy clang-tidy /usr/bin/clang-tidy-9 --slave /usr/bin/llvm-profdata llvm-profdata /usr/bin/llvm-profdata-9 --slave /usr/bin/llvm-cov llvm-cov /usr/bin/llvm-cov-9 --slave /usr/bin/clang-format clang-format /usr/bin/clang-format-9
            else
                echo "Error during installation."
                exit 1
            fi
