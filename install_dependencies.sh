    ...
    if [[ "$unamestr" == 'Darwin' ]]; then
        ...
        if [ -f /etc/lsb-release ] && cat /etc/lsb-release | grep DISTRIB_ID | grep Ubuntu >/dev/null; then
            ...
            if apt-get update >/dev/null; then
                ...
                if ! apt-get install --no-install-recommends -y software-properties-common lsb-release git python3 python3-pip autoconf bash-completion bc bolt-20 clang-19 llvm-20 clang-20 clang-format-20 clang-tidy-20 libclang-rt-dev cmake curl dos2unix g++-14 gcc-14 g++-16 gcc-16 graphviz libboost-all-dev libhwloc-dev libncurses5-dev libnuma-dev libnuma1 libpq-dev libreadline-dev libsqlite3-dev libstdc++-16-dev libtbb-dev lld-20 man parallel postgresql-server-dev-all time valgrind; then
                ...
                update-alternatives --install /usr/bin/gcc gcc /usr/bin/gcc-16 90 --slave /usr/bin/g++ g++ /usr/bin/g++-16
                update-alternatives --install /usr/bin/clang clang /usr/bin/clang-20 90 --slave /usr/bin/clang++ clang++ /usr/bin/clang++-20 --slave /usr/bin/clang-tidy clang-tidy /usr/bin/clang-tidy-20 --slave /usr/bin/llvm-profdata llvm-profdata /usr/bin/llvm-profdata-20 --slave /usr/bin/llvm-cov llvm-cov /usr/bin/llvm-cov-20 --slave /usr/bin/clang-format clang-format /usr/bin/clang-format-20  --slave /usr/bin/ld.lld ld.lld /usr/bin/ld.lld-20 --slave /usr/bin/llvm-bolt llvm-bolt /usr/bin/llvm-bolt-20 --slave /usr/bin/merge-fdata merge-fdata /usr/bin/merge-fdata-20 --slave /usr/lib/libbolt_rt_instr.a libbolt_rt_instr.a /usr/lib/llvm-20/lib/libbolt_rt_instr.a
            else
                echo "Error during installation."
                exit 1
            fi
