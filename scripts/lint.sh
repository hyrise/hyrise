...
grep -rn 'static[^:]*instance;' --exclude src/lib/utils/singleton.hpp src | grep -v '#[0-9]\+' | sed 's/^\([a-zA-Z/._]*:[0-9]*\).*/\1  Singletons should not be implemented manually. Have a look at src\/utils\/singleton.hpp/'
let "exitcode |= $?"
