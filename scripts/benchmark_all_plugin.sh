...
case "${output}" in
    Linux*)     dynamic_library_suffix="so";;
    Darwin*)    dynamic_library_suffix="dylib";;
    *)          echo 'Unsupported operating system. Aborting.' && exit 1;;
esac
...
do
  ...
  if [ -f "benchmark_all_results/complete_${commit}_plugin" ]
  then
  ...
  do
    if [ "$benchmark" = "hyriseBenchmarkTPCC" ]; then
      ...
      ( "${build_folder}"/"$benchmark" -o "${build_folder}/benchmark_all_results/${benchmark}_${commit}_st_plugin.json" -p "${build_folder}/lib/libhyriseDependencyDiscoveryPlugin.${dynamic_library_suffix}" 2>&1 ) | tee "${build_folder}/benchmark_all_results/${benchmark}_${commit}_st_plugin.log"
    else
      echo "Running $benchmark for $commit... (single-threaded)"
      ( "${build_folder}"/"$benchmark" -r ${runs} -w ${warmup_seconds} -o "${build_folder}/benchmark_all_results/${benchmark}_${commit}_st_plugin.json" -p "${build_folder}/lib/libhyriseDependencyDiscoveryPlugin.${dynamic_library_suffix}" 2>&1 ) | tee "${build_folder}/benchmark_all_results/${benchmark}_${commit}_st_plugin.log"
    fi

    if [ "$benchmark" = "hyriseBenchmarkTPCH" ]; then
      echo "Running $benchmark for $commit... (single-threaded, SF 0.01)"
      ( "${build_folder}"/"$benchmark" -s .01 -r ${runs} -w ${warmup_seconds} -o "${build_folder}/benchmark_all_results/${benchmark}_${commit}_st_s01_plugin.json" -p "${build_folder}/lib/libhyriseDependencyDiscoveryPlugin.${dynamic_library_suffix}" 2>&1 ) | tee "${build_folder}/benchmark_all_results/${benchmark}_${commit}_st_s01_plugin.log"

      echo "Running $benchmark for $commit... (multi-threaded, ordered, 1 client)"
      ( "${build_folder}"/"$benchmark" --scheduler --clients 1 --cores ${num_phy_cores} -m Ordered -o "${build_folder}/benchmark_all_results/${benchmark}_${commit}_mt_ordered_plugin.json" -p "${build_folder}/lib/libhyriseDependencyDiscoveryPlugin.${dynamic_library_suffix}" 2>&1 ) | tee "${build_folder}/benchmark_all_results/${benchmark}_${commit}_mt_ordered_plugin.log"
    fi

    echo "Running $benchmark for $commit... (multi-threaded, shuffled, $num_phy_cores clients)"
    ( "${build_folder}"/"$benchmark" --scheduler --clients ${num_phy_cores} --cores ${num_phy_cores} -m Shuffled -t ${mt_shuffled_runtime} -o "${build_folder}/benchmark_all_results/${benchmark}_${commit}_mt_plugin.json" -p "${build_folder}/lib/libhyriseDependencyDiscoveryPlugin.${dynamic_library_suffix}" 2>&1 ) | tee "${build_folder}/benchmark_all_results/${benchmark}_${commit}_mt_plugin.log"
  done
  cd "${build_folder}"

  # After all benchmarks are done, leave a marker so that this commit can be skipped next time
  touch "benchmark_all_results/complete_${commit}_plugin"
done
...
xargs < "${build_folder}/benchmark_all_results/build_time_${start_commit}_plugin.txt" | awk '{printf $0 "|\n"}'
echo -n "| $(git show -s --date=format:'%d.%m.%Y %H:%M' --format='%h | %cd | %s' "${end_commit}") | "
xargs < "${build_folder}/benchmark_all_results/build_time_${end_commit}_plugin.txt" | awk '{printf $0 "|\n"}'
...
do
  ...
  do
    output=$(../scripts/compare_benchmarks.py "${build_folder}/benchmark_all_results/${benchmark}_${start_commit}_${config}_plugin.json" - "${build_folder}/benchmark_all_results/${benchmark}_${end_commit}_${config}_plugin.json" --github 2>/dev/null)
    echo ""
    echo ""
    echo -n "**${benchmark} - "
    if [ "$benchmark" = "hyriseBenchmarkTPCH" ]; then
