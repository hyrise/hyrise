...
#
# Example usage 2:
# python3 ./scripts/benchmark_multithreaded.py -e ./build-release/hyriseBenchmarkTPCH --runs 1000 -v --scale 1 \
# --cores 10 20 30 --queries 1,3,6,12 --clients 20
...
def get_parser():
    ...
    parser.add_argument(
        ...
        type=str,
        metavar="Q",
        help="Specify the queries that will be benchmarked (comma-separated list of query IDs, e.g. --queries 1,3,18",
    )
    parser.add_argument(
        "-e",
        "--executable",
        ...
        type=str,
        metavar="E",
        help="benchmark executable (e.g., hyriseBenchmarkTPCH)",
        required=True,
    )
    parser.add_argument(
        "--result-dir",
        ...
        metavar="DIR",
        default="results",
        help="Directory where the result folder will be stored (default: 'results/')",
    )
    parser.add_argument(
        "--result-name",
        action="store",
        type=str,
        metavar="NAME",
        help="Directory where the actual results will be stored (default: current datetime)",
    )
    parser.add_argument(
        "--clients",
        action="store",
        ...
        metavar="C",
        default=10,
        help="The number of clients that schedule queries in parallel",
    )
    ...
    core_group.add_argument(
        ...
        metavar="C",
        nargs="+",
        help="List of cores to be used for the benchmarks",
    )
    core_group.add_argument(
        "--max-cores",
        action="store",
        type=int,
        metavar="C",
        help="Number of cores up to which the benchmarks will be executed",
    )

    run_group = parser.add_mutually_exclusive_group(required=True)
    run_group.add_argument(
        "-r", "--runs", action="store", type=int, metavar="N", help="Fixed number of (max) runs each query is executed"
    )
    run_group.add_argument(
        "--runs-per-core",
        action="store",
        type=int,
        metavar="N",
        help="Number of (max) runs per core each query is executed",
    )
...
def run_benchmarks(args, hyrise_args, core_counts, result_dir):
    ...
    for core_count in core_counts:
        benchmark_run += 1
        verbose_print(
            args.verbose, "Starting benchmark " + str(benchmark_run) + " of " + str(len(core_counts)) + " ..."
        )
