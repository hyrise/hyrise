...
do
  ...
  do
      ...
      ( "${build_folder}"/"$benchmark" -s 100 -t 600 -r ${runs} -w ${warmup_seconds}   --dont_cache_binary_tables -o "${build_folder}/benchmark_all_results/${benchmark}_${commit}_st_s100.json" 2>&1 ) | tee "${build_folder}/benchmark_all_results/${benchmark}_${commit}_st_s100.log"
  done
  cd "${build_folder}"

  # After all benchmarks are done, leave a marker so that this commit can be skipped next time
  touch "benchmark_all_results/complete_s100_${commit}"
done
