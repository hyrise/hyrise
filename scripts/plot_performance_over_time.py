...
for is_detailed in [False, True]:
    ...
    for i, benchmark_name in enumerate(benchmark_names):
        ...
        for success in [True, False]:
            ...
            if not data.empty:
                data.plot(ax=ax, kind='scatter', x='begin', y='duration', c=single_run_color,
                          figsize=(figw, figh), s=5, marker=('o' if success else 'x'), linewidth=1)
        ...
        window_size = max(1, int(len(rolling) * .02))
    ...
    fig.set_size_inches(old_size[0], old_size[1] + total_log_height + .1, forward=True)
    ...
    for log in log_json:
        ...
        # Add the line
        line = mpatches.ConnectionPatch(xyA=(timestamp, axes[0].get_ylim()[1]), xyB=xyB,
                                        coordsA="data", coordsB="figure pixels", axesA=axes[0],
                                        axesB=axes[-1], color=(0, 0, 0, 0.2), clip_on=False,
                                        linestyle='dashed')
        fig.add_artist(line)
        ...
        align = 'right' if textloc[0] > .5 else 'left'
        text = fig.text(textloc[0], textloc[1], log['reporter'] + ": " + log['message'], fontsize=9,
                        horizontalalignment=align, bbox=dict(facecolor='white',
                                                             edgecolor=(0, 0, 0, .2), pad=3))
