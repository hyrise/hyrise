...
if __name__ == "__main__":
    ...
    os.system(
        f"ninja -C {build_dir} hyriseBenchmarkTPCH hyriseBenchmarkTPCDS hyriseBenchmarkJoinOrder hyriseBenchmarkStarSchema"
    )
            ...
            for UPPER_LIMIT_QUEUE_SIZE_FACTOR in upper_limit_queue_size_factor:
                for benchmark in ["TPCH", "TPCDS", "JoinOrder", "StarSchema"]:
                    for mode in ["Shuffled", "Ordered"]:
                        ...
                        try:
                            os.system(
                                f"NUM_GROUPS_MIN_FACTOR={NUM_GROUPS_MIN_FACTOR} NUM_GROUPS_MAX_FACTOR={NUM_GROUPS_MAX_FACTOR} UPPER_LIMIT_QUEUE_SIZE_FACTOR={UPPER_LIMIT_QUEUE_SIZE_FACTOR} ./{build_dir}/hyriseBenchmark{benchmark} --mode={mode} --clients={client_count} --cores={core_count} --scheduler -o {output_filename} {run_limit} --warmup=1 {scale}"
                            )
