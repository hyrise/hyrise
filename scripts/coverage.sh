...
./$1/hyriseCoverage && rm -fr coverage; mkdir coverage && gcovr $1 --gcov-executable="gcov -x" --verbose -s -r . --exclude="(.*types*.|.*test*.|third_party/*|src/benchmark*)" --html --html-details -o coverage/index.html > coverage_output.txt
