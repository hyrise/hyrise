...
./$1/hyriseCoverage && rm -fr coverage; mkdir coverage && gcovr --gcov-executable="gcov -s `pwd`" -s -p --exclude='.*/(?:third_party|src/test|src/benchmark).*' --exclude-unreachable-branches --html --html-details -o coverage/index.html > coverage_output.txt
