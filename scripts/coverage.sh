...
./$1/hyriseCoverage && rm -fr coverage; mkdir coverage && gcovr -s -r . --exclude="(.*types*.|src/test*.|.*\.pb\.|third_party|src/benchmark*)" --html --html-details -o coverage/index.html > coverage_output.txt
