...
def main():
  ...
  benchmark = initialize(arguments1, "hyriseBenchmarkJoinOrder")
  ...
  for i in xrange(0,4):
    if output["benchmarks"][i]["name"] != arguments1["--queries"].replace("'", '').split(',')[i]:
      return_error = True
      print("ERROR: Query doesn't match with JSON:", output["benchmarks"][i]["name"], arguments1["--queries"].replace("'", '').split(',')[i])
