...
do
  ...
  do
    if [ "$benchmark" = "hyriseBenchmarkTPCC" ]; then
      ...
      ( "${build_folder}"/"$benchmark" --pipeline_metrics -o "${build_folder}/benchmark_all_results/${benchmark}_${commit}_st.json" 2>&1 ) | tee "${build_folder}/benchmark_all_results/${benchmark}_${commit}_st.log"
    else
      echo "Running $benchmark for $commit... (single-threaded)"
    ...
    if [ "$benchmark" = "hyriseBenchmarkTPCC" ]; then
      ...
      ( "${build_folder}"/"$benchmark" --pipeline_metrics --scheduler --clients ${clients_tpcc_low_contention} --cores ${num_phy_cores} -s ${tpcc_warehouse_count} -m Shuffled -t ${tpcc_shuffled_runtime} -o "${build_folder}/benchmark_all_results/${benchmark}_${commit}_mt_lowcont.json" 2>&1 ) | tee "${build_folder}/benchmark_all_results/${benchmark}_${commit}_mt_lowcont.log"
    else
