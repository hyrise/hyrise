...
do
  run_executable="$executable -o benchmark_memory_limit_$limit.json"
  echo "Running $run_executable with memory limit of $limit GB"
  source $SCRIPT_DIR/run_with_limited_memory.sh $limit $run_executable
