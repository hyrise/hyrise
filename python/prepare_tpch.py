...
def import_test_data(tpch):
    # read data
    table_scans = parse_hyrise_csv(tpch[0])
    table_meta_data = parse_hyrise_csv(tpch[1])
    segment_meta_data = parse_hyrise_csv(tpch[3])
    attribute_meta_data = parse_hyrise_csv(tpch[2])
    ...
    print(joined_data.columns.values)
