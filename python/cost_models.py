...
def main(args):
    ...
    for model_type in model_types:
        ...
        scores[f"{model_type}_general_model"] = generate_model_plot(
            gmodel, gtest_data, "all", model_type, "all", "all", out
        )
                ...
                try:
                    ...
                    if not args.test:
                        model_train_data, model_test_data = train_test_split(
                            data_copy.loc[
                                (data_copy["OPERATOR_NAME"] == operator)
                                & (data_copy["ENCODING"] == encoding)
                                & (data_copy["OPERATOR_IMPLEMENTATION"] == implementation_type)
                            ]
                        )

                    else:
                        model_train_data = data_copy.loc[
                            (data_copy["OPERATOR_NAME"] == operator)
                            & (data_copy["ENCODING"] == encoding)
                            & (data_copy["OPERATOR_IMPLEMENTATION"] == implementation_type)
                        ]
                        model_test_data = data_copy.loc[
                            (data_copy["OPERATOR_NAME"] == operator)
                            & (data_copy["ENCODING"] == encoding)
                            & (data_copy["OPERATOR_IMPLEMENTATION"] == implementation_type)
                        ]
                    ...
                    print(
                        f"Not enough data of the combination {encoding}, {operator}, {implementation_type}",
                        "to split into training and test data",
                    )
                    continue

                model_train_data = model_train_data.drop(
                    labels=["OPERATOR_NAME", "ENCODING", "OPERATOR_IMPLEMENTATION"], axis=1,
                )
                    ...
                    for model_type in model_types:
                        ...
                        if not model_test_data.empty:
                            scores[model_name] = generate_model_plot(
                                model, model_test_data, operator, model_type, encoding, implementation_type, out,
                            )
    log(scores, out)
