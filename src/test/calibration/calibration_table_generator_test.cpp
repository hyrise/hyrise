...
namespace opossum {
    ...
    TEST_F(CalibrationTableGeneratorTest, GenerateExpectedTables){
      ...
      for (unsigned long table_index = 0; table_index < number_of_tables; ++table_index){
        ...
        for (unsigned short column_index = 0; column_index < number_of_columns; ++column_index){
          ...
          ColumnDataDistribution expected_column_data_distribution = expected_columns.at(column_index).column_data_distribution;

          const auto column_id = ColumnID{column_index};
          EXPECT_EQ(table->column_data_type(column_id), expected_data_type);

          // Column Distribution
          ColumnDataDistribution true_column_data_distribution = wrapped_tables.at(table_index)->get_column_data_distribution(column_id);
          ...
          const auto encoded_segment = std::dynamic_pointer_cast<BaseEncodedSegment>(segment); // dynamic_past must be valid
