...
class OperatorTaskTest : public BaseTest {
 protected:
  void SetUp() override {
    _test_table_a = load_table("artifacts/test_data/tbl/int_float.tbl", 2);
    StorageManager::get().add_table("table_a", _test_table_a);

    _test_table_b = load_table("artifacts/test_data/tbl/int_float2.tbl", 2);
...
TEST_F(OperatorTaskTest, SingleDependencyTasksFromOperatorTest) {
  ...
  auto expected_result = load_table("artifacts/test_data/tbl/int_float_filtered.tbl", 2);
...
TEST_F(OperatorTaskTest, DoubleDependencyTasksFromOperatorTest) {
  ...
  auto expected_result = load_table("artifacts/test_data/tbl/joinoperators/int_inner_join.tbl", 2);
  EXPECT_TABLE_EQ_UNORDERED(expected_result, tasks.back()->get_operator()->get_output());
