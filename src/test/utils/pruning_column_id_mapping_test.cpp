...
TEST(ColumnPruning, ColumnIDMapping) {
  ...
  auto expected_column_mapping_1 = std::vector<std::optional<ColumnID>>{
    std::nullopt, std::nullopt, std::nullopt, ColumnID{0}, ColumnID{1}, ColumnID{2}
  };
  auto expected_column_mapping_2 = std::vector<std::optional<ColumnID>>{
    ColumnID{0}, ColumnID{1}, std::nullopt, std::nullopt, ColumnID{2}, ColumnID{3}
  };
  auto expected_column_mapping_3 = std::vector<std::optional<ColumnID>>{
    ColumnID{0}, ColumnID{1}, ColumnID{2}, ColumnID{3}, ColumnID{4}, std::nullopt
  };
  auto expected_column_mapping_4 = std::vector<std::optional<ColumnID>>{
    std::nullopt, ColumnID{0}, std::nullopt, ColumnID{1}, std::nullopt, ColumnID{2}
  };
