...
Matrix _table_to_matrix(const std::shared_ptr<const opossum::Table>& table) {
  ...
  for (auto column_id = opossum::ColumnID{0}; column_id < table->column_count(); ++column_id) {
    ...
    matrix[1][column_id] = opossum::data_type_to_string.left.at(table->column_is_type(column_id));
  }
...
bool check_table_equal(const std::shared_ptr<const Table>& opossum_table,
  ...
  for (auto column_id = ColumnID{0}; column_id < expected_table->column_count(); ++column_id) {
    left_col_type = opossum_table->column_is_type(column_id);
    right_col_type = expected_table->column_is_type(column_id);
    ...
    if (left_col_type != right_col_type) {
      ...
      const std::string error_msg =
          "Actual column type: " + data_type_to_string.left.at(opossum_table->column_is_type(column_id)) + "\n" +
          "Expected column type: " + data_type_to_string.left.at(expected_table->column_is_type(column_id));
    ...
    for (auto column_id = ColumnID{0}; column_id < opossum_matrix[row_id].size(); column_id++) {
      if (variant_is_null(opossum_matrix[row_id][column_id]) || variant_is_null(expected_matrix[row_id][column_id])) {
        highlight_if(!(variant_is_null(opossum_matrix[row_id][column_id]) &&
                     ...
                     row_id, column_id);
      } else if (opossum_table->column_is_type(column_id) == DataType::Float) {
      ...
      } else if (opossum_table->column_is_type(column_id) == DataType::Double) {
        ...
        if (type_cmp_mode == TypeCmpMode::Lenient && (opossum_table->column_is_type(column_id) == DataType::Int ||
                                                      opossum_table->column_is_type(column_id) == DataType::Long)) {
