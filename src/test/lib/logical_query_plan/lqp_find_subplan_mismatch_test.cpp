...
class LQPFindSubplanMismatchTest : public BaseTest {
 protected:
  ...
  void SetUp() override {
    Hyrise::get().catalog.add_table("table_a", load_table("resources/test_data/tbl/int_int.tbl", ChunkOffset{2}));
  ...
  void _init_query_nodes(QueryNodes& query_nodes) const {
    query_nodes.stored_table_node_a = StoredTableNode::make(Hyrise::get().catalog.table_id("table_a"));
