...
TEST_F(StoredTableNodeTest, FunctionalDependenciesNone) {
  ...
  table->add_soft_constraints({{_a->original_column_id, _b->original_column_id, _c->original_column_id},
...
TEST_F(StoredTableNodeTest, FunctionalDependenciesSingle) {
  const auto table = Hyrise::get().storage_manager.get_table("t_a");
  table->add_soft_constraints({{_a->original_column_id}, {ConstraintType::UNIQUE}});
...
TEST_F(StoredTableNodeTest, FunctionalDependenciesPrunedLeftColumnSet2) {
  const auto table = Hyrise::get().storage_manager.get_table("t_a");
  table->add_soft_constraints({{_b->original_column_id}, {ConstraintType::UNIQUE}});
...
TEST_F(StoredTableNodeTest, FunctionalDependenciesMultiple) {
  ...
  table->add_soft_constraints({{_a->original_column_id}, {ConstraintType::UNIQUE}});
  table->add_soft_constraints({{_a->original_column_id, _b->original_column_id}, {ConstraintType::UNIQUE}});
...
TEST_F(StoredTableNodeTest, FunctionalDependenciesExcludeNullableColumns) {
  ...
  {
    const auto table = std::make_shared<Table>(column_definitions, TableType::Data);
    table->add_soft_constraints({{ColumnID{0}, ColumnID{1}}, {ConstraintType::UNIQUE}});
  ...
  {
    const auto table = std::make_shared<Table>(column_definitions, TableType::Data);
    table->add_soft_constraints({{ColumnID{1}}, {ConstraintType::UNIQUE}});
