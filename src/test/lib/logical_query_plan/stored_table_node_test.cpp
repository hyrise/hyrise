...
TEST_F(StoredTableNodeTest, FunctionalDependenciesSingle) {
  const auto table = Hyrise::get().storage_manager.get_table("t_a");
  table->add_soft_unique_constraint({{_a->original_column_id}, KeyConstraintType::UNIQUE});
...
TEST_F(StoredTableNodeTest, FunctionalDependenciesPrunedLeftColumnSet2) {
  const auto table = Hyrise::get().storage_manager.get_table("t_a");
  table->add_soft_unique_constraint({{_b->original_column_id}, KeyConstraintType::UNIQUE});
...
TEST_F(StoredTableNodeTest, FunctionalDependenciesMultiple) {
  ...
  table->add_soft_unique_constraint({{_a->original_column_id}, KeyConstraintType::UNIQUE});
  table->add_soft_unique_constraint({{_a->original_column_id, _b->original_column_id}, KeyConstraintType::UNIQUE});
...
TEST_F(StoredTableNodeTest, FunctionalDependenciesExcludeNullableColumns) {
  ...
  {
    const auto table = std::make_shared<Table>(column_definitions, TableType::Data);
    table->add_soft_unique_constraint({{ColumnID{0}, ColumnID{1}}, KeyConstraintType::UNIQUE});
  ...
  {
    const auto table = std::make_shared<Table>(column_definitions, TableType::Data);
    table->add_soft_unique_constraint({{ColumnID{1}}, KeyConstraintType::UNIQUE});
