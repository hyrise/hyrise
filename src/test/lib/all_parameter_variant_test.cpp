...
TEST_F(AllParameterVariantTest, ToString) {
  {
    ...
    EXPECT_TRUE(to_string(parameter) == "Placeholder #17");
  }
  {
    const AllParameterVariant parameter(ColumnID{17});
    EXPECT_TRUE(to_string(parameter) == "Col #17");
  }
  {
    ...
    EXPECT_TRUE(to_string(parameter_col_a) == "a");

    const AllParameterVariant parameter_col_b(col_b);
    EXPECT_TRUE(to_string(parameter_col_b) == "b");
  }
  {
    const AllParameterVariant parameter("string");
    EXPECT_TRUE(to_string(parameter) == "string");
  }
}
