...
TEST_F(PredicatePlacementRuleTest, SimpleLiteralJoinPushdownTest) {
  ...
  join_node->set_input_left(_table_a);
  join_node->set_input_right(_table_b);

  auto predicate_node_0 = std::make_shared<PredicateNode>(greater_than_(_a_a, 10));
  predicate_node_0->set_input_left(join_node);
  ...
  EXPECT_EQ(reordered->input_right(), _table_b);
  EXPECT_EQ(reordered->input_left()->input_left(), _table_a);
}
...
TEST_F(PredicatePlacementRuleTest, SimpleBothSideJoinPushdownTest) {
  ...
  EXPECT_EQ(reordered->input_left(), join_node);
  EXPECT_EQ(reordered->input_left()->input_right(), _table_b);
  EXPECT_EQ(reordered->input_left()->input_left(), _table_a);
}
...
TEST_F(PredicatePlacementRuleTest, ForbiddenPredicatePushdownThroughProjectionTest) {
  ...
  EXPECT_EQ(reordered->input_left(), _projection_pushdown_node);
  EXPECT_EQ(reordered->input_left()->input_left(), _table_a);
}
...
TEST_F(PredicatePlacementRuleTest, PredicatePushdownThroughOtherPredicateTest) {
  ...
  predicate_node_1->set_input_left(_projection_pushdown_node);

  auto predicate_node_2 = std::make_shared<PredicateNode>(greater_than_(_a_a, _a_b));
  predicate_node_2->set_input_left(predicate_node_1);

  auto reordered = StrategyBaseTest::apply_rule(_rule, predicate_node_2);

  EXPECT_EQ(reordered, predicate_node_1);
  EXPECT_EQ(reordered->input_left(), _projection_pushdown_node);
  EXPECT_EQ(reordered->input_left()->input_left(), predicate_node_2);
  EXPECT_EQ(reordered->input_left()->input_left()->input_left(), _table_a);
}
