...
class StrategyBaseTest : public BaseTest {
 protected:
   ...
   * (_lqp = PredicateNode::make(...)->deep_copy()). Always use `EXPECT_LQP_EQ(...);` rather than `EXPECT_EQ(...);` to
   * compare LQPs: the first macro checks that both LQPs are equivalent, the second macro checks that both point to the
   * same instance.
   *
   * Otherwise, the expected LQP also points to rewritten parts. When a rule should not rewrite the LQP, assign the
   * expected LQP to a deep copy of the input LQP BEFORE applying the rule.
   */
