...
TEST_F(PausableLoopThreadTest, BasicExecution) {
  auto loop = PausableLoopThread(std::chrono::milliseconds(10), [&](size_t) {
    ++call_count;
  });
...
TEST_F(PausableLoopThreadTest, PauseAndResume) {
  auto loop = PausableLoopThread(std::chrono::milliseconds(5), [&](size_t) {
    ++call_count;
  });
...
TEST_F(PausableLoopThreadTest, SetLoopSleepTimeChangesInterval) {
  ...
  auto loop = PausableLoopThread(std::chrono::milliseconds(50), [&](size_t) {
  ...
  auto initial_timestamp_count = 0;
  {
    auto lock = std::lock_guard(timestamps_mutex);
