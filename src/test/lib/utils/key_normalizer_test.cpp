...
class KeyNormalizerTest : public BaseTest {
 protected:
  ...
  std::vector<std::byte> get_normalized_key(const std::vector<SortColumnDefinition>& sort_definitions,
                                            const uint32_t string_prefix_length = 12,
  ...
  static std::vector<RowID> get_sorted_row_ids(const std::vector<std::byte>& buffer, uint64_t tuple_key_size) {
    auto key_pointers = std::vector<const std::byte*>{};
    for (auto i = size_t{0}; i < buffer.size(); i += tuple_key_size) {
    ...
    auto sorted_row_ids = std::vector<RowID>{};
  ...
  static RowIDPosList get_sorted_pos_list(const std::vector<std::byte>& buffer, uint64_t key_size) {
    auto key_pointers = std::vector<const std::byte*>{};
    key_pointers.reserve(buffer.size() / key_size);
    for (auto i = size_t{0}; i < buffer.size(); i += key_size) {
      key_pointers.push_back(&buffer[i]);
    }

    std::ranges::sort(key_pointers, [key_size](const auto* a, const auto* b) {
...
TEST_F(KeyNormalizerTest, StringNormalizationAndPadding) {
  ...
  EXPECT_EQ(key_apple[1 + 5], PAD_CHAR);  // 1-byte null prefix + 5 chars
  EXPECT_EQ(key_apple[1 + 6], PAD_CHAR);
  EXPECT_EQ(key_apple[1 + 7], PAD_CHAR);
...
TEST_F(KeyNormalizerTest, TwoColumnsIntFloat) {
  ...
  constexpr auto expected_tuple_size = (sizeof(int32_t) + 1) + (sizeof(float) + 1) + sizeof(RowID);
...
TEST_F(KeyNormalizerTest, StringColumnDescending) {
  ...
  constexpr auto string_prefix_length = uint32_t{8};

  const auto [buffer, tuple_key_size] =
      KeyNormalizer::normalize_keys_for_table(table, sort_definitions, string_prefix_length);

  constexpr auto expected_tuple_size = string_prefix_length + 1 + sizeof(RowID);
  EXPECT_EQ(tuple_key_size, expected_tuple_size);
}
...
TEST_F(KeyNormalizerTest, ComplexSort) {
  ...
  for (auto i = size_t{0}; i < expected_offsets.size(); ++i) {
...
TEST_F(KeyNormalizerTest, DebugFloats) {
  ...
  auto key_pointers = std::vector<const std::byte*>{};
  for (auto i = size_t{0}; i < normalized_keys.size(); i += key_size) {
...
TEST_F(KeyNormalizerTest, StringEdgeCases) {
  ...
  const auto s_with_null = std::string("a\0c", 3);
...
TEST_F(KeyNormalizerTest, MultiColumnWithNullInFirstColumn) {
  const auto multi_null_table = std::make_shared<Table>(
