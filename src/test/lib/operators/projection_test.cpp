...
TEST_F(OperatorsProjectionTest, OperatorName) {
  const auto projection = std::make_shared<hyrise::Projection>(table_wrapper_a, expression_vector(a_a, a_b));
  EXPECT_EQ(projection->name(), "Projection");
}

TEST_F(OperatorsProjectionTest, ExecutedOnAllChunks) {
  const auto projection = std::make_shared<hyrise::Projection>(table_wrapper_a, expression_vector(add_(a_a, a_b)));
  projection->execute();
...
TEST_F(OperatorsProjectionTest, ForwardsDataTable) {
  ...
  const auto projection = std::make_shared<hyrise::Projection>(table_wrapper_a, expression_vector(a_b, a_a));
  projection->execute();
...
TEST_F(OperatorsProjectionTest, ForwardsDataTableAndExpression) {
  const auto projection =
      std::make_shared<hyrise::Projection>(table_wrapper_a, expression_vector(a_b, a_a, add_(a_b, a_a)));
  projection->execute();
...
TEST_F(OperatorsProjectionTest, DoNotForwardEvaluatedColumns) {
  ...
  table_scan->execute();
  const auto projection =
      std::make_shared<hyrise::Projection>(table_scan, expression_vector(a_b, a_a, add_(a_b, a_a)));
  projection->execute();
...
TEST_F(OperatorsProjectionTest, ForwardsReferenceTable) {
  ...
  table_scan->never_clear_output();
  table_scan->execute();
  const auto projection = std::make_shared<hyrise::Projection>(table_scan, expression_vector(a_b, a_a));
  projection->execute();
...
TEST_F(OperatorsProjectionTest, EvaluateForwardableColumns) {
  ...
  const auto projection = std::make_shared<hyrise::Projection>(table_scan, expression_vector(a_a, a_b, add_(a_a, 17)));
...
TEST_F(OperatorsProjectionTest, ExpressionUnorderedSetCheck) {
  ...
  const auto projection =
      std::make_shared<hyrise::Projection>(table_scan, expression_vector(a_a, a_b, add_(a_a2, 17)));
  projection->never_clear_output();
  projection->execute();
