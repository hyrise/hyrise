...
TEST_F(OperatorsProjectionTest, SetParameters) {
  // Cannot use table_wrapper_a & table_wrapper_b because they have already executed
  const auto table_wrapper_a_clone = table_wrapper_a->deep_copy();
  const auto table_wrapper_b_clone = table_wrapper_b->deep_copy();

  // TODO(Julian) create_table_scan calls get_column_expression() which requires input operators to have executed.
  // Maybe create TableScans manually
  const auto table_scan_a = create_table_scan(table_wrapper_b_clone, ColumnID{1}, PredicateCondition::GreaterThan, 5);
  ...
  const auto projection_b = std::make_shared<Projection>(
      table_wrapper_a_clone, expression_vector(correlated_parameter_(ParameterID{2}, a_a), subquery_expression));
