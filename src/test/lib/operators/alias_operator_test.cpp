...
TEST_F(AliasOperatorTest, ForwardSortedByFlagWithFlippedColumns) {
  alias_operator->execute();
  const auto result_table_unsorted = alias_operator->get_output();
  ...
  const auto sort_definition =
      std::vector<SortColumnDefinition>{SortColumnDefinition{ColumnID{0}}, SortColumnDefinition{ColumnID{2}}};
  auto sort = std::make_shared<Sort>(table_wrapper, sort_definition);
  sort->execute();
  ...
  for (ChunkID chunk_id{0}; chunk_id < result_table_sorted->chunk_count(); ++chunk_id) {
    const auto& sorted_by = result_table_sorted->get_chunk(chunk_id)->individually_sorted_by();
    ASSERT_EQ(sorted_by.size(), 2);
    EXPECT_EQ(sorted_by[0].column, ColumnID{1});
    EXPECT_EQ(sorted_by[0].sort_mode, SortMode::AscendingNullsFirst);
    EXPECT_EQ(sorted_by[1].column, ColumnID{0});
    EXPECT_EQ(sorted_by[1].sort_mode, SortMode::AscendingNullsFirst);
  }
}
