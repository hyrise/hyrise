...
TEST_F(CacheInfoTest, SanitizeAppliesFallbacksForUnknownSizes) {
  const auto from_zero = sanitize_cache_sizes(0, 0, 0);
  ...
  const auto from_error = sanitize_cache_sizes(-1, -1, -1);
...
TEST_F(CacheInfoTest, SanitizePassesThroughPlausibleSizes) {
  const auto sizes = sanitize_cache_sizes(48 * 1024, 1'280 * 1024, 32 * 1024 * 1024);
...
TEST_F(CacheInfoTest, SanitizeClampsImplausibleSizes) {
  const auto tiny = sanitize_cache_sizes(16, 1024, 4096);
  EXPECT_GE(tiny.l1d_bytes, 4 * 1024);
  EXPECT_GE(tiny.l2_bytes, 64 * 1024);
  EXPECT_GE(tiny.l3_bytes, 1024 * 1024);

  const auto huge = sanitize_cache_sizes(int64_t{1} << 40, int64_t{1} << 40, int64_t{1} << 40);
...
TEST_F(CacheInfoTest, SanitizeKeepsLevelsOrdered) {
  const auto sizes = sanitize_cache_sizes(512 * 1024, 128 * 1024, 0);
