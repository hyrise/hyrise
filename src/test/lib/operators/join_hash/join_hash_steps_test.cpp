...
TEST_F(JoinHashStepsTest, SmallHashTableAllPositions) {
  ...
  const auto expected_pos_list = PostHashTable<int>::SmallPosList{RowID{ChunkID{105}, ChunkOffset{205}},
...
TEST_F(JoinHashStepsTest, LargeHashTableSinglePositions) {
  ...
  const auto expected_pos_list = PostHashTable<int>::SmallPosList{RowID{ChunkID{150}, ChunkOffset{250}}};
  {
    EXPECT_TRUE(table.contains(5));
    EXPECT_FALSE(table.contains(1000));
  }
