...
Matrix tableToMatrix(const Table &t) {
  Matrix matrix;
...
void printMatrix(const std::vector<std::vector<AllTypeVariant>> &m) {
  std::cout << "-------------" << std::endl;
...
::testing::AssertionResult tablesEqual(const Table &tleft, const Table &tright, bool order_sensitive) {
