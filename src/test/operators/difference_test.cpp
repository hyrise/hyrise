...
class OperatorsDifferenceTest : public BaseTest {
 protected:
  virtual void SetUp() {
    ...
    _table_wrapper_b = std::make_shared<TableWrapper>(load_table_cached("src/test/tables/int_float3.tbl", 2));
...
TEST_F(OperatorsDifferenceTest, ThrowWrongColumnNumberException) {
  if (!IS_DEBUG) return;
  auto table_wrapper_c = std::make_shared<TableWrapper>(load_table_cached("src/test/tables/int.tbl", 2));
...
TEST_F(OperatorsDifferenceTest, ThrowWrongColumnOrderException) {
  ...
  auto table_wrapper_d = std::make_shared<TableWrapper>(load_table_cached("src/test/tables/float_int.tbl", 2));
