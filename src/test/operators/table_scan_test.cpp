...
class OperatorsTableScanTest : public BaseTest, public ::testing::WithParamInterface<EncodingType> {
 protected:
  ...
  std::shared_ptr<TableWrapper> get_table_op_even_dict() {
    ...
    DictionaryCompression::compress_chunks(*test_even_dict, {ChunkID{0}, ChunkID{1}}, _encoding_type);
  ...
  std::shared_ptr<TableWrapper> get_table_op_part_dict() {
    ...
    DictionaryCompression::compress_chunks(*table, {ChunkID{0}, ChunkID{2}}, _encoding_type);
  ...
  std::shared_ptr<TableWrapper> get_table_op_with_n_dict_entries(const int num_entries) {
    ...
    DictionaryCompression::compress_chunks(*table, {ChunkID{0}}, _encoding_type);
  ...
  std::shared_ptr<const Table> create_referencing_table_w_null_row_id(const bool references_dict_column) {
    ...
    if (references_dict_column) {
      DictionaryCompression::compress_table(*table, _encoding_type);
    }
...
TEST_P(OperatorsTableScanTest, DoubleScan) {
  ...
  auto scan_1 = std::make_shared<TableScan>(get_table_op(), ColumnID{0}, ScanType::OpGreaterThanEquals, 1234);
  scan_1->execute();
...
TEST_P(OperatorsTableScanTest, EmptyResultScan) {
  auto scan_1 = std::make_shared<TableScan>(get_table_op(), ColumnID{0}, ScanType::OpGreaterThan, 90000);
  scan_1->execute();
...
TEST_P(OperatorsTableScanTest, SingleScanReturnsCorrectRowCount) {
  ...
  auto scan = std::make_shared<TableScan>(get_table_op(), ColumnID{0}, ScanType::OpGreaterThanEquals, 1234);
  scan->execute();
...
TEST_P(OperatorsTableScanTest, ScanOnReferencedDictColumn) {
  ...
  for (const auto& test : tests) {
    auto scan1 = std::make_shared<TableScan>(get_table_op_even_dict(), ColumnID{1}, ScanType::OpLessThan, 108);
    scan1->execute();
...
TEST_P(OperatorsTableScanTest, ScanWithEmptyInput) {
  auto scan_1 = std::make_shared<opossum::TableScan>(get_table_op(), ColumnID{0}, ScanType::OpGreaterThan, 12345);
  scan_1->execute();
...
TEST_P(OperatorsTableScanTest, OperatorName) {
  auto scan_1 = std::make_shared<opossum::TableScan>(get_table_op(), ColumnID{0}, ScanType::OpGreaterThanEquals, 1234);
