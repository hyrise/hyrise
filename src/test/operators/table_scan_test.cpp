...
class OperatorsTableScanTest : public BaseTest {
 protected:
  void SetUp() override {
    ...
    StorageManager::get().add_table("table_int3_dict", test_table_int3_dict);
    {
      auto compression = std::make_unique<ChunkCompression>("table_int3_dict", std::vector<ChunkID>{0u, 1u}, false);
      compression->execute();
    }
