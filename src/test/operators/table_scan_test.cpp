...
class OperatorsTableScanTest : public BaseTest, public ::testing::WithParamInterface<EncodingType> {
 protected:
  ...
  std::shared_ptr<TableWrapper> get_table_op() {
    auto table_wrapper = std::make_shared<TableWrapper>(load_table_cached("src/test/tables/int_float.tbl", 2));
    table_wrapper->execute();
    return table_wrapper;
  }

  std::shared_ptr<TableWrapper> get_int_string_table_op() {
    auto table_wrapper = std::make_shared<TableWrapper>(load_table_cached("src/test/tables/int_string.tbl", 2));
    table_wrapper->execute();
    return table_wrapper;
  }

  std::shared_ptr<TableWrapper> get_table_op_null() {
    auto table_wrapper_null = std::make_shared<TableWrapper>(load_table_cached("src/test/tables/int_float_with_null.tbl", 2));
  ...
  std::shared_ptr<const Table> create_referencing_table_w_null_row_id(const bool references_dict_segment) {
    const auto table = load_table_cached("src/test/tables/int_int_w_null_8_rows.tbl", 4);
...
TEST_P(OperatorsTableScanTest, ScanOnIntValueSegmentWithFloatColumnWithNullValues) {
  auto table = load_table_cached("src/test/tables/int_int_w_null_8_rows.tbl", 4);
...
TEST_P(OperatorsTableScanTest, ScanForNullValuesOnValueSegment) {
  auto table_wrapper = std::make_shared<TableWrapper>(load_table_cached("src/test/tables/int_int_w_null_8_rows.tbl", 4));
  table_wrapper->execute();
...
TEST_P(OperatorsTableScanTest, ScanForNullValuesOnValueSegmentWithoutNulls) {
  auto table = load_table_cached("src/test/tables/int_float.tbl", 4);
