...
class OperatorDeepCopyTest : public BaseTest {
 protected:
  void SetUp() override {
    ...
    _table_wrapper_c = std::make_shared<TableWrapper>(load_table_cached("src/test/tables/int_float3.tbl", 2));
    _table_wrapper_d = std::make_shared<TableWrapper>(load_table_cached("src/test/tables/int_int3.tbl", 2));
    ...
    _test_table = load_table_cached("src/test/tables/int_float.tbl", 2);
...
TYPED_TEST(DeepCopyTestJoin, DeepCopyJoin) {
  std::shared_ptr<Table> expected_result = load_table_cached("src/test/tables/joinoperators/int_left_join.tbl", 1);
...
TEST_F(OperatorDeepCopyTest, DeepCopyDifference) {
  std::shared_ptr<Table> expected_result = load_table_cached("src/test/tables/int_float_filtered2.tbl", 2);
...
TEST_F(OperatorDeepCopyTest, DeepCopyLimit) {
  std::shared_ptr<Table> expected_result = load_table_cached("src/test/tables/int_int3_limit_1.tbl", 1);
...
TEST_F(OperatorDeepCopyTest, DeepCopySort) {
  std::shared_ptr<Table> expected_result = load_table_cached("src/test/tables/int_float_sorted.tbl", 1);
...
TEST_F(OperatorDeepCopyTest, DeepCopyTableScan) {
  std::shared_ptr<Table> expected_result = load_table_cached("src/test/tables/int_float_filtered2.tbl", 1);
...
TEST_F(OperatorDeepCopyTest, Subselect) {
  ...
  const auto table = load_table_cached("src/test/tables/int_int_int.tbl", 2);
