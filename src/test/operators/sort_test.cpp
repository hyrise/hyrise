...
TEST_F(OperatorsSortTest, AscendingSortOfOneColumn) {
  ...
  EXPECT_ORDER_DEPENDENT_TABLE_EQ(sort->get_output(), expected_result);
}
...
TEST_F(OperatorsSortTest, DoubleSortOfOneColumn) {
  ...
  EXPECT_ORDER_DEPENDENT_TABLE_EQ(sort2->get_output(), expected_result);
}
...
TEST_F(OperatorsSortTest, MultipleColumnSortIsStable) {
  ...
  EXPECT_ORDER_DEPENDENT_TABLE_EQ(sort_after_a->get_output(), expected_result);
}
...
TEST_F(OperatorsSortTest, SortTableWithRefandValueColumns) {
  ...
  DEFAULT_EXPECT_TABLE_EQ(union_all->get_output(), load_table("src/test/tables/int_float__int_float2_filtered__union.tbl", 2));

  auto sort = std::make_shared<Sort>(union_all, ColumnID{1});
  sort->execute();

  EXPECT_ORDER_DEPENDENT_TABLE_EQ(sort->get_output(),
