...
class AggregateSortedTest : public BaseTest {
 public:
  void SetUp() override {
    ...
    _table_sorted_value_clustered = load_table("resources/test_data/tbl/int_sorted_value_clustered.tbl", 6);
  }
...
TYPED_TEST(OperatorsAggregateTest, CanCountStringColumnsWithNull) {
  test_output<TypeParam>(this->_table_wrapper_1_1_string_null, {{ColumnID{1}, AggregateFunction::Count}}, {ColumnID{0}},
                         "resources/test_data/tbl/aggregateoperator/groupby_string_1gb_1agg/count_str_null.tbl", 1,
                         false);
}
...
TYPED_TEST(OperatorsAggregateTest, OneGroupbyCountStarWithNull) {
  test_output<TypeParam>(this->_table_wrapper_1_1_null, {{INVALID_COLUMN_ID, AggregateFunction::Count}}, {ColumnID{0}},
                         "resources/test_data/tbl/aggregateoperator/groupby_int_1gb_1agg/count_star_null.tbl", 1,
                         false);
}
...
TEST_F(AggregateSortedTest, SingleAggregateMaxSorted) {
  for (ColumnID sort_by_column_id = ColumnID{0}; sort_by_column_id <= ColumnID{1}; ++sort_by_column_id) {
    const auto sort = std::make_shared<Sort>(this->_table_wrapper_1_1, sort_by_column_id);
    sort->execute();
...
TEST_F(AggregateSortedTest, AggregateMaxMultiColumnSorted) {
  ...
  for (ColumnID sorted_by_column_id = ColumnID{0}; sorted_by_column_id < column_count; ++sorted_by_column_id) {
    const auto sort = std::make_shared<Sort>(this->_table_wrapper_multi_columns, sorted_by_column_id);
    sort->execute();
...
TEST_F(AggregateSortedTest, AggregateSetsOrderedBy) {
  const auto sort = std::make_shared<Sort>(this->_table_wrapper_multi_columns, ColumnID{1});
  sort->execute();
