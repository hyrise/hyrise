...
TEST_F(JitOperatorWrapperTest, JitOperatorsSpecializedWithMultipleInliningOfSameFunction) {
  ...
  // then the two function calls with the two time referenced object "cxlumn_expression" are inlined.

  // Specialize SQL query: SELECT a+a FROM src/test/tables/10_ints.tbl;

  // read cxlumn a into jit tuple at index 0
  auto read_operator = std::make_shared<JitReadTuples>();
  auto tuple_value = read_operator->add_input_cxlumn(DataType::Int, false, CxlumnID(0));

  // compute a+a and write result to jit tuple at index 1
  auto cxlumn_expression = std::make_shared<JitExpression>(tuple_value);
  auto add_type = JitExpressionType::Addition;
  auto result_tuple_index = read_operator->add_temporary_value();  // of jit runtime context
  auto expression = std::make_shared<JitExpression>(cxlumn_expression, add_type, cxlumn_expression, result_tuple_index);
  ...
  write_operator->add_output_cxlumn("a+a", expression->result());
