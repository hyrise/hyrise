...
class OperatorsTableScanStringTest : public BaseTest, public ::testing::WithParamInterface<EncodingType> {
 protected:
  void SetUp() override {
    std::shared_ptr<Table> test_table = load_table_cached("src/test/tables/int_float.tbl", 2);
    StorageManager::get().add_table("table_a", std::move(test_table));
    _gt = std::make_shared<GetTable>("table_a");
    _gt->execute();

    // load string table
    std::shared_ptr<Table> test_table_string = load_table_cached("src/test/tables/int_string_like.tbl", 2);
    ...
    std::shared_ptr<Table> test_table_special_chars =
        load_table_cached("src/test/tables/int_string_like_special_chars.tbl", 2);
    ...
    if (::testing::UnitTest::GetInstance()->current_test_info()->value_param()) {
      ...
      auto test_table_string_compressed = load_table_cached("src/test/tables/int_string_like.tbl", 5);
...
TEST_P(OperatorsTableScanStringTest, ScanEquals) {
  auto scan = create_table_scan(_gt_string_compressed, ColumnID{1}, PredicateCondition::Equals, "Reeperbahn");
  scan->execute();
  EXPECT_EQ(scan->get_output()->row_count(), 1u);
  std::shared_ptr<Table> expected_result = load_table_cached("src/test/tables/int_string_like_equals.tbl", 1);
  EXPECT_TABLE_EQ_UNORDERED(scan->get_output(), expected_result);
}

TEST_P(OperatorsTableScanStringTest, ScanNotEquals) {
  auto scan = create_table_scan(_gt_string_compressed, ColumnID{1}, PredicateCondition::NotEquals, "Reeperbahn");
  scan->execute();
  EXPECT_EQ(scan->get_output()->row_count(), 5u);
  std::shared_ptr<Table> expected_result = load_table_cached("src/test/tables/int_string_like_not_equals.tbl", 1);
  EXPECT_TABLE_EQ_UNORDERED(scan->get_output(), expected_result);
}
...
TEST_P(OperatorsTableScanStringTest, ScanLessThan) {
  ...
  scan->execute();
  EXPECT_EQ(scan->get_output()->row_count(), 5u);
  std::shared_ptr<Table> expected_result = load_table_cached("src/test/tables/int_string_like_less_than.tbl", 1);
  EXPECT_TABLE_EQ_UNORDERED(scan->get_output(), expected_result);
}
...
TEST_F(OperatorsTableScanStringTest, ScanLikeEmptyString) {
  std::shared_ptr<Table> expected_result = load_table_cached("src/test/tables/int_string_like.tbl", 1);
  // wildcard has to be placed at front and/or back of search string
  auto scan = create_table_scan(_gt_string, ColumnID{1}, PredicateCondition::Like, "%");
  scan->execute();
  EXPECT_TABLE_EQ_UNORDERED(scan->get_output(), expected_result);
}
...
TEST_P(OperatorsTableScanStringTest, ScanLikeEmptyStringOnDict) {
  std::shared_ptr<Table> expected_result = load_table_cached("src/test/tables/int_string_like.tbl", 1);
  // wildcard has to be placed at front and/or back of search string
  auto scan = create_table_scan(_gt_string_compressed, ColumnID{1}, PredicateCondition::Like, "%");
  scan->execute();
  EXPECT_TABLE_EQ_UNORDERED(scan->get_output(), expected_result);
}

TEST_F(OperatorsTableScanStringTest, ScanLikeUnderscoreWildcard) {
  std::shared_ptr<Table> expected_result = load_table_cached("src/test/tables/int_string_like_starting.tbl", 1);
  // wildcard has to be placed at front and/or back of search string
  auto scan = create_table_scan(_gt_string, ColumnID{1}, PredicateCondition::Like, "%D%_m_f%");
  scan->execute();
  EXPECT_TABLE_EQ_UNORDERED(scan->get_output(), expected_result);
}
...
TEST_F(OperatorsTableScanStringTest, ScanLike_Starting) {
  std::shared_ptr<Table> expected_result = load_table_cached("src/test/tables/int_string_like_starting.tbl", 1);
  auto scan = create_table_scan(_gt_string, ColumnID{1}, PredicateCondition::Like, "Dampf%");
  scan->execute();
  EXPECT_TABLE_EQ_UNORDERED(scan->get_output(), expected_result);
}
...
TEST_P(OperatorsTableScanStringTest, ScanLikeStartingOnDictSegment) {
  std::shared_ptr<Table> expected_result = load_table_cached("src/test/tables/int_string_like_starting.tbl", 1);
  auto scan = create_table_scan(_gt_string_compressed, ColumnID{1}, PredicateCondition::Like, "Dampf%");
  scan->execute();
  EXPECT_TABLE_EQ_UNORDERED(scan->get_output(), expected_result);
}
...
TEST_F(OperatorsTableScanStringTest, ScanLikeEnding) {
  std::shared_ptr<Table> expected_result = load_table_cached("src/test/tables/int_string_like_ending.tbl", 1);
  auto scan = create_table_scan(_gt_string, ColumnID{1}, PredicateCondition::Like, "%gesellschaft");
  scan->execute();
  EXPECT_TABLE_EQ_UNORDERED(scan->get_output(), expected_result);
}
...
TEST_F(OperatorsTableScanStringTest, ScanLikeOnSpecialChars) {
  std::shared_ptr<Table> expected_result_1 = load_table_cached("src/test/tables/int_string_like_special_chars_1.tbl", 1);
  std::shared_ptr<Table> expected_result_2 = load_table_cached("src/test/tables/int_string_like_special_chars_2.tbl", 1);
  std::shared_ptr<Table> expected_result_4 = load_table_cached("src/test/tables/int_string_like_special_chars_3.tbl", 1);
  ...
  std::shared_ptr<Table> expected_result2 = load_table_cached("src/test/tables/int_string_like_special_chars_2.tbl", 1);
...
TEST_F(OperatorsTableScanStringTest, ScanLikeContainingWildcard) {
  std::shared_ptr<Table> expected_result = load_table_cached("src/test/tables/int_string_like_containing_wildcard.tbl", 1);
...
TEST_F(OperatorsTableScanStringTest, ScanLikeContaining) {
  std::shared_ptr<Table> expected_result = load_table_cached("src/test/tables/int_string_like_containing.tbl", 1);
...
TEST_F(OperatorsTableScanStringTest, ScanLikeNotFound) {
  auto scan = create_table_scan(_gt_string, ColumnID{1}, PredicateCondition::Like, "%not_there%");
  scan->execute();
...
TEST_F(OperatorsTableScanStringTest, ScanNotLikeEmptyString) {
  // wildcard has to be placed at front and/or back of search string
  auto scan = create_table_scan(_gt_string, ColumnID{1}, PredicateCondition::NotLike, "%");
  scan->execute();
  EXPECT_EQ(scan->get_output()->row_count(), 0u);
}
...
TEST_P(OperatorsTableScanStringTest, ScanNotLikeAllRowsOnDict) {
  std::shared_ptr<Table> expected_result = load_table_cached("src/test/tables/int_string_like.tbl", 1);
  // wildcard has to be placed at front and/or back of search string
  auto scan = create_table_scan(_gt_string_compressed, ColumnID{1}, PredicateCondition::NotLike, "%foo%");
  scan->execute();
  EXPECT_TABLE_EQ_UNORDERED(scan->get_output(), expected_result);
}

TEST_F(OperatorsTableScanStringTest, ScanNotLikeUnderscoreWildcard) {
  std::shared_ptr<Table> expected_result = load_table_cached("src/test/tables/int_string_like_not_starting.tbl", 1);
  // wildcard has to be placed at front and/or back of search string
  auto scan = create_table_scan(_gt_string, ColumnID{1}, PredicateCondition::NotLike, "D_m_f%");
  scan->execute();
  EXPECT_TABLE_EQ_UNORDERED(scan->get_output(), expected_result);
}
