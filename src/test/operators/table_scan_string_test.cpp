...
TEST_P(OperatorsTableScanStringTest, ScanEquals) {
  auto scan = std::make_shared<TableScan>(_gt_string_compressed,
                                          OperatorScanPredicate{ColumnID{1}, PredicateCondition::Equals, "Reeperbahn"});
  scan->execute();
...
TEST_P(OperatorsTableScanStringTest, ScanNotEquals) {
  auto scan = std::make_shared<TableScan>(
      _gt_string_compressed, OperatorScanPredicate{ColumnID{1}, PredicateCondition::NotEquals, "Reeperbahn"});
  scan->execute();
...
TEST_P(OperatorsTableScanStringTest, ScanLessThan) {
  auto scan = std::make_shared<TableScan>(_gt_string_compressed,
                                          OperatorScanPredicate{ColumnID{1}, PredicateCondition::LessThan, "Schiff"});
  scan->execute();
  EXPECT_EQ(scan->get_output()->row_count(), 5u);
...
TEST_F(OperatorsTableScanStringTest, ScanLikeNonStringColumn) {
  auto scan = std::make_shared<TableScan>(_gt, OperatorScanPredicate{ColumnID{0}, PredicateCondition::Like, "%test"});
  EXPECT_THROW(scan->execute(), std::exception);
}

TEST_F(OperatorsTableScanStringTest, ScanLikeNonStringValue) {
  auto scan =
      std::make_shared<TableScan>(_gt_string, OperatorScanPredicate{ColumnID{1}, PredicateCondition::Like, 1234});
  scan->execute();
  EXPECT_EQ(scan->get_output()->row_count(), 1u);
}
...
TEST_F(OperatorsTableScanStringTest, ScanLikeEmptyString) {
  ...
  auto scan =
      std::make_shared<TableScan>(_gt_string, OperatorScanPredicate{ColumnID{1}, PredicateCondition::Like, "%"});
  scan->execute();
  EXPECT_TABLE_EQ_UNORDERED(scan->get_output(), expected_result);
}
...
TEST_P(OperatorsTableScanStringTest, ScanLikeEmptyStringOnDict) {
  ...
  // wildcard has to be placed at front and/or back of search string
  auto scan = std::make_shared<TableScan>(_gt_string_compressed,
                                          OperatorScanPredicate{ColumnID{1}, PredicateCondition::Like, "%"});
  scan->execute();
  EXPECT_TABLE_EQ_UNORDERED(scan->get_output(), expected_result);
}
...
TEST_F(OperatorsTableScanStringTest, ScanLikeUnderscoreWildcard) {
  ...
  // wildcard has to be placed at front and/or back of search string
  auto scan =
      std::make_shared<TableScan>(_gt_string, OperatorScanPredicate{ColumnID{1}, PredicateCondition::Like, "%D%_m_f%"});
  scan->execute();
  EXPECT_TABLE_EQ_UNORDERED(scan->get_output(), expected_result);
}
...
TEST_F(OperatorsTableScanStringTest, ScanLike_Starting) {
  std::shared_ptr<Table> expected_result = load_table("src/test/tables/int_string_like_starting.tbl", 1);
  auto scan =
      std::make_shared<TableScan>(_gt_string, OperatorScanPredicate{ColumnID{1}, PredicateCondition::Like, "Dampf%"});
  scan->execute();
  EXPECT_TABLE_EQ_UNORDERED(scan->get_output(), expected_result);
}
...
TEST_P(OperatorsTableScanStringTest, ScanLikeStartingOnDictSegment) {
  std::shared_ptr<Table> expected_result = load_table("src/test/tables/int_string_like_starting.tbl", 1);
  auto scan = std::make_shared<TableScan>(_gt_string_compressed,
                                          OperatorScanPredicate{ColumnID{1}, PredicateCondition::Like, "Dampf%"});
  scan->execute();
  EXPECT_TABLE_EQ_UNORDERED(scan->get_output(), expected_result);
}
...
TEST_P(OperatorsTableScanStringTest, ScanLikeStartingOnReferencedDictSegment) {
  std::shared_ptr<Table> expected_result = load_table("src/test/tables/int_string_like_starting.tbl", 1);
  auto scan1 = std::make_shared<TableScan>(_gt_string_compressed,
                                           OperatorScanPredicate{ColumnID{0}, PredicateCondition::GreaterThan, 0});
  scan1->execute();
  auto scan2 =
      std::make_shared<TableScan>(scan1, OperatorScanPredicate{ColumnID{1}, PredicateCondition::Like, "Dampf%"});
  scan2->execute();
...
TEST_F(OperatorsTableScanStringTest, ScanLikeEnding) {
  ...
  auto scan = std::make_shared<TableScan>(
      _gt_string, OperatorScanPredicate{ColumnID{1}, PredicateCondition::Like, "%gesellschaft"});
  scan->execute();
  EXPECT_TABLE_EQ_UNORDERED(scan->get_output(), expected_result);
}
...
TEST_P(OperatorsTableScanStringTest, ScanLikeEndingOnDictSegment) {
  std::shared_ptr<Table> expected_result = load_table("src/test/tables/int_string_like_ending.tbl", 1);
  auto scan = std::make_shared<TableScan>(
      _gt_string_compressed, OperatorScanPredicate{ColumnID{1}, PredicateCondition::Like, "%gesellschaft"});
  scan->execute();
  EXPECT_TABLE_EQ_UNORDERED(scan->get_output(), expected_result);
}
...
TEST_P(OperatorsTableScanStringTest, ScanLikeEndingOnReferencedDictSegment) {
  ...
  scan1->execute();
  auto scan2 =
      std::make_shared<TableScan>(scan1, OperatorScanPredicate{ColumnID{1}, PredicateCondition::Like, "%gesellschaft"});
  scan2->execute();
  EXPECT_TABLE_EQ_UNORDERED(scan2->get_output(), expected_result);
}
...
TEST_F(OperatorsTableScanStringTest, ScanLikeOnSpecialChars) {
  ...
  auto scan1 = std::make_shared<TableScan>(_gt_special_chars,
                                           OperatorScanPredicate{ColumnID{1}, PredicateCondition::Like, "%2^2%"});
  scan1->execute();
  EXPECT_TABLE_EQ_UNORDERED(scan1->get_output(), expected_result_1);

  auto scan2 = std::make_shared<TableScan>(_gt_special_chars,
                                           OperatorScanPredicate{ColumnID{1}, PredicateCondition::Like, "%$%$%"});
  scan2->execute();
  ...
  auto scan3 = std::make_shared<TableScan>(_gt_special_chars,
                                           OperatorScanPredicate{ColumnID{1}, PredicateCondition::Like, "%(%)%"});
  scan3->execute();
  EXPECT_TABLE_EQ_UNORDERED(scan3->get_output(), expected_result_2);

  auto scan4 = std::make_shared<TableScan>(
      _gt_special_chars, OperatorScanPredicate{ColumnID{1}, PredicateCondition::Like, "%la\\.^$+?)({}.*__bl%"});
  scan4->execute();
...
TEST_F(OperatorsTableScanStringTest, ScanLikeContainingWildcard) {
  ...
  auto scan = std::make_shared<TableScan>(
      _gt_string, OperatorScanPredicate{ColumnID{1}, PredicateCondition::Like, "Schiff%schaft"});
  scan->execute();
  EXPECT_TABLE_EQ_UNORDERED(scan->get_output(), expected_result);
}
...
TEST_F(OperatorsTableScanStringTest, ScanLikeContaining) {
  ...
  auto scan = std::make_shared<TableScan>(
      _gt_string, OperatorScanPredicate{ColumnID{1}, PredicateCondition::Like, "%schifffahrtsgesellschaft%"});
  scan->execute();
  EXPECT_TABLE_EQ_UNORDERED(scan->get_output(), expected_result);
}
...
TEST_P(OperatorsTableScanStringTest, ScanLikeContainingOnDictSegment) {
  std::shared_ptr<Table> expected_result = load_table("src/test/tables/int_string_like_containing.tbl", 1);
  auto scan = std::make_shared<TableScan>(
      _gt_string_compressed,
      OperatorScanPredicate{ColumnID{1}, PredicateCondition::Like, "%schifffahrtsgesellschaft%"});
  scan->execute();
  EXPECT_TABLE_EQ_UNORDERED(scan->get_output(), expected_result);
}
...
TEST_P(OperatorsTableScanStringTest, ScanLikeContainingOnReferencedDictSegment) {
  ...
  auto scan1 = std::make_shared<TableScan>(_gt_string_compressed,
                                           OperatorScanPredicate{ColumnID{0}, PredicateCondition::GreaterThan, 0});
  scan1->execute();
  auto scan2 = std::make_shared<TableScan>(
      scan1, OperatorScanPredicate{ColumnID{1}, PredicateCondition::Like, "%schifffahrtsgesellschaft%"});
  scan2->execute();
  EXPECT_TABLE_EQ_UNORDERED(scan2->get_output(), expected_result);
}
...
TEST_F(OperatorsTableScanStringTest, ScanLikeNotFound) {
  auto scan = std::make_shared<TableScan>(_gt_string,
                                          OperatorScanPredicate{ColumnID{1}, PredicateCondition::Like, "%not_there%"});
  scan->execute();
...
TEST_P(OperatorsTableScanStringTest, ScanLikeNotFoundOnReferencedDictSegment) {
  ...
  scan1->execute();
  auto scan2 =
      std::make_shared<TableScan>(scan1, OperatorScanPredicate{ColumnID{1}, PredicateCondition::Like, "%not_there%"});
  scan2->execute();
...
TEST_F(OperatorsTableScanStringTest, ScanNotLikeEmptyString) {
  // wildcard has to be placed at front and/or back of search string
  auto scan =
      std::make_shared<TableScan>(_gt_string, OperatorScanPredicate{ColumnID{1}, PredicateCondition::NotLike, "%"});
  scan->execute();
  EXPECT_EQ(scan->get_output()->row_count(), 0u);
}
...
TEST_P(OperatorsTableScanStringTest, ScanNotLikeEmptyStringOnDict) {
  // wildcard has to be placed at front and/or back of search string
  auto scan = std::make_shared<TableScan>(_gt_string_compressed,
                                          OperatorScanPredicate{ColumnID{1}, PredicateCondition::NotLike, "%"});
  scan->execute();
  EXPECT_EQ(scan->get_output()->row_count(), 0u);
}
...
TEST_F(OperatorsTableScanStringTest, ScanNotLikeAllRows) {
  ...
  // wildcard has to be placed at front and/or back of search string
  auto scan =
      std::make_shared<TableScan>(_gt_string, OperatorScanPredicate{ColumnID{1}, PredicateCondition::NotLike, "%foo%"});
  scan->execute();
  EXPECT_TABLE_EQ_UNORDERED(scan->get_output(), expected_result);
}
...
TEST_P(OperatorsTableScanStringTest, ScanNotLikeAllRowsOnDict) {
  ...
  // wildcard has to be placed at front and/or back of search string
  auto scan = std::make_shared<TableScan>(_gt_string_compressed,
                                          OperatorScanPredicate{ColumnID{1}, PredicateCondition::NotLike, "%foo%"});
  scan->execute();
  EXPECT_TABLE_EQ_UNORDERED(scan->get_output(), expected_result);
}
...
TEST_F(OperatorsTableScanStringTest, ScanNotLikeUnderscoreWildcard) {
  ...
  // wildcard has to be placed at front and/or back of search string
  auto scan = std::make_shared<TableScan>(_gt_string,
                                          OperatorScanPredicate{ColumnID{1}, PredicateCondition::NotLike, "D_m_f%"});
  scan->execute();
  EXPECT_TABLE_EQ_UNORDERED(scan->get_output(), expected_result);
}
