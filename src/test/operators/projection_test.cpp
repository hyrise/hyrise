...
class OperatorsProjectionTest : public BaseTest {
 public:
  void SetUp() override {
    table_wrapper_a = std::make_shared<TableWrapper>(load_table_cached("src/test/tables/int_float.tbl", 2));
    table_wrapper_a->execute();
    table_wrapper_b = std::make_shared<TableWrapper>(load_table_cached("src/test/tables/int_float.tbl", 2));
...
TEST_F(OperatorsProjectionTest, ExecutedOnAllChunks) {
  ...
  projection->execute();
  EXPECT_TABLE_EQ_UNORDERED(projection->get_output(), load_table_cached("src/test/tables/projection/int_float_add.tbl"));
}
