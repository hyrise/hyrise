...
TEST_F(JitHashmapValueTest, Accessors) {
  {
    ...
    ASSERT_EQ(hashmap_value.cxlumn_index(), 123u);
  }
  {
    ...
    ASSERT_EQ(hashmap_value.cxlumn_index(), 456u);
  }
  {
    JitHashmapValue hashmap_value{DataType::Double, true, 789u};
    ASSERT_EQ(hashmap_value.data_type(), DataType::Double);
    ASSERT_EQ(hashmap_value.is_nullable(), true);
    ASSERT_EQ(hashmap_value.cxlumn_index(), 789u);
  }
}

TEST_F(JitHashmapValueTest, GetAndSet) {
  JitRuntimeContext context;
  // Create a hashmap data structure with three cxlumns with ten elements each
  context.hashmap.cxlumns.resize(3);
  context.hashmap.cxlumns[0].resize(10);
  context.hashmap.cxlumns[1].resize(10);
  context.hashmap.cxlumns[2].resize(10);
  ...
  const auto typed_test = [&](const DataType data_type, auto value_in) {
    ...
    const auto cxlumn_index = static_cast<size_t>(std::rand()) % 3;
...
TEST_F(JitHashmapValueTest, IsNullAndSetIsNull) {
...

  {
    const auto cxlumn_index = static_cast<size_t>(std::rand()) % 3;
    const auto row_index = static_cast<size_t>(std::rand()) % 10;
    JitHashmapValue hashmap_value{DataType::Int, true, cxlumn_index};
