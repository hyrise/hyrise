...
TEST_F(OperatorsUnionAllTest, UnionOfValueTables) {
  std::shared_ptr<Table> expected_result = load_table("artifacts/test_data/tbl/int_float_union.tbl", 2);
...
TEST_F(OperatorsUnionAllTest, ThrowWrongColumnNumberException) {
  ...
  std::shared_ptr<Table> test_table_c = load_table("artifacts/test_data/tbl/int.tbl", 2);
...
TEST_F(OperatorsUnionAllTest, ThrowWrongColumnOrderException) {
  if (!HYRISE_DEBUG) GTEST_SKIP();
  std::shared_ptr<Table> test_table_d = load_table("artifacts/test_data/tbl/float_int.tbl", 2);
