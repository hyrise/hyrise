...
void OperatorsUpdateTest::helper(std::shared_ptr<GetTable> table_to_update, std::shared_ptr<GetTable> update_values,
  ...
  auto ref_table = std::make_shared<TableScan>(table_to_update, greater_than_(get_column_expression(table_to_update, ColumnID{0}), value_(0)));
...
TEST_F(OperatorsUpdateTest, MissingChunks) {
  ...
  auto table_scan1 =
      std::make_shared<TableScan>(gt, equals_(get_column_expression(gt, ColumnID{0}), value_("12345")));
