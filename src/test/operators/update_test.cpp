...
void OperatorsUpdateTest::helper(std::shared_ptr<GetTable> table_to_update, std::shared_ptr<GetTable> update_values,
  ...
  auto projection1 =
      std::make_shared<Projection>(ref_table, Projection::ColumnExpressions({OperatorExpression::create_column(ColumnID{0})}));
  auto projection2 =
      std::make_shared<Projection>(ref_table, Projection::ColumnExpressions({OperatorExpression::create_column(ColumnID{1})}));
