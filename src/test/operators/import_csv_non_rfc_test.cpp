...
TEST_F(OperatorsImportCsvNonRfcTest, SingleFloatColumn) {
  auto importer = std::make_shared<ImportCsv>("src/test/csv/float.csv", nullopt, false);
  importer->execute();
...
TEST_F(OperatorsImportCsvNonRfcTest, FloatIntTable) {
  auto importer = std::make_shared<ImportCsv>("src/test/csv/float_int.csv", nullopt, false);
  importer->execute();
...
TEST_F(OperatorsImportCsvNonRfcTest, StringNoQuotes) {
  auto importer = std::make_shared<ImportCsv>("src/test/csv/string.csv", nullopt, false);
  importer->execute();
...
TEST_F(OperatorsImportCsvNonRfcTest, StringQuotes) {
  auto importer = std::make_shared<ImportCsv>("src/test/csv/string_quotes.csv", nullopt, false);
  importer->execute();
  std::shared_ptr<Table> expected_table = load_table("src/test/tables/string.tbl", 5);
  EXPECT_TABLE_EQ(importer->get_output(), expected_table, true);
}
...
TEST_F(OperatorsImportCsvNonRfcTest, StringEscaping) {
  auto importer = std::make_shared<ImportCsv>("src/test/csv/string_escaped_unsafe.csv", nullopt, false);
  importer->execute();
...
TEST_F(OperatorsImportCsvNonRfcTest, TrailingNewline) {
  auto importer = std::make_shared<ImportCsv>("src/test/csv/float_int_trailing_newline.csv", nullopt, false);
  importer->execute();
  std::shared_ptr<Table> expected_table = load_table("src/test/tables/float_int.tbl", 2);
  EXPECT_TABLE_EQ(importer->get_output(), expected_table, true);
}
...
TEST_F(OperatorsImportCsvNonRfcTest, FileDoesNotExist) {
  auto importer = std::make_shared<ImportCsv>("src/test/csv/not_existing_file.csv", nullopt, false);
...
TEST_F(OperatorsImportCsvNonRfcTest, SemicolonSeparator) {
  CsvConfig config;
  config.separator = ';';
  auto importer = std::make_shared<ImportCsv>("src/test/csv/ints_semicolon_separator.csv", config, nullopt, false);
  importer->execute();
