...
TEST_F(OperatorsInsertTest, SelfInsert) {
  ...
  EXPECT_EQ((*t->get_chunk(ChunkID{0})->get_segment(CxlumnID{1}))[0], AllTypeVariant(12345));
  EXPECT_EQ((*t->get_chunk(ChunkID{0})->get_segment(CxlumnID{0}))[0], AllTypeVariant(458.7f));
  EXPECT_EQ((*t->get_chunk(ChunkID{0})->get_segment(CxlumnID{1}))[3], AllTypeVariant(12345));
  EXPECT_EQ((*t->get_chunk(ChunkID{0})->get_segment(CxlumnID{0}))[3], AllTypeVariant(458.7f));

  EXPECT_EQ(t->get_chunk(ChunkID{0})->get_segment(CxlumnID{0})->size(), 6u);
  EXPECT_EQ(t->get_chunk(ChunkID{0})->get_segment(CxlumnID{1})->size(), 6u);
}
...
TEST_F(OperatorsInsertTest, InsertStringNullValue) {
  ...
  auto null_val = (*(t->get_chunk(ChunkID{1})->get_segment(CxlumnID{0})))[2];
...
TEST_F(OperatorsInsertTest, InsertIntFloatNullValues) {
  ...
  auto null_val_int = (*(t->get_chunk(ChunkID{2})->get_segment(CxlumnID{0})))[0];
  EXPECT_TRUE(variant_is_null(null_val_int));

  auto null_val_float = (*(t->get_chunk(ChunkID{1})->get_segment(CxlumnID{1})))[2];
...
TEST_F(OperatorsInsertTest, InsertSingleNullFromDummyProjection) {
  ...
  auto null_val = (*(t->get_chunk(ChunkID{1})->get_segment(CxlumnID{0})))[0];
  EXPECT_TRUE(variant_is_null(null_val));
}
