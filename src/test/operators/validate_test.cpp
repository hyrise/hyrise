...
class OperatorsValidateTest : public BaseTest {
 protected:
  void SetUp() override {
    _test_table = load_table("artifacts/test_data/tbl/validate_input.tbl", 2u);
...
TEST_F(OperatorsValidateTest, SimpleValidate) {
  ...
  std::shared_ptr<Table> expected_result = load_table("artifacts/test_data/tbl/validate_output_validated.tbl", 2u);
...
TEST_F(OperatorsValidateTest, ScanValidate) {
  ...
  std::shared_ptr<Table> expected_result =
      load_table("artifacts/test_data/tbl/validate_output_validated_scanned.tbl", 2u);
