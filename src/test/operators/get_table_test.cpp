...
class OperatorsGetTableTest : public BaseTest {
 protected:
  void SetUp() override {
    ...
    manager.add_table("tableWithValues", load_table("artifacts/test_data/tbl/int_float2.tbl", 1u));
  }
...
TEST_F(OperatorsGetTableTest, GetOutput) {
  ...
  EXPECT_TABLE_EQ_UNORDERED(gt->get_output(), load_table("artifacts/test_data/tbl/int_float2.tbl", 1u));
}
