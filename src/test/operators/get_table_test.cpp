...
TEST_F(OperatorsGetTableTest, PrunedChunks) {
  ...
  EXPECT_EQ(table->get_chunk(ChunkID{0})->get_indices({ColumnID{0}}).size(), 1u);
  EXPECT_EQ(table->get_chunk(ChunkID{0})->get_indices({ColumnID{1}}).size(), 2u);
  EXPECT_EQ(table->get_chunk(ChunkID{1})->get_indices({ColumnID{0}}).size(), 1u);
  EXPECT_EQ(table->get_chunk(ChunkID{1})->get_indices({ColumnID{1}}).size(), 2u);
}
...
TEST_F(OperatorsGetTableTest, PrunedColumns) {
  ...
  EXPECT_EQ(table->get_chunk(ChunkID{2})->get_indices({ColumnID{0}}).size(), 1u);
  EXPECT_EQ(table->get_chunk(ChunkID{2})->get_indices({ColumnID{1}}).size(), 0u);
  EXPECT_EQ(table->get_chunk(ChunkID{3})->get_indices({ColumnID{0}}).size(), 1u);
  EXPECT_EQ(table->get_chunk(ChunkID{3})->get_indices({ColumnID{1}}).size(), 0u);
}
