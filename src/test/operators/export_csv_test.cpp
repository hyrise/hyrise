...
TEST_F(OperatorsExportCsvTest, SingleChunkAndMetaInfo) {
  ...
  auto ex = std::make_shared<opossum::ExportCsv>(table_wrapper, test_filename);
  ex->execute();
...
TEST_F(OperatorsExportCsvTest, ReferenceSegment) {
  ...
  scan->execute();
  auto ex = std::make_shared<opossum::ExportCsv>(scan, test_filename);
  ex->execute();
...
TEST_F(OperatorsExportCsvTest, ExportNumericNullValues) {
  auto table_wrapper = std::make_shared<TableWrapper>(load_table("artifacts/test_data/tbl/int_float_with_null.tbl", 4));
  table_wrapper->execute();
...
TEST_F(OperatorsExportCsvTest, ExportStringNullValues) {
  auto table_wrapper = std::make_shared<TableWrapper>(load_table("artifacts/test_data/tbl/string_with_null.tbl", 4));
  table_wrapper->execute();
