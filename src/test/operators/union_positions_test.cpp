...
class UnionPositionsTest : public BaseTest {
 public:
  void SetUp() override {
    _table_10_ints = load_table("artifacts/test_data/tbl/10_ints.tbl", 3);
    StorageManager::get().add_table("10_ints", _table_10_ints);

    _table_int_float4 = load_table("artifacts/test_data/tbl/int_float4.tbl", 3);
    StorageManager::get().add_table("int_float4", _table_int_float4);
    StorageManager::get().add_table("int_int", load_table("artifacts/test_data/tbl/int_int.tbl", 2));
...
TEST_F(UnionPositionsTest, SelfUnionExlusiveRanges) {
  ...
  EXPECT_TABLE_EQ_UNORDERED(union_unique_op->get_output(),
                            load_table("artifacts/test_data/tbl/10_ints_exclusive_ranges.tbl"));
}
...
TEST_F(UnionPositionsTest, EarlyResultLeft) {
  ...
  EXPECT_TABLE_EQ_UNORDERED(union_unique_op->get_output(), load_table("artifacts/test_data/tbl/int_float2.tbl"));
...
TEST_F(UnionPositionsTest, SelfUnionOverlappingRangesMultipleSegments) {
  ...
  EXPECT_TABLE_EQ_UNORDERED(union_unique_op->get_output(),
                            load_table("artifacts/test_data/tbl/int_float4_overlapping_ranges.tbl"));
}
...
TEST_F(UnionPositionsTest, MultipleReferencedTables) {
  ...
  EXPECT_TABLE_EQ_UNORDERED(union_unique_op->get_output(),
                            load_table("artifacts/test_data/tbl/int_float4_int_int_union_positions.tbl"));
...
TEST_F(UnionPositionsTest, MultipleShuffledPosList) {
  ...
  EXPECT_TABLE_EQ_UNORDERED(set_union_op->get_output(),
                            load_table("artifacts/test_data/tbl/union_positions_multiple_shuffled_pos_list.tbl"));
}
