 ...
 public:
  ...
  static std::shared_ptr<Table> get_table(const InputTableConfiguration& key) {
    ...
    if (input_table_iter == input_tables.end()) {
       ...
       * indexes for the table have to be created. The index type is either GroupKeyIndex for dictionary segments or 
...
// TODO(anyone) #1852 Currently the JoinIndex operator has some bugs with unknown faults.
// These bugs lead to failures of tests generated by the following lines.
