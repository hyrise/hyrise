...
class JoinHashStepsTest : public BaseTest {
 protected:
  static void SetUpTestCase() {
    ...
    _table_int_with_nulls = std::make_shared<TableWrapper>(load_table("src/test/tables/int_float_with_null.tbl", 10));
    _table_int_with_nulls->execute();

    _table_with_nulls_and_zeros =
        std::make_shared<TableWrapper>(load_table("src/test/tables/int_int4_with_null.tbl", 10));
...
TEST_F(JoinHashStepsTest, MaterializeInput) {
  ...
  auto radix_container =
      materialize_input<int, int>(_table_with_nulls_and_zeros_scanned->get_output(), ColumnID{0}, histograms, 0);
...
TEST_F(JoinHashStepsTest, MaterializeAndBuildWithKeepNulls) {
  ...
  auto materialized_without_nulls =
      materialize_input<int, int>(_table_with_nulls_and_zeros->get_output(), ColumnID{0}, histograms, radix_bit_count);
