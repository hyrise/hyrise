...
TYPED_TEST(JoinEquiTest, InnerRefJoin) {
  ...
  auto scan_a = this->create_table_scan(
      this->_table_wrapper_a, ColumnID{0}, PredicateCondition::GreaterThanEquals, 0);
  scan_a->execute();
  auto scan_b = this->create_table_scan(
      this->_table_wrapper_b, ColumnID{0}, PredicateCondition::GreaterThanEquals, 0);
  scan_b->execute();
...
TYPED_TEST(JoinEquiTest, InnerValueDictRefJoin) {
  ...
  scan_a->execute();
  auto scan_b = this->create_table_scan(
      this->_table_wrapper_b_dict, ColumnID{0}, PredicateCondition::GreaterThanEquals, 0);
  scan_b->execute();
...
TYPED_TEST(JoinEquiTest, InnerDictValueRefJoin) {
  // scan that returns all rows
  auto scan_a = this->create_table_scan(
      this->_table_wrapper_a_dict, ColumnID{0}, PredicateCondition::GreaterThanEquals, 0);
  scan_a->execute();
  auto scan_b = this->create_table_scan(
      this->_table_wrapper_b, ColumnID{0}, PredicateCondition::GreaterThanEquals, 0);
  scan_b->execute();
...
TYPED_TEST(JoinEquiTest, InnerRefJoinFiltered) {
  auto scan_a = this->create_table_scan(this->_table_wrapper_a,
                                            ColumnID{0}, PredicateCondition::GreaterThan, 1000);
  scan_a->execute();
  auto scan_b = this->create_table_scan(
      this->_table_wrapper_b, ColumnID{0}, PredicateCondition::GreaterThanEquals, 0);
  scan_b->execute();
...
TYPED_TEST(JoinEquiTest, InnerRefDictJoinFiltered) {
  auto scan_a = this->create_table_scan(this->_table_wrapper_a_dict,
                                            ColumnID{0}, PredicateCondition::GreaterThan, 1000);
  scan_a->execute();
  auto scan_b = this->create_table_scan(
      this->_table_wrapper_b_dict, ColumnID{0}, PredicateCondition::GreaterThanEquals, 0);
...
TYPED_TEST(JoinEquiTest, InnerRefJoinFilteredBig) {
  auto scan_c = this->create_table_scan(
      this->_table_wrapper_c, ColumnID{0}, PredicateCondition::GreaterThanEquals, 0);
  scan_c->execute();
  auto scan_d = this->create_table_scan(
      this->_table_wrapper_d, ColumnID{1}, PredicateCondition::GreaterThanEquals, 6);
  scan_d->execute();
...
TYPED_TEST(JoinEquiTest, MultiJoinOnReferenceLeft) {
  // scan that returns all rows
  auto scan_a = this->create_table_scan(
      this->_table_wrapper_f, ColumnID{0}, PredicateCondition::GreaterThanEquals, 0);
  scan_a->execute();
  auto scan_b = this->create_table_scan(
      this->_table_wrapper_g, ColumnID{0}, PredicateCondition::GreaterThanEquals, 0);
  scan_b->execute();
  auto scan_c = this->create_table_scan(
      this->_table_wrapper_h, ColumnID{0}, PredicateCondition::GreaterThanEquals, 0);
  scan_c->execute();
...
TYPED_TEST(JoinEquiTest, MultiJoinOnReferenceLeftFiltered) {
  // scan that returns all rows
  auto scan_a = this->create_table_scan(this->_table_wrapper_f,
                                            ColumnID{0}, PredicateCondition::GreaterThan, 6);
  scan_a->execute();
  auto scan_b = this->create_table_scan(
      this->_table_wrapper_g, ColumnID{0}, PredicateCondition::GreaterThanEquals, 0);
  scan_b->execute();
...
TYPED_TEST(JoinEquiTest, RightJoinEmptyRefSegment) {
  ...
  auto scan_a = this->create_table_scan(this->_table_wrapper_a,
                                            ColumnID{0}, PredicateCondition::Equals, 0);
  scan_a->execute();
...
TYPED_TEST(JoinEquiTest, LeftJoinEmptyRefSegment) {
  // scan that returns no rows
  auto scan_b = this->create_table_scan(this->_table_wrapper_b,
                                            ColumnID{0}, PredicateCondition::Equals, 0);
  scan_b->execute();
...
TYPED_TEST(JoinEquiTest, DISABLED_JoinOnUnion /* #160 */) {
  ...
  auto filtered_left = this->create_table_scan(
      this->_table_wrapper_e,
      ColumnID{0}, PredicateCondition::LessThanEquals, 10);
  filtered_left->execute();
  auto filtered_left2 = this->create_table_scan(
      this->_table_wrapper_f,
      ColumnID{0}, PredicateCondition::LessThanEquals, 10);
  filtered_left2->execute();
  auto filtered_right = this->create_table_scan(
      this->_table_wrapper_g,
      ColumnID{0}, PredicateCondition::LessThanEquals, 10);
  filtered_right->execute();
  auto filtered_right2 = this->create_table_scan(
