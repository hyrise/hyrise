...
/**
 * Utility function to build a group as a binary blob and append it to a run
 */
...
void add_group(Run& run, const Args&&... args) {
  ...
  const auto fixed_size_data_byte_count = ((is_variably_sized_v<Args> ? 0 : get_value_size(args)) + ...);
  ...
  const auto write_value = [&](char* destination, const auto& value) {
    using VALUE_TYPE = std::decay_t<decltype(value)>;
  ...
  auto* group_begin_ptr = reinterpret_cast<char*>(data.data()) + old_size * sizeof(GroupRunElementType);
  auto* fixed_size_ptr = group_begin_ptr;
  auto* variably_sized_ptr = group_begin_ptr + size_t{fixed_size_data_byte_count};
  auto* value_end_offsets_ptr =
      group_begin_ptr + group_size * sizeof(GroupRunElementType) - group_meta_data_byte_count;
  auto last_value_end_offset = fixed_size_data_byte_count;
  ...
  const auto for_each_value = [&](const auto& value) {
    ...
    if constexpr (is_variably_sized_v<VALUE_TYPE>) {
      if constexpr (is_optional_v<VALUE_TYPE>) {
        // Write is_null
        *variably_sized_ptr = value ? 0 : 1;
        ++variably_sized_ptr;

        variably_sized_ptr = write_value(variably_sized_ptr, *value);
      } else {
        variably_sized_ptr = write_value(variably_sized_ptr, value);
      }

      value_end_offsets_ptr = write_value(value_end_offsets_ptr, last_value_end_offset + get_value_size(value));
      last_value_end_offset += get_value_size(value);
    } else {
      if constexpr (is_optional_v<VALUE_TYPE>) {
        // Write is_null
        *fixed_size_ptr = value ? 0 : 1;
        ++fixed_size_ptr;

        fixed_size_ptr = write_value(fixed_size_ptr, *value);
      } else {
        fixed_size_ptr = write_value(fixed_size_ptr, value);
      }
    }
...
//// Test the testutils themselves
//TEST_F(AggregateHashSortTest, TestUtils) {
//  const auto group_by_column_ids = std::vector<ColumnID>{ColumnID{0}};
//  const auto environment = AggregateHashSortEnvironment::create({}, table, {}, group_by_column_ids, {});
//
//  auto run_a = BasicRun<VariableGroupSizePolicy>{{*environment, 0}, {}};
//  add_group(run_a, int32_t{5}, std::optional{"abc"s}, std::optional<int32_t>{13}, "xy"s);
//  EXPECT_EQ(run_a.group_data, uninitialized_vector<uint32_t>{uint32_t{5}});
//  std::cout << "abcc" << std::endl;
//
//
//}

TEST_F(AggregateHashSortTest, Environment) {
  ...
  const auto environment = AggregateHashSortEnvironment::create({}, table, aggregate_column_definitions, group_by_column_ids, {});

  EXPECT_EQ(environment->offsets, std::vector<size_t>({0, 0, 1, 5}));
  EXPECT_EQ(environment->variably_sized_column_ids, std::vector({ColumnID{1}, ColumnID{2}}));
  EXPECT_EQ(environment->fixed_size_column_ids, std::vector({ColumnID{0}, ColumnID{4}}));
  EXPECT_EQ(environment->fixed_size_value_offsets, std::vector<size_t>({0, 5}));
  EXPECT_EQ(environment->variably_sized_values_begin_offset, 13);
  EXPECT_EQ(environment->fixed_group_size, divide_and_ceil(size_t{13}, GROUP_RUN_ELEMENT_SIZE));
  EXPECT_EQ(environment->aggregate_definitions.size(), 2);
  EXPECT_EQ(environment->aggregate_definitions.at(0), AggregateHashSortAggregateDefinition(AggregateFunction::CountRows));
  EXPECT_EQ(environment->aggregate_definitions.at(1), AggregateHashSortAggregateDefinition(AggregateFunction::CountDistinct, DataType::String, ColumnID{1}));
}
...
TEST_F(AggregateHashSortTest, CreateRun) {
  ...
  const auto variable_group_size_run = ChunkRunSource<BasicRun<VariableGroupSizePolicy>>::from_table_range(*environment, table, RowID{ChunkID{0}, ChunkOffset{0}}, size_t{3});

  auto expected_policy = VariableGroupSizePolicy{*environment, 0};
  BasicRun<VariableGroupSizePolicy> expected_run{expected_policy, {}};
  add_group(expected_run, std::optional{"x"s},  std::optional<int32_t>{11}, "abcd"s, int32_t{0});
  add_group(expected_run, std::optional{"y"s},  std::optional<int32_t>{13}, "abcd"s, int32_t{1});
  add_group(expected_run, std::optional{"xy"s}, std::optional<int32_t>{},   "iiii"s, int32_t{2});

  EXPECT_EQ(variable_group_size_run.group_data.size(), 24u);
  EXPECT_EQ(variable_group_size_run.group_data, expected_run.group_data);
  EXPECT_EQ(variable_group_size_run.group_end_offsets, expected_run.group_end_offsets);
...
//  auto environment = AggregateHashSortEnvironment{};
//  environment.fixed_group_size = 6u;
//  environment.offsets = {0, 9, 17};
//
//  auto run = create_run<BasicRun<DynamicFixedGroupSizePolicy>>(environment);
//
...
//
//  auto compare = DynamicFixedGroupSizePolicy::HashTableCompare{environment};
//
...
//
//  const auto actual_segment_a = run.materialize_group_column<int64_t>(environment, ColumnID{0}, true);
...
//
//  const auto actual_segment_b = run.materialize_group_column<int64_t>(environment, ColumnID{1}, false);
...
//
//  const auto actual_segment_c = run.materialize_group_column<int32_t>(environment, ColumnID{2}, true);
...
//  const auto environment = AggregateHashSortEnvironment::create({}, table, {}, group_by_column_ids);
//
//  auto run = create_run<BasicRun<VariableGroupSizePolicy>>(environment);
//
//  // clang-format off
...
//
//  EXPECT_EQ(run.get_variably_sized_value_range(environment, 0, 0), std::make_pair(size_t(13), size_t(3)));
//  EXPECT_EQ(run.get_variably_sized_value_range(environment, 0, 1), std::make_pair(size_t(16), size_t(5)));
//  EXPECT_EQ(run.get_variably_sized_value_range(environment, 1, 0), std::make_pair(size_t(53), size_t(3)));
//  EXPECT_EQ(run.get_variably_sized_value_range(environment, 1, 1), std::make_pair(size_t(56), size_t(1)));
//  EXPECT_EQ(run.get_variably_sized_value_range(environment, 2, 0), std::make_pair(size_t(89), size_t(0)));
//  EXPECT_EQ(run.get_variably_sized_value_range(environment, 2, 1), std::make_pair(size_t(89), size_t(1)));
//  EXPECT_EQ(run.get_variably_sized_value_range(environment, 3, 0), std::make_pair(size_t(121), size_t(1)));
//  EXPECT_EQ(run.get_variably_sized_value_range(environment, 3, 1), std::make_pair(size_t(122), size_t(5)));
//  EXPECT_EQ(run.get_variably_sized_value_range(environment, 4, 0), std::make_pair(size_t(157), size_t(3)));
//  EXPECT_EQ(run.get_variably_sized_value_range(environment, 4, 1), std::make_pair(size_t(160), size_t(5)));
//
//  auto compare = VariablySizedCompare{environment};
//
//  EXPECT_TRUE(compare(run.make_key(0), run.make_key(4)));
...
//
//  const auto actual_segment_a = run.materialize_group_column<int32_t>(environment, ColumnID{0}, true);
...
//  EXPECT_SEGMENT_EQ_ORDERED(actual_segment_a, expected_segment_a);
//
//  const auto actual_segment_b = run.materialize_group_column<pmr_string>(environment, ColumnID{1}, false);
//  const auto expected_segment_b = std::make_shared<ValueSegment<pmr_string>>(std::vector<pmr_string>{"abc", "hij", "", "z", "abc", "abc"});
//  EXPECT_SEGMENT_EQ_ORDERED(actual_segment_b, expected_segment_b);
//
//  const auto actual_segment_c = run.materialize_group_column<double>(environment, ColumnID{2}, false);
//  const auto expected_segment_c = std::make_shared<ValueSegment<double>>(std::vector<double>{3.0, 14.0, 25.0, 36.0, 3.0, 3.0});
//  EXPECT_SEGMENT_EQ_ORDERED(actual_segment_c, expected_segment_c);
//
//  const auto actual_segment_d = run.materialize_group_column<pmr_string>(environment, ColumnID{3}, true);
...
//}
//
//TEST_F(AggregateHashSortTest, ChunkRunSourceFixedOnly) {
...
//
//  auto environment = AggregateHashSortEnvironment::create({}, table, {}, group_by_column_ids, {});
//
//  EXPECT_EQ(environment->fixed_group_size, 5u);
//  EXPECT_EQ(environment->offsets, std::vector<size_t>({0, 4, 9}));
//
//  auto run_source = ChunkRunSource<BasicRun<DynamicFixedGroupSizePolicy>>{environment, table, RowID{ChunkID{0}, ChunkOffset{0}}, 3u};
//  run_source.fetch_run(environment);
//
...
//
//  // clang-format off
//  auto expected_group_run_a = create_run<BasicRun<DynamicFixedGroupSizePolicy>>(environment);
...
//
//  // clang-format off
//  auto expected_group_run_b = create_run<BasicRun<DynamicFixedGroupSizePolicy>>(environment);
...
//  const auto [data_per_column, value_end_offsets, end_row_id] = TableRunSource<BasicRun<VariableGroupSizePolicy>>::materialize_variably_sized_columns(environment, table,
...
//
//  // clang-format off
//  auto expected_group_run_a = create_run<BasicRun<VariableGroupSizePolicy>>(environment);
...
//
//  // clang-format off
//  auto expected_group_run_b = create_run<BasicRun<VariableGroupSizePolicy>>(environment);
...
//  const auto [run, end_row_id] = TableRunSource<BasicRun<VariableGroupSizePolicy>>::from_table_range(environment, table, begin_row_id, row_count);
//
//  EXPECT_EQ(end_row_id, RowID(ChunkID{2}, ChunkOffset{1}));
//
//  // clang-format off
//  auto expected_group_run = create_run<BasicRun<VariableGroupSizePolicy>>(environment);
...
//  };
//
//  const auto environment = AggregateHashSortEnvironment::create({}, table, aggregate_column_definitions, {ColumnID{0}}, {});
//
//  const auto [run, end_row_id] = TableRunSource<BasicRun<DynamicFixedGroupSizePolicy>>::from_table_range(environment, table, RowID{ChunkID{0}, ChunkOffset{1}}, 3);
//
...
//  const auto environment = AggregateHashSortEnvironment::create({}, table, aggregate_column_definitions, group_by_column_ids, {});
//
//  auto run = create_run<Run>(environment);
//
//  // clang-format off
...
//  partition<Run>(environment, 6, run_source, fan_out, partitions, 0);
...
//  partition<Run>(environment, 4, run_source, fan_out, partitions, 0);
//  EXPECT_EQ(run_source->run_idx, 0);
...
//
//  auto run_partition_a = create_run<Run>(environment);
//  auto run_partition_b = create_run<Run>(environment);
//
//  // clang-format off
...
//  const auto aggregate_column_definitions = std::vector<AggregateColumnDefinition>{{ColumnID{1}, AggregateFunction::Sum}};
//
//  auto environment = AggregateHashSortEnvironment::create({}, table, aggregate_column_definitions, group_by_column_ids, {});
//  environment.config.hash_table_max_load_factor = 1.0f;
//  // Provoke `continue_hashing` to be true
//  environment.config.continue_hashing_density_threshold = 1.3f;
//
//  auto run = create_run<Run>(environment);
//
//  // clang-format off
...
//      hashing<Run>(environment, 3, run_source, fan_out, partitions, 0);
//
//  EXPECT_EQ(run_source->run_idx, 0);
//  EXPECT_EQ(run_source->run_offset, 4);
...
//  environment.config.continue_hashing_density_threshold = 1.5f;
//
//  auto run = create_run<Run>(environment);
//
//  // clang-format off
...
//  ASSERT_EQ(partitions.at(1).runs.size(), 1u);
//
//  auto group_run_partition_a = create_run<Run>(environment);
//  auto group_run_partition_b = create_run<Run>(environment);
//
//  // clang-format off
...
//
//  const auto environment = AggregateHashSortEnvironment::create(config, table, {}, group_by_column_ids, {});
//
//  auto run = create_run<Run>(environment);
//
//  // clang-format off
...
//  const auto run_source = std::make_shared<PartitionRunSource<Run>>(std::move(runs));
//  const auto abstract_run_source = std::static_pointer_cast<AbstractRunSource<Run>>(run_source);
//  const auto partitions = adaptive_hashing_and_partition<Run>(environment, abstract_run_source, fan_out, 0);
//
//  ASSERT_EQ(partitions.size(), 2u);
//
//  ASSERT_EQ(partitions.at(0).size(), 2u);
