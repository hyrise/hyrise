...
class JoinTest : public BaseTest {
 ...
 protected:
  ...
  void test_hash_join_output(const std::shared_ptr<const AbstractOperator>& left,
                             const std::shared_ptr<const AbstractOperator>& right, const ColumnIDPair& column_ids,
    ...
    // build and execute join
    auto join = std::make_shared<JoinHash>(left, right, mode, column_ids, predicate_condition, std::nullopt,
