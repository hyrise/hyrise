...
class JoinTest : public BaseTest {
 public:
  static void SetUpTestCase() {  // called ONCE before tests are run
    ...
    _table_wrapper_a = std::make_shared<TableWrapper>(load_table("artifacts/test_data/tbl/int_float.tbl", 2));
    _table_wrapper_b = std::make_shared<TableWrapper>(load_table("artifacts/test_data/tbl/int_float2.tbl", 2));
    _table_wrapper_c = std::make_shared<TableWrapper>(load_table("artifacts/test_data/tbl/int_string.tbl", 4));
    _table_wrapper_d = std::make_shared<TableWrapper>(load_table("artifacts/test_data/tbl/string_int.tbl", 3));
    _table_wrapper_e = std::make_shared<TableWrapper>(load_table("artifacts/test_data/tbl/int_int.tbl", 4));
    _table_wrapper_f = std::make_shared<TableWrapper>(load_table("artifacts/test_data/tbl/int_int2.tbl", 4));
    _table_wrapper_g = std::make_shared<TableWrapper>(load_table("artifacts/test_data/tbl/int_int3.tbl", 4));
    _table_wrapper_h = std::make_shared<TableWrapper>(load_table("artifacts/test_data/tbl/int_int4.tbl", 4));
    _table_wrapper_i = std::make_shared<TableWrapper>(load_table("artifacts/test_data/tbl/int5.tbl", 1));
    _table_wrapper_j = std::make_shared<TableWrapper>(load_table("artifacts/test_data/tbl/int3.tbl", 1));
    _table_wrapper_k = std::make_shared<TableWrapper>(load_table("artifacts/test_data/tbl/int4.tbl", 1));
    _table_wrapper_l = std::make_shared<TableWrapper>(load_table("artifacts/test_data/tbl/int.tbl", 1));
    _table_wrapper_m = std::make_shared<TableWrapper>(
        load_table("artifacts/test_data/tbl/aggregateoperator/groupby_int_1gb_0agg/input_null.tbl", 20));
    _table_wrapper_n = std::make_shared<TableWrapper>(
        load_table("artifacts/test_data/tbl/aggregateoperator/groupby_int_1gb_1agg/input_null.tbl", 20));
    _table_wrapper_o =
        std::make_shared<TableWrapper>(load_table("artifacts/test_data/tbl/float_zero_precision.tbl", 1));
    _table_wrapper_p =
        std::make_shared<TableWrapper>(load_table("artifacts/test_data/tbl/double_zero_precision.tbl", 1));
    _table_wrapper_q = std::make_shared<TableWrapper>(load_table("artifacts/test_data/tbl/string_numbers.tbl", 1));

    // load and create DictionarySegment tables
    auto table = load_table("artifacts/test_data/tbl/int_float.tbl", 2);
    ChunkEncoder::encode_chunks(table, {ChunkID{0}, ChunkID{1}});
    _table_wrapper_a_dict = std::make_shared<TableWrapper>(std::move(table));

    table = load_table("artifacts/test_data/tbl/int_float2.tbl", 2);
    ChunkEncoder::encode_chunks(table, {ChunkID{0}, ChunkID{1}});
    _table_wrapper_b_dict = std::make_shared<TableWrapper>(std::move(table));

    table = load_table("artifacts/test_data/tbl/int_float.tbl", 2);
    ChunkEncoder::encode_chunks(table, {ChunkID{0}});
    _table_wrapper_c_dict = std::make_shared<TableWrapper>(std::move(table));

    table = load_table("artifacts/test_data/tbl/aggregateoperator/groupby_int_1gb_0agg/input_null.tbl", 20);
    ChunkEncoder::encode_chunks(table, {ChunkID{0}});
    _table_wrapper_m_dict = std::make_shared<TableWrapper>(std::move(table));

    table = load_table("artifacts/test_data/tbl/aggregateoperator/groupby_int_1gb_1agg/input_null.tbl", 20);
    ChunkEncoder::encode_chunks(table, {ChunkID{0}});
