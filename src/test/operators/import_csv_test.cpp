...
TEST_F(OperatorsImportCsvTest, Parallel) {
  ...
  auto expected_table = std::make_shared<Table>(column_definitions, TableType::Data, UseMvcc::No, 20);
...
TEST_F(OperatorsImportCsvTest, ImportNumericNullValues) {
  ...
  TableColumnDefinitions column_definitions{{"a", DataType::Float, true}, {"b", DataType::Int, false}, {"c", DataType::Int, false}};
