...
TEST_F(OperatorsImportCsvTest, SingleFloatColumn) {
  auto importer = std::make_shared<ImportCsv>("artifacts/test_data/csv/float.csv");
  importer->execute();
  std::shared_ptr<Table> expected_table = load_table("artifacts/test_data/tbl/float.tbl", 5);
  EXPECT_TABLE_EQ_ORDERED(importer->get_output(), expected_table);
}

TEST_F(OperatorsImportCsvTest, FloatIntTable) {
  auto importer = std::make_shared<ImportCsv>("artifacts/test_data/csv/float_int.csv");
  importer->execute();
  std::shared_ptr<Table> expected_table = load_table("artifacts/test_data/tbl/float_int.tbl", 2);
  EXPECT_TABLE_EQ_ORDERED(importer->get_output(), expected_table);
}

TEST_F(OperatorsImportCsvTest, StringNoQuotes) {
  auto importer = std::make_shared<ImportCsv>("artifacts/test_data/csv/string.csv");
  importer->execute();
  std::shared_ptr<Table> expected_table = load_table("artifacts/test_data/tbl/string.tbl", 5);
  EXPECT_TABLE_EQ_ORDERED(importer->get_output(), expected_table);
}

TEST_F(OperatorsImportCsvTest, StringQuotes) {
  auto importer = std::make_shared<ImportCsv>("artifacts/test_data/csv/string_quotes.csv");
  importer->execute();
  std::shared_ptr<Table> expected_table = load_table("artifacts/test_data/tbl/string.tbl", 5);
  EXPECT_TABLE_EQ_ORDERED(importer->get_output(), expected_table);
}
...
TEST_F(OperatorsImportCsvTest, StringEscaping) {
  auto importer = std::make_shared<ImportCsv>("artifacts/test_data/csv/string_escaped.csv");
  importer->execute();
...
TEST_F(OperatorsImportCsvTest, EmptyFile) {
  auto importer = std::make_shared<ImportCsv>("artifacts/test_data/csv/float_int_empty.csv");
  importer->execute();
  std::shared_ptr<Table> expected_table = load_table("artifacts/test_data/tbl/float_int_empty.tbl", 2);
  EXPECT_TABLE_EQ_ORDERED(importer->get_output(), expected_table);
}

TEST_F(OperatorsImportCsvTest, TrailingNewline) {
  auto importer = std::make_shared<ImportCsv>("artifacts/test_data/csv/float_int_trailing_newline.csv");
  importer->execute();
  std::shared_ptr<Table> expected_table = load_table("artifacts/test_data/tbl/float_int.tbl", 2);
  EXPECT_TABLE_EQ_ORDERED(importer->get_output(), expected_table);
}
...
TEST_F(OperatorsImportCsvTest, FileDoesNotExist) {
  auto importer = std::make_shared<ImportCsv>("artifacts/test_data/csv/not_existing_file.csv");
  EXPECT_THROW(importer->execute(), std::exception);
}

TEST_F(OperatorsImportCsvTest, SaveToStorageManager) {
  auto importer =
      std::make_shared<ImportCsv>("artifacts/test_data/csv/float.csv", Chunk::DEFAULT_SIZE, std::string("float_table"));
  importer->execute();
  std::shared_ptr<Table> expected_table = load_table("artifacts/test_data/tbl/float.tbl", 5);
  EXPECT_TABLE_EQ_ORDERED(importer->get_output(), expected_table);
...
TEST_F(OperatorsImportCsvTest, EmptyStrings) {
  auto importer = std::make_shared<ImportCsv>("artifacts/test_data/csv/empty_strings.csv");
  importer->execute();
...
TEST_F(OperatorsImportCsvTest, Parallel) {
  ...
  auto importer = std::make_shared<OperatorTask>(
      std::make_shared<ImportCsv>("artifacts/test_data/csv/float_int_large.csv"), CleanupTemporaries::Yes);
  importer->schedule();
...
TEST_F(OperatorsImportCsvTest, SemicolonSeparator) {
  std::string csv_file = "artifacts/test_data/csv/ints_semicolon_separator.csv";
...
TEST_F(OperatorsImportCsvTest, ChunkSize) {
  auto importer = std::make_shared<ImportCsv>("artifacts/test_data/csv/float_int_large.csv", ChunkOffset{20});
  importer->execute();
...
TEST_F(OperatorsImportCsvTest, MaxChunkSize) {
  auto importer =
      std::make_shared<ImportCsv>("artifacts/test_data/csv/float_int_large_chunksize_max.csv", Chunk::DEFAULT_SIZE);
  importer->execute();
...
TEST_F(OperatorsImportCsvTest, StringEscapingNonRfc) {
  std::string csv_file = "artifacts/test_data/csv/string_escaped_unsafe.csv";
  auto csv_meta = process_csv_meta_file(csv_file + CsvMeta::META_FILE_EXTENSION);
...
TEST_F(OperatorsImportCsvTest, ImportNumericNullValues) {
  auto importer = std::make_shared<ImportCsv>("artifacts/test_data/csv/float_int_with_null.csv");
  importer->execute();
...
TEST_F(OperatorsImportCsvTest, ImportStringNullValues) {
  auto importer = std::make_shared<ImportCsv>("artifacts/test_data/csv/string_with_null.csv");
  importer->execute();
...
TEST_F(OperatorsImportCsvTest, ImportUnquotedNullString) {
  auto importer = std::make_shared<ImportCsv>("artifacts/test_data/csv/null_literal.csv");
  importer->execute();
...
TEST_F(OperatorsImportCsvTest, ImportUnquotedNullStringThrows) {
  auto importer = std::make_shared<ImportCsv>("artifacts/test_data/csv/string_with_bad_null.csv");
  EXPECT_THROW(importer->execute(), std::exception);
}

TEST_F(OperatorsImportCsvTest, WithAndWithoutQuotes) {
  std::string csv_file = "artifacts/test_data/csv/with_and_without_quotes.csv";
  auto csv_meta = process_csv_meta_file(csv_file + CsvMeta::META_FILE_EXTENSION);
...
TEST_F(OperatorsImportCsvTest, StringDoubleEscape) {
  std::string csv_file = "artifacts/test_data/csv/string_double_escape.csv";
  auto csv_meta = process_csv_meta_file(csv_file + CsvMeta::META_FILE_EXTENSION);
...
TEST_F(OperatorsImportCsvTest, ImportQuotedInt) {
  auto importer = std::make_shared<ImportCsv>("artifacts/test_data/csv/quoted_int.csv");
  EXPECT_THROW(importer->execute(), std::exception);
}

TEST_F(OperatorsImportCsvTest, UnconvertedCharactersThrows) {
  auto importer = std::make_shared<ImportCsv>("artifacts/test_data/csv/unconverted_characters_int.csv");
  EXPECT_THROW(importer->execute(), std::logic_error);

  importer = std::make_shared<ImportCsv>("artifacts/test_data/csv/unconverted_characters_float.csv");
  EXPECT_THROW(importer->execute(), std::logic_error);

  importer = std::make_shared<ImportCsv>("artifacts/test_data/csv/unconverted_characters_double.csv");
  EXPECT_THROW(importer->execute(), std::logic_error);
}
}  // namespace opossum
