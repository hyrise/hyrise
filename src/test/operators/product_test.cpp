...
class OperatorsProductTest : public BaseTest {
 public:
  ...
  virtual void SetUp() {
    _table_wrapper_a = std::make_shared<TableWrapper>(load_table("artifacts/test_data/tbl/int.tbl", 5));
    _table_wrapper_b = std::make_shared<TableWrapper>(load_table("artifacts/test_data/tbl/float.tbl", 2));
    _table_wrapper_c = std::make_shared<TableWrapper>(load_table("artifacts/test_data/tbl/int_int.tbl", 2));
...
TEST_F(OperatorsProductTest, ValueSegments) {
  ...
  std::shared_ptr<Table> expected_result = load_table("artifacts/test_data/tbl/int_float_product.tbl", 3);
...
TEST_F(OperatorsProductTest, ReferenceAndValueSegments) {
  ...
  std::shared_ptr<Table> expected_result = load_table("artifacts/test_data/tbl/int_filtered_float_product.tbl", 3);
  EXPECT_TABLE_EQ_UNORDERED(product->get_output(), expected_result);
}
...
TEST_F(OperatorsProductTest, SelfProduct) {
  ...
  const auto expected_result = load_table("artifacts/test_data/tbl/int_int_self_product.tbl");
  EXPECT_TABLE_EQ_UNORDERED(product->get_output(), expected_result);
}
