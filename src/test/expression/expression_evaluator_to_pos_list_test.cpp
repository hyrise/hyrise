...
TEST_F(ExpressionEvaluatorToPosListTest, ExistsCorrelated) {
  ...
  const auto sub_query = pqp_sub_query_(table_scan, DataType::Int, false, std::make_pair(ParameterID{0}, ColumnID{0}));

  EXPECT_TRUE(test_expression(table_b, ChunkID{0}, *exists_(sub_query), {}));
  EXPECT_TRUE(test_expression(table_b, ChunkID{1}, *exists_(sub_query), {1}));

  EXPECT_TRUE(test_expression(table_b, ChunkID{0}, *not_exists_(sub_query), {0, 1, 2, 3}));
  EXPECT_TRUE(test_expression(table_b, ChunkID{1}, *not_exists_(sub_query), {0, 2}));
}
...
TEST_F(ExpressionEvaluatorToPosListTest, ExistsUncorrelated) {
  ...
  const auto sub_query_returning_all = pqp_sub_query_(table_wrapper_all, DataType::Int, false);
  ...
  const auto sub_query_returning_none = pqp_sub_query_(table_wrapper_empty, DataType::Int, false);

  EXPECT_TRUE(test_expression(table_b, ChunkID{0}, *exists_(sub_query_returning_all), {0, 1, 2, 3}));
  EXPECT_TRUE(test_expression(table_b, ChunkID{0}, *exists_(sub_query_returning_none), {}));

  EXPECT_TRUE(test_expression(table_b, ChunkID{0}, *not_exists_(sub_query_returning_all), {}));
  EXPECT_TRUE(test_expression(table_b, ChunkID{0}, *not_exists_(sub_query_returning_none), {0, 1, 2, 3}));
}
