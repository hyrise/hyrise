...
TEST_F(ExpressionEvaluatorToValuesTest, SubstrSeries) {
  ...
  EXPECT_TRUE(
      test_expression<pmr_string>(table_a, *substr_("test", 2, c), {"est", std::nullopt, "est", std::nullopt}));
...
TEST_F(ExpressionEvaluatorToValuesTest, ConcatSeries) {
  ...
  EXPECT_TRUE(test_expression<pmr_string>(table_a, *concat_("yo", s1, s2),
...
TEST_F(ExpressionEvaluatorToValuesTest, ExtractLiterals) {
  ...
  EXPECT_THROW(test_expression<pmr_string>(*extract_(DatetimeComponent::Hour, "1992-09-30"), {"30"}),
               std::logic_error);
...
TEST_F(ExpressionEvaluatorToValuesTest, ExtractSeries) {
  ...
  EXPECT_TRUE(
      test_expression<pmr_string>(table_a, *extract_(DatetimeComponent::Day, dates), {"06", "05", "03", "02"}));
