 ...
 public:
  ...
  bool test_expression(const std::shared_ptr<Table>& table, const AbstractExpression& expression,
                       ...
                       const std::shared_ptr<const ExpressionEvaluator::UncorrelatedSubQueryResults>&
                           uncorrelated_sub_query_results = nullptr) {
    const auto actual_result =
        ExpressionEvaluator{table, ChunkID{0}, uncorrelated_sub_query_results}.evaluate_expression_to_result<R>(
            expression);
...
TEST_F(ExpressionEvaluatorToValuesTest, InArbitraryExpression) {
  ...
  // storing the pre-computed result a of sub_query
...
TEST_F(ExpressionEvaluatorToValuesTest, InSubQueryUncorrelatedWithoutPrecalculated) {
  ...
  const auto sub_query_a = pqp_sub_query_(pqp_a, DataType::Int, false);
  ...
  const auto sub_query_b = pqp_sub_query_(pqp_b, DataType::Int, true);

  // Test it without pre-calculated uncorrelated_sub_query_results
  //  EXPECT_TRUE(test_expression<int32_t>(table_a, *in_(6, sub_query_a), {0}));
  //  EXPECT_TRUE(test_expression<int32_t>(table_a, *in_(a, sub_query_a), {1, 1, 1, 1}));
  //  EXPECT_TRUE(test_expression<int32_t>(table_a, *in_(add_(a, 2), sub_query_a), {1, 1, 0, 0}));
  //  EXPECT_TRUE(test_expression<int32_t>(table_a, *in_(b, sub_query_a), {1, 1, 1, 0}));
  //  EXPECT_TRUE(test_expression<int32_t>(table_a, *in_(34, sub_query_b), {1}));
  //  EXPECT_TRUE(test_expression<int32_t>(table_a, *in_(34.0, sub_query_b), {1}));
  //  EXPECT_TRUE(test_expression<int32_t>(table_a, *in_(34.5, sub_query_b), {std::nullopt}));
  //  EXPECT_TRUE(test_expression<int32_t>(table_a, *in_("hello", sub_query_b), {0}));
  //  EXPECT_TRUE(test_expression<int32_t>(table_a, *in_(c, sub_query_b), {1, std::nullopt, 1, std::nullopt}));
  //  EXPECT_TRUE(test_expression<int32_t>(table_a, *not_in_(6, sub_query_a), {1}));
  EXPECT_TRUE(test_expression<int32_t>(table_a, *not_in_(a, sub_query_a), {0, 0, 0, 0}));
  //  EXPECT_TRUE(test_expression<int32_t>(table_a, *not_in_(add_(a, 2), sub_query_a), {0, 0, 1, 1}));
  //  EXPECT_TRUE(test_expression<int32_t>(table_a, *not_in_(b, sub_query_a), {0, 0, 0, 1}));
  //  EXPECT_TRUE(test_expression<int32_t>(table_a, *not_in_(34, sub_query_b), {0}));
  //  EXPECT_TRUE(test_expression<int32_t>(table_a, *not_in_(34.0, sub_query_b), {0}));
  //  EXPECT_TRUE(test_expression<int32_t>(table_a, *not_in_(34.5, sub_query_b), {std::nullopt}));
  //  EXPECT_TRUE(test_expression<int32_t>(table_a, *not_in_("hello", sub_query_b), {1}));
  //  EXPECT_TRUE(test_expression<int32_t>(table_a, *not_in_(c, sub_query_b), {0, std::nullopt, 0, std::nullopt}));
}
...
TEST_F(ExpressionEvaluatorToValuesTest, InSubQueryUncorrelatedWithPrecalculated) {
  ...
  // Test it with pre-calculated uncorrelated_sub_query_results
  auto uncorrelated_sub_query_results = std::make_shared<ExpressionEvaluator::UncorrelatedSubQueryResults>();
  table_wrapper_a->execute();
  pqp_a->execute();
  table_wrapper_b->execute();
  pqp_b->execute();
  uncorrelated_sub_query_results->emplace(pqp_a, pqp_a->get_output());
  uncorrelated_sub_query_results->emplace(pqp_b, pqp_b->get_output());
...
TEST_F(ExpressionEvaluatorToValuesTest, InSubQueryUncorrelatedWithBrokenPrecalculated) {
  ...
  const auto sub_query_c = pqp_sub_query_(projection_c, DataType::Int, true);
  EXPECT_TRUE(test_expression<int32_t>(table_a, *in_(3, sub_query_c), {0}));
  EXPECT_TRUE(test_expression<int32_t>(table_a, *in_(4, sub_query_c), {1}));
  EXPECT_THROW(test_expression<int32_t>(table_a, *in_(4, sub_query_c), {0}, uncorrelated_sub_query_results),
               std::logic_error);
}
...
TEST_F(ExpressionEvaluatorToValuesTest, InSubQueryCorrelated) {
  ...
  const auto sub_query_a = pqp_sub_query_(pqp_a, DataType::Int, false, std::make_pair(ParameterID{0}, ColumnID{0}));

  EXPECT_TRUE(test_expression<int32_t>(table_a, *in_(4, sub_query_a), {1, 1, 0, 1}));
  EXPECT_TRUE(test_expression<int32_t>(table_a, *in_(6, sub_query_a), {0, 1, 1, 0}));
  EXPECT_TRUE(test_expression<int32_t>(table_a, *in_(16, sub_query_a), {0, 0, 0, 1}));
  EXPECT_TRUE(test_expression<int32_t>(table_a, *in_(b, sub_query_a), {1, 0, 0, 0}));
  EXPECT_TRUE(test_expression<int32_t>(table_a, *in_(null_(), sub_query_a),
                                       {std::nullopt, std::nullopt, std::nullopt, std::nullopt}));
  EXPECT_TRUE(test_expression<int32_t>(table_a, *not_in_(4, sub_query_a), {0, 0, 1, 0}));
  EXPECT_TRUE(test_expression<int32_t>(table_a, *not_in_(6, sub_query_a), {1, 0, 0, 1}));
  EXPECT_TRUE(test_expression<int32_t>(table_a, *not_in_(16, sub_query_a), {1, 1, 1, 0}));
  EXPECT_TRUE(test_expression<int32_t>(table_a, *not_in_(b, sub_query_a), {0, 1, 1, 1}));
  EXPECT_TRUE(test_expression<int32_t>(table_a, *not_in_(null_(), sub_query_a),
                                       {std::nullopt, std::nullopt, std::nullopt, std::nullopt}));
  ...
  const auto sub_query_b = pqp_sub_query_(pqp_b, DataType::Int, true, std::make_pair(ParameterID{0}, ColumnID{0}));

  EXPECT_TRUE(test_expression<int32_t>(table_a, *in_(34, sub_query_b), {1, std::nullopt, std::nullopt, std::nullopt}));
  EXPECT_TRUE(test_expression<int32_t>(table_a, *in_(35, sub_query_b), {1, 1, std::nullopt, std::nullopt}));
  EXPECT_TRUE(test_expression<int32_t>(table_a, *in_(null_(), sub_query_b),
                                       {std::nullopt, std::nullopt, std::nullopt, std::nullopt}));
  EXPECT_TRUE(test_expression<int32_t>(table_a, *in_(36, sub_query_b), {std::nullopt, 1, 1, std::nullopt}));
  ...
  EXPECT_TRUE(test_expression<int32_t>(table_a, *not_in_(null_(), sub_query_b),
                                       {std::nullopt, std::nullopt, std::nullopt, std::nullopt}));
  EXPECT_TRUE(test_expression<int32_t>(table_a, *not_in_(36, sub_query_b), {std::nullopt, 0, 0, std::nullopt}));
  EXPECT_TRUE(test_expression<int32_t>(table_a, *not_in_(36.0, sub_query_b), {std::nullopt, 0, 0, std::nullopt}));
  EXPECT_TRUE(test_expression<int32_t>(table_a, *not_in_(36.3, sub_query_b),
                                       {std::nullopt, std::nullopt, std::nullopt, std::nullopt}));
}
...
TEST_F(ExpressionEvaluatorToValuesTest, NotInArbitraryExpression) {
  // We support `<expression_a> IN <expression_b>`, even though it looks weird, because <expression_b> might be a column
  // storing the pre-computed result of a sub_query
...
TEST_F(ExpressionEvaluatorToValuesTest, Exists) {
  ...
  const auto pqp_sub_query_expression =
      pqp_sub_query_(a_plus_x_eq_13_scan, DataType::Int, false, std::make_pair(ParameterID{0}, ColumnID{0}));

  const auto exists_expression = exists_(pqp_sub_query_expression);
  EXPECT_TRUE(test_expression<int32_t>(table_a, *exists_expression, {0, 0, 1, 1}));

  const auto not_exists_expression = not_exists_(pqp_sub_query_expression);
