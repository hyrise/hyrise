...
class PQPSubQueryExpressionTest : public BaseTest {
 public:
  void SetUp() {
    ...
    sub_query_table = std::make_shared<PQPSubQueryExpression>(pqp_table);
  }

  std::shared_ptr<Table> table_a;
  std::shared_ptr<PQPColumnExpression> a_a, a_b;
  PQPSubQueryExpression::Parameters parameters_a;
  std::shared_ptr<PQPSubQueryExpression> sub_query_single_value_one_parameter;
  std::shared_ptr<PQPSubQueryExpression> sub_query_table;
...
TEST_F(PQPSubQueryExpressionTest, DeepEquals) {
  ...
  EXPECT_ANY_THROW(dummy(*sub_query_single_value_one_parameter == *sub_query_single_value_one_parameter));
  EXPECT_ANY_THROW(dummy(*sub_query_table == *sub_query_table));
}

TEST_F(PQPSubQueryExpressionTest, DeepCopy) {
  const auto sub_query_single_value_one_parameter_copy =
      std::dynamic_pointer_cast<PQPSubQueryExpression>(sub_query_single_value_one_parameter->deep_copy());
  ASSERT_TRUE(sub_query_single_value_one_parameter_copy);

  ASSERT_EQ(sub_query_single_value_one_parameter_copy->parameters.size(), 1u);
  EXPECT_EQ(sub_query_single_value_one_parameter_copy->parameters[0].first, ParameterID{2});
  EXPECT_EQ(sub_query_single_value_one_parameter_copy->parameters[0].second, ColumnID{3});
  EXPECT_NE(sub_query_single_value_one_parameter_copy->pqp, sub_query_single_value_one_parameter->pqp);
  EXPECT_EQ(sub_query_single_value_one_parameter_copy->pqp->type(), OperatorType::Limit);

  const auto sub_query_table_copy = std::dynamic_pointer_cast<PQPSubQueryExpression>(sub_query_table->deep_copy());
  ASSERT_TRUE(sub_query_table_copy);

  ASSERT_EQ(sub_query_table_copy->parameters.size(), 0u);
  EXPECT_NE(sub_query_table_copy->pqp, sub_query_single_value_one_parameter->pqp);
  EXPECT_EQ(sub_query_table_copy->pqp->type(), OperatorType::TableScan);
}

TEST_F(PQPSubQueryExpressionTest, RequiresCalculation) {
  EXPECT_TRUE(sub_query_single_value_one_parameter->requires_computation());
  EXPECT_TRUE(sub_query_table->requires_computation());
}
...
TEST_F(PQPSubQueryExpressionTest, DataType) {
  ...
  EXPECT_EQ(sub_query_float->data_type(), DataType::Float);
}

TEST_F(PQPSubQueryExpressionTest, IsNullable) {
  /** Nullability of PQPSubQueryExpressions only depends on the parameter passed */

  EXPECT_ANY_THROW(sub_query_table->is_nullable());
  EXPECT_FALSE(sub_query_single_value_one_parameter->is_nullable());
  const auto sub_query_nullable =
      std::make_shared<PQPSubQueryExpression>(pqp_single_value_one_parameter, DataType::Int, true, parameters_a);
  EXPECT_TRUE(sub_query_nullable->is_nullable());
}

TEST_F(PQPSubQueryExpressionTest, AsColumnName) {
  EXPECT_TRUE(std::regex_search(sub_query_table->as_column_name(), std::regex{"SUBQUERY \\(PQP, 0x[0-9a-f]+\\)"}));
  EXPECT_TRUE(std::regex_search(sub_query_single_value_one_parameter->as_column_name(),
