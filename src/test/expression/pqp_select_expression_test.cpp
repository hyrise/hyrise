...
TEST_F(PQPSelectExpressionTest, DeepCopy) {
  const auto select_single_value_one_parameter_copy = std::dynamic_pointer_cast<PQPSelectExpression>(select_single_value_one_parameter->deep_copy());
...
TEST_F(PQPSelectExpressionTest, DataType) {
  ...
  const auto select_float = std::make_shared<PQPSelectExpression>(pqp_single_value_one_parameter, DataType::Float, true, parameters_a);
...
TEST_F(PQPSelectExpressionTest, IsNullable) {
  ...
  const auto select_nullable = std::make_shared<PQPSelectExpression>(pqp_single_value_one_parameter, DataType::Int, true, parameters_a);
...
TEST_F(PQPSelectExpressionTest, AsColumnName) {
  ...
  EXPECT_TRUE(std::regex_search(select_single_value_one_parameter->as_column_name(), std::regex{"SUBSELECT \\(PQP, 0x[0-9a-f]+\\)"}));
}
