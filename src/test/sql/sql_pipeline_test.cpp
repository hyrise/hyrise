...
class SQLPipelineTest : public BaseTest {
 protected:
  static void SetUpTestCase() {  // called ONCE before the tests
    _table_a_multi = load_table("artifacts/test_data/tbl/int_float.tbl", 2);
    _table_a_multi->append({11, 11.11});
    _table_b = load_table("artifacts/test_data/tbl/int_float2.tbl", 2);
  ...
  void SetUp() override {
    ...
    _table_a = load_table("artifacts/test_data/tbl/int_float.tbl", 2);
