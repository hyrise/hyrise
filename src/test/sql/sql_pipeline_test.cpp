...
class SQLPipelineTest : public BaseTest {
 protected:
  ...
  bool _contains_validate(const std::shared_ptr<AbstractLQPNode>& root_node) {
    auto node = root_node;
    while (node != nullptr) {
...
TEST_F(SQLPipelineTest, GetUnoptimizedLQPInvalid) {
  SQLPipeline sql_pipeline(_invalid_sql);
  EXPECT_THROW(sql_pipeline.get_unoptimized_logical_plan(), std::exception);
}
...
TEST_F(SQLPipelineTest, GetOptimizedLQPInvalid) {
  SQLPipeline sql_pipeline(_invalid_sql);
  EXPECT_THROW(sql_pipeline.get_optimized_logical_plan(), std::exception);
}
...
TEST_F(SQLPipelineTest, GetTimes) {
  ...
  EXPECT_GT(sql_pipeline.compile_time_seconds(), 0.0f);
  EXPECT_GT(sql_pipeline.execution_time_seconds(), 0.0f);
}
