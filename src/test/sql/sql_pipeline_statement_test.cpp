...
TEST_F(SQLPipelineStatementTest, CopySubselectFromCache) {
  const auto sub_query_query = "SELECT * FROM table_int WHERE a = (SELECT MAX(b) FROM table_int)";

  auto first_sub_query_sql_pipeline = SQLPipelineBuilder{sub_query_query}.create_pipeline_statement();

  const auto first_sub_query_result = first_sub_query_sql_pipeline.get_result_table();
  ...
  EXPECT_TABLE_EQ_UNORDERED(first_sub_query_result, expected_first_result);

  SQLPipelineBuilder{"INSERT INTO table_int VALUES (11, 11, 11)"}.create_pipeline_statement().get_result_table();

  auto second_sub_query_sql_pipeline = SQLPipelineBuilder{sub_query_query}.create_pipeline_statement();
  const auto second_sub_query_result = second_sub_query_sql_pipeline.get_result_table();
  ...
  EXPECT_TABLE_EQ_UNORDERED(second_sub_query_result, expected_second_result);
}
