...
TEST_F(LQPTranslatorTest, SubQueryExpressionCorrelated) {
  ...
  const auto sub_query = lqp_sub_query_(sub_query_lqp, std::make_pair(ParameterID{0}, int_float5_a),
                                 std::make_pair(ParameterID{1}, int_float5_d));

  const auto lqp =
  ProjectionNode::make(expression_vector(sub_query, int_float5_a), int_float5_node);
  // clang-format on
...
TEST_F(LQPTranslatorTest, ReuseSubQueryExpression) {
  ...
  const auto sub_query_a = lqp_sub_query_(sub_query);
  const auto sub_query_b = lqp_sub_query_(sub_query);

  const auto lqp =
  ProjectionNode::make(expression_vector(add_(sub_query_a, 3)),
    ProjectionNode::make(expression_vector(5, sub_query_b),
      int_float_node));
  // clang-format on
  ...
  const auto sub_query_in_temporary_column = pqp_column_(ColumnID{1}, DataType::Int, false, "SUBQUERY");

  EXPECT_EQ(*projection_a->expressions.at(0), *add_(sub_query_in_temporary_column, 3));
}
