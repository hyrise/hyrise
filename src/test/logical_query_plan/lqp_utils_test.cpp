...
TEST_F(LQPUtilsTest, LQPColumnIsNullableWithoutOuterJoin) {
  // Test that for LQPs without (Left,Right)Outer Joins, lqp_column_is_nullable() is equivalent to
  // expression.is_nullable()

  // clang-format off
  const auto lqp =
  JoinNode::make(JoinMode::Inner, equals_(add_(a_a, null_()), b_x),
    ProjectionNode::make(expression_vector(a_a, a_b, add_(a_a, null_())),
     node_a),
    node_b);
  // clang-format on
...
TEST_F(LQPUtilsTest, LQPColumnIsNullableWithOuterJoin) {
  ...
  JoinNode::make(JoinMode::Left, equals_(a_a, b_x),
    node_a,
    node_b);
  // clang-format on
  ...
  ProjectionNode::make(expression_vector(a_a, b_x, add_(a_a, b_x), add_(a_a, 3), is_null_(add_(a_a, b_x))),
    JoinNode::make(JoinMode::Left, equals_(a_a, b_x),
      node_a,
      node_b));
  // clang-format on
  ...
  ProjectionNode::make(expression_vector(a_a, b_x, add_(a_a, b_x), add_(a_a, 3), is_null_(add_(a_a, b_x))),
    JoinNode::make(JoinMode::Right, equals_(a_a, b_x),
      node_a,
      node_b));
  // clang-format on
  ...
  ProjectionNode::make(expression_vector(a_a, b_x, add_(a_a, b_x), add_(a_a, 3), is_null_(add_(a_a, b_x))),
    JoinNode::make(JoinMode::Outer, equals_(a_a, b_x),
      node_a,
      node_b));
  // clang-format on
