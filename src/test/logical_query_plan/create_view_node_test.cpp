...
TEST_F(CreateViewNodeTest, Description) {
  EXPECT_EQ(_create_view_node->description(), "[CreateView] Name: 'some_view' (\n"
                                              "[0] [MockNode 'Unnamed']\n"
                                              ")");
}
...
TEST_F(CreateViewNodeTest, Equals) {
  ...
  const auto different_view = std::make_shared<View>(different_view_node, std::unordered_map<ColumnID, std::string>{{ColumnID{0}, {"b"}}});
...
TEST_F(CreateViewNodeTest, Copy) {
  ...
  const auto same_view = std::make_shared<View>(_view_node, std::unordered_map<ColumnID, std::string>{{ColumnID{0}, {"a"}}});
  const auto same_create_view_node = std::make_shared<CreateViewNode>("some_view", _view);
