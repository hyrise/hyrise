...
TEST_F(ProjectionNodeTest, NodeExpressions) {
  ASSERT_EQ(_projection_node->node_expression_count(), 5u);
  EXPECT_EQ(*_projection_node->node_expression(0), *lqp_column_(_c));
  EXPECT_EQ(*_projection_node->node_expression(1), *lqp_column_(_a));
  EXPECT_EQ(*_projection_node->node_expression(2), *lqp_column_(_b));
  EXPECT_EQ(*_projection_node->node_expression(3), *add_(_b, _c));
  EXPECT_EQ(*_projection_node->node_expression(4), *add_(_a, _c));
}
