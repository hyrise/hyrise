...
bool contains_predicate(const std::vector<std::shared_ptr<const AbstractJoinPlanPredicate>>& predicates,
...
TEST_F(JoinGraphTest, SingleVertexMultiplePredicates) {
  ...
  const auto vertex = std::make_shared<MockNode>(MockNode::ColumnDefinitions{{DataType::Int, "a"}, {DataType::Int, "b"}});
  ...
  const auto predicate_c = std::make_shared<JoinPlanLogicalPredicate>(
  predicate_a, JoinPlanPredicateLogicalOperator::Or, predicate_b);
...
TEST_F(JoinGraphTest, MultipleVerticesMultiplePredicates) {
  ...
  const auto predicate_b = std::make_shared<JoinPlanAtomicPredicate>(vertex_b_x, PredicateCondition::Equals, vertex_c_x);
  const auto predicate_c = std::make_shared<JoinPlanLogicalPredicate>(
  predicate_a, JoinPlanPredicateLogicalOperator::Or, predicate_b);
  const auto predicate_d = std::make_shared<JoinPlanLogicalPredicate>(
  predicate_a, JoinPlanPredicateLogicalOperator::And, predicate_b);

  const auto join_graph = JoinGraph::from_predicates({vertex_a, vertex_b, vertex_c}, {}, {predicate_a, predicate_b, predicate_c, predicate_d});
...
TEST_F(JoinGraphTest, Print) {
  ...
  const auto predicate_a = std::make_shared<JoinPlanAtomicPredicate>(vertex_a_x, PredicateCondition::Equals, 5);
  const auto predicate_b = std::make_shared<JoinPlanAtomicPredicate>(vertex_b_y, PredicateCondition::Equals, vertex_c_z);
  const auto predicate_c = std::make_shared<JoinPlanLogicalPredicate>(
  predicate_a, JoinPlanPredicateLogicalOperator::Or, predicate_b);

  const auto join_graph = JoinGraph::from_predicates({vertex_a, vertex_b, vertex_c}, {}, {predicate_a, predicate_b, predicate_c});
