...
TEST_F(ASTToOperatorTranslatorTest, AggregateNodeNoArithmetics) {
  ...
  auto sum_expression =
    Expression::create_aggregate_function(AggregateFunction::Sum, {Expression::create_column_identifier(ColumnID{0})}, {"sum_of_a"});
...
TEST_F(ASTToOperatorTranslatorTest, AggregateNodeWithArithmetics) {
  ...
  auto sum_expression = Expression::create_aggregate_function(AggregateFunction::Sum, {expr_multiplication}, {"sum_of_b_times_two"});
  auto aggregate_node = std::make_shared<AggregateNode>(std::vector<std::shared_ptr<Expression>>{sum_expression},
