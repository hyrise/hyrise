...
TEST_F(JoinDetectionRuleTest, NoOptimizationAcrossProjection) {
  ...
  const auto projection_node = ProjectionNode::make(expression_vector(_a_a, _b_a));
  projection_node->set_left_input(join_node);

  const auto predicate_node = PredicateNode::make(equals(_a_a, _b_a));
  predicate_node->set_left_input(projection_node);

  auto actual_lqp = StrategyBaseTest::apply_rule(_rule, predicate_node);

  // clang-format off
  const auto expected_lqp =
  PredicateNode::make(equals(_a_a, _b_a),
    ProjectionNode::make(expression_vector(_a_a, _b_a),
      JoinNode::make(JoinMode::Cross,
          _table_node_a,
          _table_node_b
  )));
