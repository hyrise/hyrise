...
class AggregateNodeTest : public BaseTest {
 protected:
  void SetUp() override {
    ...
    _aggregate_node = std::make_shared<AggregateNode>(std::vector<std::shared_ptr<ExpressionNode>>{
      ExpressionNode::create_function_reference("SUM", {
...
TEST_F(AggregateNodeTest, ColumnIdForColumnIdentifier) {
  EXPECT_EQ(_aggregate_node->get_column_id_for_column_identifier({"a", nullopt}), 0);
  EXPECT_EQ(_aggregate_node->get_column_id_for_column_identifier({"a", "t_a"}), 0);
  EXPECT_EQ(_aggregate_node->find_column_id_for_column_identifier({"b", nullopt}), nullopt);
  EXPECT_EQ(_aggregate_node->find_column_id_for_column_identifier({"b", "t_a"}), nullopt);

  EXPECT_EQ(_aggregate_node->get_column_id_for_column_identifier({"some_sum", nullopt}), 2);
  EXPECT_EQ(_aggregate_node->find_column_id_for_column_identifier({"some_sum", "t_a"}), nullopt);
}
...
//  ), 0);
  ...
  // There is no a+b
  EXPECT_EQ(_aggregate_node->find_column_id_for_expression(
    ExpressionNode::create_binary_operator(
      ExpressionType::Addition,
      ExpressionNode::create_column_identifier(ColumnID{0}),
      ExpressionNode::create_column_identifier(ColumnID{1})
