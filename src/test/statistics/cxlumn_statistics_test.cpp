...
class CxlumnStatisticsTest : public BaseTest {
 protected:
  void SetUp() override {
    ...
    _cxlumn_statistics_uniform_columns = table_statistics2.cxlumn_statistics();
  }

  // For single value scans (i.e. all but BETWEEN)
  template <typename T>
  void predict_selectivities_and_compare(const std::shared_ptr<CxlumnStatistics<T>>& column_statistic,
  ...
  // For two column scans (type of value1 is CxlumnID)
  void predict_selectivities_and_compare(
    ...
    for (CxlumnID::base_type column_1 = 0; column_1 < cxlumn_statistics.size(); ++column_1) {
      for (CxlumnID::base_type column_2 = 0; column_2 < cxlumn_statistics.size() && column_1 != column_2; ++column_2) {
        auto result_container = cxlumn_statistics[column_1]->estimate_predicate_with_column(
            predicate_condition, *cxlumn_statistics[column_2]);
        auto table_scan =
            std::make_shared<TableScan>(table_wrapper, CxlumnID{column_1}, predicate_condition, CxlumnID{column_2});
        table_scan->execute();
  ...
  void predict_selectivities_for_stored_procedures_and_compare(
      const std::shared_ptr<CxlumnStatistics<T>>& column_statistic, const PredicateCondition predicate_condition,
  ...
  std::vector<std::shared_ptr<const BaseCxlumnStatistics>> _cxlumn_statistics_uniform_columns;
...
TEST_F(CxlumnStatisticsTest, TwoColumnsEqualsTest) {
  PredicateCondition predicate_condition = PredicateCondition::Equals;

  auto col_stat1 = std::make_shared<CxlumnStatistics<int>>(0.0f, 10.f, 0, 10);
  auto col_stat2 = std::make_shared<CxlumnStatistics<int>>(0.0f, 10.f, -10, 20);
  ...
  auto col_stat3 = std::make_shared<CxlumnStatistics<float>>(0.0f, 10.f, 0.f, 10.f);
  auto col_stat4 = std::make_shared<CxlumnStatistics<float>>(0.0f, 3.f, -10.f, 20.f);
  ...
  auto col_stat5 = std::make_shared<CxlumnStatistics<float>>(0.0f, 10.f, 20.f, 30.f);
...
TEST_F(CxlumnStatisticsTest, TwoColumnsLessThanTest) {
  PredicateCondition predicate_condition = PredicateCondition::LessThan;

  auto col_stat1 = std::make_shared<CxlumnStatistics<int>>(0.0f, 10.f, 1, 20);
  auto col_stat2 = std::make_shared<CxlumnStatistics<int>>(0.0f, 30.f, 11, 40);
  ...
  auto col_stat3 = std::make_shared<CxlumnStatistics<float>>(0.0f, 6.f, 0, 10);
  auto col_stat4 = std::make_shared<CxlumnStatistics<float>>(0.0f, 12.f, -10, 30);
...
TEST_F(CxlumnStatisticsTest, TwoColumnsRealDataTest) {
  ...
  for (auto predicate_condition : predicate_conditions) {
    predict_selectivities_and_compare(_table_uniform_distribution, _cxlumn_statistics_uniform_columns,
                                      predicate_condition);
  }
}

TEST_F(CxlumnStatisticsTest, NonNullRatioOneColumnTest) {
...
TEST_F(CxlumnStatisticsTest, NonNullRatioTwoColumnTest) {
  auto stats_0 =
      std::const_pointer_cast<BaseCxlumnStatistics>(_cxlumn_statistics_uniform_columns[0]);  // values from 0 to 5
  auto stats_1 =
      std::const_pointer_cast<BaseCxlumnStatistics>(_cxlumn_statistics_uniform_columns[1]);  // values from 0 to 2
  auto stats_2 =
      std::const_pointer_cast<BaseCxlumnStatistics>(_cxlumn_statistics_uniform_columns[2]);  // values from 1 to 2
