...
TEST_F(GenericHistogramTest, IsUniformlyDistributed) {
  ...
  const auto almost_equally_distributed_table = load_table("resources/test_data/tbl/int_almost_equal_distribution.tbl");
  const auto not_equally_distributed_table = load_table("resources/test_data/tbl/int_not_equal_distribution.tbl");

  const auto table_statistics_equal = TableStatistics::from_table(*equally_distributed_table);
  const auto table_statistics_almost = TableStatistics::from_table(*almost_equally_distributed_table);
  const auto table_statistics_not = TableStatistics::from_table(*not_equally_distributed_table);
  
  for (const auto column_statistic : table_statistics_equal->column_statistics) {
  ...
  for (const auto column_statistic : table_statistics_almost->column_statistics) {
    const auto statistic = std::dynamic_pointer_cast<AttributeStatistics<int32_t>>(column_statistic);
    ASSERT_TRUE(statistic);
    ...
    ASSERT_TRUE(histogram->is_uniformly_distributed(0.2));
  }

  for (const auto column_statistic : table_statistics_not->column_statistics) {
    const auto statistic = std::dynamic_pointer_cast<AttributeStatistics<int32_t>>(column_statistic);
    ASSERT_TRUE(statistic);
