...
TEST_F(HistogramUtilsTest, ValueDistributionFromColumnString) {
  ...
  const auto actual_distribution = value_distribution_from_column<std::string>(*table, ColumnID{1}, StringHistogramDomain{'A', 'B', 2});
  const auto expected_distribution = std::vector<std::pair<std::string, HistogramCountType>>{
    {"A", 1},
    {"B", 2}
  };
