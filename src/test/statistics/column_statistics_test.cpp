...
TEST_F(ColumnStatisticsTest, SetStatisticsObject) {
  ...
  column_statistics.set_statistics_object(std::make_shared<RangeFilter<int32_t>>(std::vector{std::pair<int32_t, int32_t>(0, 100)}));
...
TEST_F(ColumnStatisticsTest, Scaled) {
  ...
  column_statistics.range_filter = std::make_shared<RangeFilter<int32_t>>(std::vector{std::pair<int32_t, int32_t>(0, 100)});
  ...
  const auto scaled_column_statistics = std::dynamic_pointer_cast<ColumnStatistics<int32_t>>(column_statistics.scaled(0.5f));
...
TEST_F(ColumnStatisticsTest, Sliced) {
  ...
  const auto scaled_column_statistics = std::dynamic_pointer_cast<ColumnStatistics<int32_t>>(column_statistics.sliced(PredicateCondition::GreaterThanEquals, 51));
  ASSERT_NE(scaled_column_statistics, nullptr);
  EXPECT_NE(scaled_column_statistics->histogram, nullptr);
  EXPECT_NE(scaled_column_statistics->min_max_filter, nullptr);
  EXPECT_NE(scaled_column_statistics->range_filter, nullptr);
