...
class AdaptiveRadixTreeIndexTest : public BaseTest {
 protected:
  void SetUp() override {
    ...
    index1 = std::make_shared<AdaptiveRadixTreeIndex>(std::vector<BaseColumnCSPtr>({dict_col1}));
  ...
  AdaptiveRadixTreeIndexSPtr index1 = nullptr;
  BaseColumnSPtr dict_col1 = nullptr;
  ARTNodeSPtr root = nullptr;
...
TEST_F(AdaptiveRadixTreeIndexTest, VectorOfRandomInts) {
  ...
  auto index = std::make_shared<AdaptiveRadixTreeIndex>(std::vector<BaseColumnCSPtr>({column}));
