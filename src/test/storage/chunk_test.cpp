...
class StorageChunkTest : public BaseTest {
 protected:
  void SetUp() override {
    vc_int = make_shared_by_data_type<BaseColumn, ValueColumn>(TypeSymbol::Int);
    vc_int->append(4);
    vc_int->append(6);
    vc_int->append(3);

    vc_str = make_shared_by_data_type<BaseColumn, ValueColumn>(TypeSymbol::String);
    vc_str->append("Hello,");
    vc_str->append("world");
    vc_str->append("!");
  }
...
TEST_F(StorageChunkTest, UnknownColumnType) {
  ...
  if (IS_DEBUG) {
    auto wrapper = []() { make_shared_by_data_type<BaseColumn, ValueColumn>(TypeSymbol::Null); };
