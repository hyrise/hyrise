...
TEST_F(StorageTableTest, GetColumnType) {
  EXPECT_EQ(t.column_type(ColumnID{0}), TypeSymbol::Int);
  EXPECT_EQ(t.column_type(ColumnID{1}), TypeSymbol::String);
  // TODO(anyone): Do we want checks here?
...
TEST_F(StorageTableTest, ColumnNameTooLong) {
  EXPECT_THROW(t.add_column(std::string(std::numeric_limits<ColumnNameLength>::max() + 1ul, 'A'), TypeSymbol::Int);
               , std::exception);
}
...
TEST_F(StorageTableTest, EmplaceChunk) {
  ...
  t.append({4, "Hello,"});
  std::shared_ptr<BaseColumn> vc_int = make_shared_by_data_type<BaseColumn, ValueColumn>(TypeSymbol::Int);
  std::shared_ptr<BaseColumn> vc_str = make_shared_by_data_type<BaseColumn, ValueColumn>(TypeSymbol::String);
  Chunk c;
  c.add_column(vc_int);
  c.add_column(vc_str);
...
TEST_F(StorageTableTest, EmplaceChunkDoesNotReplaceIfNumberOfChunksGreaterOne) {
  ...
  std::shared_ptr<BaseColumn> vc_int2 = make_shared_by_data_type<BaseColumn, ValueColumn>(TypeSymbol::Int);
  std::shared_ptr<BaseColumn> vc_str2 = make_shared_by_data_type<BaseColumn, ValueColumn>(TypeSymbol::String);
  Chunk c2;
