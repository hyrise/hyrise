...
class ColumnIterableTest : public BaseTest {
 public:
  void SetUp() override {
    const auto table_column_definitions = TableColumnDefinitions{{"a", DataType::Int}};
...
TEST_F(ColumnIterableTest, ForEachFromRowID) {
  ...
  auto end_row_id_b = column_iterable.for_each<int32_t>(
      [&](const auto& segment_postition, const RowID& row_id) {
  ...
  auto end_row_id_c = column_iterable.for_each<int32_t>(
      [&](const auto& segment_postition, const RowID& row_id) {
