...
class EndToEndTest : public BaseTest, public ::testing::WithParamInterface<SQLTestParam> {
 protected:
  ...
  void execute_and_check(const std::string & query, const std::string & expected_result_path,
                         bool order_sensitive = false) {
    auto expected_result = load_table(expected_result_path, 2);
    ...
    EXPECT_TABLE_EQ(result_table, expected_result, order_sensitive);
  }
