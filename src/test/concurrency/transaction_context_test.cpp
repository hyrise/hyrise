...
TEST_F(TransactionContextTest, CommitShouldCommitAllFollowingPendingTransactions) {
  ...
  auto context_1 = manager().new_transaction_context(IsAutoCommitTransaction::Yes);
  auto context_2 = manager().new_transaction_context(IsAutoCommitTransaction::Yes);
...
TEST_F(TransactionContextTest, CommitShouldIncreaseCommitIDIfReadWrite) {
  auto context = manager().new_transaction_context(IsAutoCommitTransaction::Yes);
