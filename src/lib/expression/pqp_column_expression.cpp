...
std::shared_ptr<PQPColumnExpression> PQPColumnExpression::from_table(const Table& table,
  ...
  return std::make_shared<PQPColumnExpression>(column_id, table.column_data_type(column_id), column_name);
}
...
PQPColumnExpression::PQPColumnExpression(const ColumnID init_column_id, const DataType data_type,
                                         const std::string& column_name)
...
std::shared_ptr<AbstractExpression> PQPColumnExpression::_on_deep_copy(
  ...
  return std::make_shared<PQPColumnExpression>(column_id, _data_type, _column_name);
}
