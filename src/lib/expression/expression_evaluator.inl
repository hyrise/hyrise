...
struct DefaultBaseOperator {
  static constexpr auto may_produce_nulls_from_values = false;
  static constexpr auto may_produce_value_from_null = false;
};

template<typename ResultDataType, typename Functor>
struct BinaryFunctorWrapper : public DefaultBaseOperator {
...
ExpressionEvaluator::ExpressionResult<T> ExpressionEvaluator::evaluate_expression(const AbstractExpression& expression) {
  switch (expression.type) {
    ...
    case ExpressionType::Predicate: {
      const auto& predicate_expression = static_cast<const

      return evaluate_binary_predicate_expression<T>(static_cast<const ArithmeticExpression&>(expression));
...
ExpressionEvaluator::ExpressionResult<ResultDataType> ExpressionEvaluator::evaluate_binary_operator(const ExpressionResult<LeftOperandDataType>& left_operands,
  ...
  if constexpr (result_is_string || left_is_string || right_is_string) {
    ...
    const auto result_is_nullable = left_is_nullable || left_is_null || right_is_nullable || right_is_null || Functor::may_produce_nulls_from_values;
