...
size_t count_expensive_child_expressions(const std::shared_ptr<AbstractExpression>& expression) {
  auto count_recursive_impl = [](const std::shared_ptr<AbstractExpression>& current_expression, size_t counter,
                                 auto& count_recursive) -> size_t {
    for (const auto& argument : current_expression->arguments) {
      counter = count_recursive(argument, counter, count_recursive);
    }

    if (current_expression->requires_computation() || current_expression->type == ExpressionType::PQPColumn) {
      return counter + 1;
    }
    return counter;
  };
