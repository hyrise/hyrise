...
void expression_set_parameters(const std::shared_ptr<AbstractExpression>& expression,
  ...
  visit_expression(expression, [&](auto& sub_expression) {
    ...
    } else if (const auto pqp_sub_query_expression = std::dynamic_pointer_cast<PQPSubQueryExpression>(sub_expression);
               pqp_sub_query_expression) {
      pqp_sub_query_expression->pqp->set_parameters(parameters);
      return ExpressionVisitation::DoNotVisitArguments;
...
void expression_set_transaction_context(const std::shared_ptr<AbstractExpression>& expression,
  ...
  visit_expression(expression, [&](auto& sub_expression) {
    ...
    Assert(pqp_sub_query_expression, "Expected a PQPSubQueryExpression here")
        pqp_sub_query_expression->pqp->set_transaction_context_recursively(transaction_context);
