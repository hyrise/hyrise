...
std::string IsNullExpression::as_cxlumn_name() const {
  std::stringstream stream;

  if (predicate_condition == PredicateCondition::IsNull) {
    stream << _enclose_argument_as_cxlumn_name(*operand()) << " IS NULL";
  } else {
    stream << _enclose_argument_as_cxlumn_name(*operand()) << " IS NOT NULL";
  }
