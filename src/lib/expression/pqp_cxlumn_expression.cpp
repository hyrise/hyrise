...
bool PQPCxlumnExpression::_shallow_equals(const AbstractExpression& expression) const {
  const auto* pqp_cxlumn_expression = dynamic_cast<const PQPCxlumnExpression*>(&expression);
  if (!pqp_cxlumn_expression) return false;
  return cxlumn_id == pqp_cxlumn_expression->cxlumn_id;
}
