...
std::shared_ptr<ExistsExpression> exists_(const std::shared_ptr<AbstractExpression>& sub_query_expression) {  // NOLINT - clang-tidy doesn't like the suffix
  return std::make_shared<ExistsExpression>(sub_query_expression, ExistsExpressionType::Exists);
}

std::shared_ptr<ExistsExpression> not_exists_(const std::shared_ptr<AbstractExpression>& sub_query_expression) {  // NOLINT - clang-tidy doesn't like the suffix
  return std::make_shared<ExistsExpression>(sub_query_expression, ExistsExpressionType::NotExists);
}
// clang-format on
