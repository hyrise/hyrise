...
using CaseResult = boost::variant<Null, ColumnReference, Result>;
...
struct AbstractCaseExpression {
  AbstractCaseExpression(const DataType result_data_type, const std::optional<std::string>& alias)
      : result_data_type(result_data_type), alias(alias) {}
...
struct CaseExpression : public AbstractCaseExpression {
  ...
  explicit CaseExpression(const ClauseType& clause, const ElseType& else_ = Null{},
      ...
      : CaseExpression(std::vector<ClauseType>{clause}, else_, alias) {}
  explicit CaseExpression(const std::vector<ClauseType>& clauses, const ElseType& else_ = Null{},
                          const std::optional<std::string>& alias = std::nullopt)
      : AbstractCaseExpression(data_type_from_type<Result>(), alias), clauses(clauses), else_(else_) {}
