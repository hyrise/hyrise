...
class CsvRfcParser {
 public:
   ...
   */
  explicit CsvRfcParser(size_t buffer_size, const CsvConfig& csv_config = {});
 ...
 protected:
   ...
   *  Column Type,c,float
   */
  static const std::shared_ptr<Table> _process_meta_file(const std::string& meta_file, const CsvConfig& config);
