...
class AbstractCsvConverter {
 public:
  ...
  virtual void insert(const char *value, ChunkOffset position) = 0;
...
class CsvConverter : public AbstractCsvConverter {
 public:
  explicit CsvConverter(ChunkOffset size, CsvConfig config) : _parsed_values(size), _config(config) {}
 ...
 private:
  /*
   * Returns a conversion function that converts from a null terminated string to type T.
   ...
   * csv characters.
   */
  static std::function<T(const char *)> _get_conversion_function(CsvConfig config);
  tbb::concurrent_vector<T> _parsed_values;
  CsvConfig _config;
};

template <>
inline std::function<int(const char *)> CsvConverter<int>::_get_conversion_function(CsvConfig config) {
  return [](const char *str) { return std::atoi(str); };
}

template <>
inline std::function<int64_t(const char *)> CsvConverter<int64_t>::_get_conversion_function(CsvConfig config) {
  return [](const char *str) { return static_cast<int64_t>(std::atoll(str)); };
}

template <>
inline std::function<float(const char *)> CsvConverter<float>::_get_conversion_function(CsvConfig config) {
  return [](const char *str) { return std::atof(str); };
}

template <>
inline std::function<double(const char *)> CsvConverter<double>::_get_conversion_function(CsvConfig config) {
  // atof actually converts to a double and can be used here
  return [](const char *str) { return std::atof(str); };
}

template <>
inline std::function<std::string(const char *)> CsvConverter<std::string>::_get_conversion_function(CsvConfig config) {
