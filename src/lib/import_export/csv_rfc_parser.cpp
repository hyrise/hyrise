...
CsvRfcParser::CsvRfcParser(size_t buffer_size, CsvConfig csv_config)
...
const std::shared_ptr<Table> CsvRfcParser::_process_meta_file(const std::string& meta_file, CsvConfig config) {
  std::ifstream file;
  file.exceptions(std::ifstream::failbit | std::ifstream::badbit);
