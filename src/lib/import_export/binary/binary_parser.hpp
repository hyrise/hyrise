...
class BinaryParser {
 ...
 private:
   ...
   */
  static std::tuple<const std::shared_ptr<Segments>, std::shared_ptr<MvccData>, uint32_t, std::vector<SortColumnDefinition>> _import_chunk(std::ifstream& file, std::shared_ptr<Table>& table);
