...
void ParseServerPreparedStatementTask::_on_execute() {
  try {
    ...
    const auto prepared_statement = std::make_shared<LQPPreparedStatement>(prepared_plans[0], sql_translator.value_placeholder_parameter_ids());

    _promise.set_value(prepared_statement);
