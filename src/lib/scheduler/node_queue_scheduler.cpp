...
void NodeQueueScheduler::finish() {
  ...
  for (auto node_id = NodeID{0}; node_id < _node_count; ++node_id) {
    ...
    for (auto worker_id = size_t{0}; worker_id < node_worker_count; ++worker_id) {
      ...
      shut_down_task->set_node_id(node_id);
      shut_down_task->schedule();
    }
  }
...
void NodeQueueScheduler::_group_numa_aware(const std::vector<std::shared_ptr<AbstractTask>>& tasks) const {
  auto round_robin_counter = std::vector<int>(_queues.size(), 0);

  auto grouped_tasks = std::vector<std::shared_ptr<AbstractTask>>(NUM_GROUPS_PER_NODE * _queues.size());
