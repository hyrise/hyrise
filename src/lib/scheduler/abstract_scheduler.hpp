...
class AbstractScheduler : public Noncopyable {
 public:
  ...
  void schedule_and_wait_for_tasks(std::vector<std::shared_ptr<AbstractTask>>& tasks);
 ...
 protected:
  ...
  virtual void _group_tasks(std::vector<std::shared_ptr<AbstractTask>>& tasks) const;
};
