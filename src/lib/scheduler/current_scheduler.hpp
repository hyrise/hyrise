 ...
 */
class CurrentScheduler : public Noncopyable {
 public:
  inline const std::shared_ptr<AbstractScheduler>& get() { return _instance; }
  void set(const std::shared_ptr<AbstractScheduler>& instance);
 ...
 private:
  CurrentScheduler() = default;

  friend class Hyrise;

  std::shared_ptr<AbstractScheduler> _instance = std::make_shared<NoScheduler>();
};
