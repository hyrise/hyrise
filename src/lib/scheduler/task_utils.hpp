...
std::pair<size_t, std::vector<std::shared_ptr<AbstractTask>>> group_chunks_for_scheduling(const std::shared_ptr<const Table>& table, const Functor& functor) {
  ...
  // We assert that jobs have been properly created. Unless we have an empty table and nothing needs to be done.
  Assert(chunk_count == 0 || jobs.size() == group_count, "Number of jobs should equal group count.");
  Assert(chunk_count == 0 || group_id == group_count, "Unexpected number of groups created.");
