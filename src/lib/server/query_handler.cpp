...
void QueryHandler::setup_prepared_plan(const std::string& statement_name, const std::string& query) {
  ...
  auto prepared_plans = translation_result.first;
  ...
  const auto prepared_plan =
      std::make_shared<PreparedPlan>(prepared_plans[0], translation_result.second.parameter_ids_of_value_placeholders);
