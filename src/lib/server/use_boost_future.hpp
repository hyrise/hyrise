...
#ifndef BOOST_ASIO_USE_BOOST_FUTURE_HPP
#define BOOST_ASIO_USE_BOOST_FUTURE_HPP
...
#if defined(BOOST_ASIO_HAS_STD_FUTURE_CLASS) || defined(GENERATING_DOCUMENTATION)
 ...
 * boost::asio::use_boost_future. For example:
 *
...
class use_boost_future_t {
 public:
  ...
  template <typename OtherAllocator>
  use_boost_future_t<OtherAllocator> rebind(const OtherAllocator& allocator) const {
    return use_boost_future_t<OtherAllocator>(allocator);
  }
   ...
   *     use_boost_future([](boost::system::error_code ec, std::size_t n)
   *       {
   *         return ec ? 0 : n;
   *       }));
   * ...
  ...
  detail::boost_packaged_token<typename decay<Function>::type, Allocator>
#endif  // defined(GENERATING_DOCUMENTATION)
  operator()(BOOST_ASIO_MOVE_ARG(Function) f) const;

 private:
  // Helper type to ensure that use_boost_future can be constexpr default-constructed
  ...
  typename conditional<is_same<std::allocator<void>, Allocator>::value, std_allocator_void, Allocator>::type allocator_;
};
