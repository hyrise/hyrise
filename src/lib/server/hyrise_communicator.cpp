...
uint64_t HyriseCommunicator::send_query_response(std::shared_ptr<const Table> table,std::shared_ptr<PostgresHandler> postgres_handler) {
...
std::pair<std::shared_ptr<const Table>, OperatorType> HyriseCommunicator::execute_pipeline(
...
std::shared_ptr<AbstractOperator> HyriseCommunicator::bind_prepared_plan(const PreparedStatementDetails& statement_details) {
  ...
  Assert(statement_details.parameters.size() == prepared_plan->parameter_ids.size(), "Prepared statement parameter count mismatch");

  // TODO(toni): WTH?
  if (statement_details.statement_name.empty()) Hyrise::get().storage_manager.drop_prepared_plan(statement_details.statement_name);
  ...
  for (auto parameter_idx = size_t{0}; parameter_idx < statement_details.parameters.size(); ++parameter_idx) {
    parameter_expressions[parameter_idx] = std::make_shared<ValueExpression>(statement_details.parameters[parameter_idx]);
  }
...
std::shared_ptr<const Table> HyriseCommunicator::execute_prepared_statement(std::shared_ptr<AbstractOperator> physical_plan){
