...
class SimpleQueryCommand : public AbstractCommand {
 public:
  explicit SimpleQueryCommand(HyriseSession& session)
      : AbstractCommand(session), _state(SimpleQueryCommandState::Started) {}
 ...
 protected:
  enum class SimpleQueryCommandState { Started, RowDescriptionSent, RowDataSent, CommandCompleteSent };
