...
ParameterizedPlanCacheHandler::ParameterizedPlanCacheHandler(const std::shared_ptr<SQLLogicalPlanCache>& lqp_cache, const std::shared_ptr<AbstractLQPNode>& unoptimized_lqp, std::chrono::nanoseconds& cache_duration, const UseMvcc use_mvcc) : _lqp_cache(lqp_cache), _cache_duration(cache_duration), _use_mvcc(use_mvcc) {
...
const std::tuple<std::shared_ptr<AbstractLQPNode>, std::vector<std::shared_ptr<AbstractExpression>>> ParameterizedPlanCacheHandler::_split_lqp_values(const std::shared_ptr<AbstractLQPNode>& lqp) {
