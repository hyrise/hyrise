...
class ParameterizedPlanCacheHandler {
 public:
 	ParameterizedPlanCacheHandler(const std::shared_ptr<SQLLogicalPlanCache>& lqp_cache, const std::shared_ptr<AbstractLQPNode>& unoptimized_lqp, std::chrono::nanoseconds& cache_duration, const UseMvcc use_mvcc);
 ...
 private:
 	...
 	static const std::tuple<std::shared_ptr<AbstractLQPNode>, std::vector<std::shared_ptr<AbstractExpression>>> _split_lqp_values(const std::shared_ptr<AbstractLQPNode>& lqp);
