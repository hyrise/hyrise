...
bool all_type_variant_near(const AllTypeVariant& lhs, const AllTypeVariant& rhs, double max_abs_error) {
  ...
  } else if (lhs.type() == typeid(double) && rhs.type() == typeid(float)) {  //  NOLINT - see above
    return std::fabs(boost::get<double>(lhs) - boost::get<float>(rhs)) < max_abs_error;
  } else if (lhs.type() == typeid(float) && rhs.type() == typeid(double)) {  //  NOLINT - see above
    return std::fabs(boost::get<float>(lhs) - boost::get<double>(rhs)) < max_abs_error;
  }
