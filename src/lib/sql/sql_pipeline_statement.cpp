...
const std::shared_ptr<const Table>& SQLPipelineStatement::get_result_table() {
  ...
  DebugAssert(_query_has_output || !_transaction_context ||
                  !dynamic_cast<const hsql::SelectStatement*>(_parsed_sql_statement->getStatement(0)) ||
                  !_result_table || _result_table->is_validated(),
              "Result table of a SELECT should have been validated");
