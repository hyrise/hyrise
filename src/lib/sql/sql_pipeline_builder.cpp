...
SQLPipeline SQLPipelineBuilder::create_pipeline() const {
  ...
  auto post_caching_optimizer = _post_caching_optimizer ? _post_caching_optimizer : Optimizer::create_post_caching_optimizer();
  auto pipeline = SQLPipeline(_sql, _transaction_context, _use_mvcc, optimizer, post_caching_optimizer, _pqp_cache, _lqp_cache);
