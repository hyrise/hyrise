...
namespace opossum {
  ...
  std::shared_ptr<AbstractLQPNode> SQLTranslator::_translate_insert(const hsql::InsertStatement &insert) {
    ...
    AssertInput(StorageManager::get().has_table(table_name),
  ...
  SQLTranslator::TableSourceState SQLTranslator::_translate_table_origin(const hsql::TableRef &hsql_table_ref) {
    ...
    for (const auto& expression : lqp->column_expressions()) {
      ...
      if (identifiers.empty()) {
        ...
        identifiers_in_order.emplace_back(
        identifiers.back()); // todo(jj): use vector<vector<SQLIdentifier>> / merge data structure into SQLIdentifierResolver?
      }
    }
    ...
    for (const auto &hsql_select_expr : select_list) {
      if (hsql_select_expr->type == hsql::kExprStar) {
        ...
        if (hsql_select_expr->alias) {
          ...
          _inflated_select_list_identifiers.emplace_back(SQLIdentifier{hsql_select_expr->alias});
        } else {
          _inflated_select_list_identifiers.emplace_back(SQLIdentifier{""});
        }
      }
    }
...
//              const auto identifiers = _filter_identifiers_by_expression(group_by_expression, select_list_elements);
    ...
    _inflated_select_list_identifiers.insert(_inflated_select_list_identifiers.begin(),
                                             new_select_list_identifiers.begin(),
                                             new_select_list_identifiers.end());
  ...
  SQLTranslator::_translate_create_view(const hsql::CreateStatement &create_statement) {
  ...
  std::shared_ptr<AbstractLQPNode>
  SQLTranslator::_translate_create_table(const hsql::CreateStatement &create_statement) {
