...
std::shared_ptr<const Table> SQLPipeline::get_result_table() {
  ...
  for (auto& pipeline_statement : _sql_pipeline_statements) {
    ...
    DTRACE_PROBE3(hyrise, query_start, pipeline_statement->get_sql_string(),
        pipeline_statement->get_tasks().size() , _sql_pipeline_statements);

    pipeline_statement->get_result_table();

    DTRACE_PROBE3(hyrise, query_end, pipeline_statement->get_sql_string(),
        pipeline_statement->get_tasks().size(),_sql_pipeline_statements);
