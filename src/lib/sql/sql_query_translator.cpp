    ...
    for (auto& filter : _filters_by_table) {
      ...
      if (table_filters.size() == 1) {
        ...
        for (auto filter_index = 0u; filter_index < table_filters.size(); filter_index++) {
          ...
          auto estimated_result_size = stats.predicate_result_size(table, get<0>(tuple), get<1>(tuple), get<2>(tuple));
        ...
        std::sort(predicate_estimates.begin(), predicate_estimates.end(), [](auto &left, auto &right) {
      ...
      for (auto &predicate_estimate : predicate_estimates) {
        ...
        auto table_scan = std::make_shared<TableScan>(
                input_task->get_operator(), ColumnName(column_name), filter_op, value);
