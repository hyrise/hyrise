...
void SQLQueryPlan::add_tree_by_root(const std::shared_ptr<AbstractOperator>& op) { _roots.push_back(op); }
...
SQLQueryPlan SQLQueryPlan::deep_copy() const {
  ...
  for (const auto& root : _roots) {
    DebugAssert(root != nullptr, "Root operator in plan should not be null.");
...
void SQLQueryPlan::set_transaction_context(const std::shared_ptr<TransactionContext>& context) {
  for (const auto& root : _roots) {
