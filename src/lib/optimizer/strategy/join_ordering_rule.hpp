...
class JoinOrderingRule : public AbstractRule {
 ...
 private:
  std::shared_ptr<AbstractLQPNode> _traverse(const std::shared_ptr<AbstractLQPNode>& lqp) const;
  void _traverse_inputs(const std::shared_ptr<AbstractLQPNode>& lqp) const;
