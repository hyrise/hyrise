...
std::optional<SubqueryToJoinRule::PredicateNodeInfo> SubqueryToJoinRule::is_predicate_node_join_candidate(
  ...
  if (const auto in_expression = std::dynamic_pointer_cast<InExpression>(predicate_node.predicate())) {
    ...
    if (std::dynamic_pointer_cast<LQPSubqueryExpression>(binary_predicate->left_operand())) {
      result.subquery = std::static_pointer_cast<LQPSubqueryExpression>(binary_predicate->left_operand()->deep_copy());
    ...
    } else if (std::dynamic_pointer_cast<LQPSubqueryExpression>(binary_predicate->right_operand())) {
      result.subquery = std::static_pointer_cast<LQPSubqueryExpression>(binary_predicate->right_operand()->deep_copy());
      result.join_predicate = std::make_shared<BinaryPredicateExpression>(
