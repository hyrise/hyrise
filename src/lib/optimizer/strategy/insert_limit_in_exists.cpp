...
void InsertLimitInExistsRule::apply_to(const std::shared_ptr<AbstractLQPNode>& node, const AbstractCostEstimator& cost_estimator,
    ...
    for (const auto& expression : sub_node->node_expressions) {
      ...
      visit_expression(expression, [&](const auto& sub_expression) {
        ...
        if (const auto subquery_expression = std::dynamic_pointer_cast<LQPSubqueryExpression>(sub_expression)) {
          apply_to(subquery_expression->lqp, cost_estimator, context);
        }
