...
bool PredicatePlacementRule::_is_expensive_predicate(const std::shared_ptr<AbstractExpression>& predicate) {
   ...
   */
  auto predicate_contains_correlated_sub_query = false;
  visit_expression(predicate, [&](const auto& sub_expression) {
    if (const auto sub_query_expression = std::dynamic_pointer_cast<LQPSubQueryExpression>(sub_expression);
        sub_query_expression && !sub_query_expression->arguments.empty()) {
      predicate_contains_correlated_sub_query = true;
      return ExpressionVisitation::DoNotVisitArguments;
    } else {
      return ExpressionVisitation::VisitArguments;
    }
  });
  return predicate_contains_correlated_sub_query;
}
