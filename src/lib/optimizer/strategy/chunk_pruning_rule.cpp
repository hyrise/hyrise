...
std::set<ChunkID> ChunkPruningRule::_calculate_exclude_list(const std::vector<std::shared_ptr<ChunkStatistics>>& stats,
  ...
  for (uint32_t i = 0; i < stats.size(); ++i) {
    ...
    if (stats[i]->can_prune(predicate->column_reference().original_column_id(),
                            boost::get<AllTypeVariant>(predicate->value()), predicate->predicate_condition())) {
