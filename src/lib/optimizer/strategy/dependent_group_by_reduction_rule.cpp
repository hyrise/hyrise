 ...
 */
bool reduce_group_by_columns_for_constraint(const ExpressionsConstraintDefinition& constraint,
                                            AggregateNode& aggregate_node) {

  // Collect non-nullable group-by columns
  ExpressionUnorderedSet group_by_columns;
  group_by_columns.reserve(aggregate_node.aggregate_expressions_begin_idx + 1);
  for (auto expression_idx = size_t{0}; expression_idx < aggregate_node.aggregate_expressions_begin_idx;
    ...
    group_by_columns.insert(expression);
  }

  auto group_by_list_changed = false;
  const auto& constraint_columns = constraint.column_expressions;

  // Intersect primary key/unique columns and group-by columns.
...
void DependentGroupByReductionRule::apply_to(const std::shared_ptr<AbstractLQPNode>& lqp) const {
  ...
  visit_lqp(lqp, [&](const auto& node) {
    ...
    if(input_constraints->empty()) return;
    ...
    auto input_constraints_sorted = std::vector{input_constraints->cbegin(), input_constraints->cend()};
    std::sort(input_constraints_sorted.cbegin(), input_constraints_sorted.cend(),
              [](const ExpressionsConstraintDefinition& left, const ExpressionsConstraintDefinition& right) {
    ...
    for(size_t constraint_idx{0}; constraint_idx < input_constraints_sorted.size(); ++constraint_idx) {
      ...
      group_by_list_changed |= reduce_group_by_columns_for_constraint(constraint, aggregate_node);
