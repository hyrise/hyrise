 ...
 * an example, in TPC-H Q10, the aggregate on customer includes both c_custkey as well as c_name (amonst others) in
 ...
 * This rule identifies cases where functionally dependent columns are given as group-by columns while the FD's
 * determinant column(s) is a/are also group-by column(s). It then replaces that column (e.g., c_name) with a dummy
 * aggregate function ANY(c_name). This reduces the grouping cost in the aggregate operator.
 ...
 * ANY() selects "any" value from the list of values per group (since we group by the FD's determinat column(s) in this
 * case, the group is ensured to be of size one). This rule implements choke point 1.4 of "TPC-H Analyzed: Hidden
 * Messages and Lessons Learned from an Influential Benchmark" (Boncz et al.). Due to the current lack of foreign
 * key suppport, not all queries that are listed in the paper can be optimized.
 *
