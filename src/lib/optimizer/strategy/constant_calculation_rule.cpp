...
void ConstantCalculationRule::apply_to(const std::shared_ptr<AbstractLQPNode>& node,
  ...
  if (node->type == LQPNodeType::Aggregate) return _apply_to_inputs(node, cost_estimator, context);
