...
void reorder_predicates(const std::vector<std::shared_ptr<AbstractLQPNode>>& predicates,
  ...
  for (const auto& predicate : predicates) {
    ...
    const auto penalty = predicate->type == LQPNodeType::Join && !std::dynamic_pointer_cast<JoinNode>(predicate)->is_semi_reduction() ? PredicateReorderingRule::JOIN_PENALTY : Cost{1};
