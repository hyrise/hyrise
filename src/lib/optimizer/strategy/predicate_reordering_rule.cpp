...
void PredicateReorderingRule::_reorder_predicates(
  ...
  //    12) min #out + sqrt(cost - #out)
  //    13) min #out * sqrt(cost - #out)
  //    14) min #out * (cost - #out)
  ...
  for (const auto& predicate : predicates) {
    ...
    const auto benefit = output_cardinality + (cost - output_cardinality) * (predicate->type == LQPNodeType::Join ? 1.25 : 1);
  ...
  const auto nodes_and_cardinalities_count = nodes_and_cardinalities.size() - 1;
  for (auto predicate_index = size_t{0}; predicate_index < nodes_and_cardinalities_count; ++predicate_index) {
