...
void rewrite_to_join(const std::shared_ptr<AbstractLQPNode>& node, const std::shared_ptr<AbstractExpression>& left_value, const std::vector<std::shared_ptr<AbstractExpression>>& elements, DataType data_type) {
  // TODO test NULL
  ...
  const auto join_predicate = std::make_shared<BinaryPredicateExpression>(PredicateCondition::Equals, left_value, list_column);
...
void rewrite_to_disjunction(const std::shared_ptr<AbstractLQPNode>& node, const std::shared_ptr<AbstractExpression>& left_value, const std::vector<std::shared_ptr<AbstractExpression>>& elements, DataType data_type) {
...
void InExpressionToJoinRule::apply_to(const std::shared_ptr<AbstractLQPNode>& node) const {  // TODO rename to top_node
  ...
  visit_lqp(node, [&](const auto& sub_node) {
    ...
    if (common_data_type && forced_algorithm == Algorithm::Join) {  // TODO
