  ...
  visit_lqp(root, [&](const auto& node) {
    ...
    for (const auto& join_predicate : join_node->join_predicates()) {
      ...
      const auto reduce_if_beneficial = [&](const auto side_of_join) {
        ...
        const auto& right_predicate = expression_evaluable_on_lqp(predicate_expression->left_operand(), *right_input)
                                          ? predicate_expression->left_operand()
                                          : predicate_expression->right_operand();
        DebugAssert(!expression_evaluable_on_lqp(right_predicate, *join_node->input(side_of_join)),
                    "Expected predicate to be uniquely evaluable on one side of the join");
