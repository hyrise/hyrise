#include "data_induced_parameter_rule.hpp"
...
#include "expression/expression_functional.cpp"

namespace hyrise {

std::string DataInducedParameterRule::name() const {
  static const auto name = std::string{"DataInducedParameterRule"};
  return name;
}

void DataInducedParameterRule::_apply_to_plan_without_subqueries(const std::shared_ptr<AbstractLQPNode>& lqp_root) const {
  ...
  visit_lqp(lqp_root, [&lqp_root, &estimator, &opposite_side, this, &semi_join_reductions](const auto& node) {
    ...
    for (const auto& join_predicate : join_node->join_predicates()) {
      ...
      const auto reduce_if_beneficial = [&](const auto side_of_join) {
        ...
        auto tst = join_node->join_predicates();

        // TODO CHANGE HERE
        /**
         *  TODO Planned changes:
         *  1. Get min and max statistics of join attribute using sub queries (can this also be other attributes?)
         *      - how to get the join attribute? join_node->join_predicates()[0]->arguments[0] ?
         *      - how does subquery code snippet below work
         *  2. Build Between Predicate
                - not touched yet
         *  3. Insert Between Predicate into LQP
         */


        // get min and max of join attribute by building two queries
/*          Get input tables with join_node->right_input() or join_node->left_input()
        ...
        //         const auto min_c_y = AggregateNode::make(expression_vector(), expression_vector(min_(c_y), max_(c_y)), subquery);
*/
