  ...
  switch (begin->type) {
    case LQPNodeType::Predicate: {
      ...
      const auto expression =
          subsequent_expression ? expression_functional::and_(predicate, *subsequent_expression) : predicate;
    ...
    case LQPNodeType::Union: {
      ...
      if (left_input_expression && right_input_expression) {
        ...
        if (begin->output_count() == 1) {
          // todo(jj): Add comment
          expression =
              subsequent_expression ? expression_functional::and_(expression, *subsequent_expression) : expression;
        }
