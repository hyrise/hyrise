...
/**
 * This optimizer rule finds chains of adjacent PredicateNodes and sorts them based on the expected cardinality. By
 * that, predicates with a low cost are executed first to (hopefully) reduce the size of intermediate results. We also
 * treat single-predicate semi- and anti-joins as predicates since they act as a filter for the input relation and can
 * be executed efficiently. However, we weigh their cost a bit higher to account for their execution overhead.
 *
 * Note:
