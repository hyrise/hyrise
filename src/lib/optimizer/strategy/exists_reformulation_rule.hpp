...
// Does not cover - cases where the sub_query is not correlated (because we cannot use a semi join without a predicate)
...
//                - cases where the sub_query uses multiple external parameters, or uses one twice
//                    (because that makes things more complicated; it is sometimes possible though)
//                - other complex sub_querys (because we have not thought about all eventualities yet)
