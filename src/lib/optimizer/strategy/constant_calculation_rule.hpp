...
class ConstantCalculationRule : public AbstractRule {
 ...
 private:
  ...
  std::shared_ptr<LQPExpression> _replace_column_reference_in_expression(const std::shared_ptr<LQPExpression>& expression, const std::map<LQPColumnReference, AllTypeVariant>& column_reference_to_value_map);
