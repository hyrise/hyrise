...
class PredicatePushdownRule : public AbstractRule {
 ...
 protected:
  bool _predicate_value_demotable(const PredicateNodeSPtr& predicate_node,
                              const AbstractLQPNodeSPtr& node) const;
  bool _contained_in_left_subtree(const AbstractLQPNodeSPtr& node, const LQPColumnReference& column) const;
  bool _contained_in_right_subtree(const AbstractLQPNodeSPtr& node,
                                   const LQPColumnReference& column) const;
  bool _contained_in_node(const AbstractLQPNodeSPtr& node, const LQPColumnReference& column) const;
};
}  // namespace opossum
