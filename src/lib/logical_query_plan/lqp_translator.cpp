...
std::shared_ptr<AbstractOperator> LQPTranslator::_translate_predicate_node_to_index_scan(
  ...
  Assert(indexed_chunks.size() <= (chunk_count - pruned_chunk_ids.size()), "More chunks indexed than expected.");
  if (indexed_chunks.size() == (chunk_count - pruned_chunk_ids.size())) {
    // All segments indexed, no reason for Union.
    return index_scan;
  }
