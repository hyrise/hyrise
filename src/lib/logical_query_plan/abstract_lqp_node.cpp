...
std::optional<std::pair<std::shared_ptr<const AbstractLQPNode>, std::shared_ptr<const AbstractLQPNode>>> AbstractLQPNode::deep_equals(const std::shared_ptr<const AbstractLQPNode>& rhs) const {
  return _deep_equals_impl(shared_from_this(), rhs);
}

std::optional<std::pair<std::shared_ptr<const AbstractLQPNode>, std::shared_ptr<const AbstractLQPNode>>> AbstractLQPNode::_deep_equals_impl(const std::shared_ptr<const AbstractLQPNode>& lhs, const std::shared_ptr<const AbstractLQPNode>& rhs) {
  ...
  const auto left_child_mismatch = _deep_equals_impl(lhs->left_child(), rhs->left_child());
  if (left_child_mismatch) return left_child_mismatch;

  const auto right_child_mismatch = _deep_equals_impl(lhs->right_child(), rhs->right_child());
  if (right_child_mismatch) return right_child_mismatch;

  return std::nullopt;
}

bool AbstractLQPNode::_equals(const AbstractLQPNode&lqp_left, const std::vector<std::shared_ptr<LQPExpression>>& expressions_left,
  ...
  for (size_t expression_idx = 0; expression_idx < expressions_left.size(); ++expression_idx) {
    if (!_equals(lqp_left, expressions_left[expression_idx], lqp_right, expressions_right[expression_idx])) return false;
  }

  return true;
}

bool AbstractLQPNode::_equals(const AbstractLQPNode&lqp_left, const std::shared_ptr<const LQPExpression>& expression_left,
  ...
  for (size_t arg_idx = 0; arg_idx < expression_left->aggregate_function_arguments().size(); ++arg_idx) {
    if (!_equals(lqp_left, expression_left->aggregate_function_arguments()[arg_idx], lqp_right, expression_right->aggregate_function_arguments()[arg_idx])) return false;
  }

  if (!_equals(lqp_left, expression_left->left_child(), lqp_right, expression_right->left_child())) return false;
  if (!_equals(lqp_right, expression_left->right_child(), lqp_right, expression_right->right_child())) return false;

  return true;
}

bool AbstractLQPNode::_equals(const AbstractLQPNode&lqp_left, const std::vector<LQPColumnReference>& column_references_left,
  ...
  for (size_t column_reference_idx = 0; column_reference_idx < column_references_left.size(); ++column_reference_idx) {
    if (!_equals(lqp_left, column_references_left[column_reference_idx], lqp_right, column_references_right[column_reference_idx])) return false;
  }
  return true;
}
...
bool AbstractLQPNode::_equals(const AbstractLQPNode&lqp_left, const LQPColumnReference& column_reference_left,
  ...
  const auto column_reference_right_adapted_to_left = AbstractLQPNode::adapt_column_reference_to_different_lqp(column_reference_left, mutable_lqp_left.shared_from_this(), mutable_lqp_right.shared_from_this());
