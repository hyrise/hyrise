...
std::shared_ptr<AbstractLQPNode> SortNode::_deep_copy_impl(const std::shared_ptr<AbstractLQPNode>& copied_left_child,
  ...
  for (const auto& order_by_definition : _order_by_definitions) {
    const auto column_reference = adapt_column_reference_to_different_lqp(
    order_by_definition.column_reference, left_child(), copied_left_child);
