...
DropIndexNode::DropIndexNode(const std::string& init_index_name, const bool init_if_exists, const std::string& init_table_name)
    : AbstractNonQueryNode(LQPNodeType::DropIndex), index_name(init_index_name), if_exists(init_if_exists), table_name(init_table_name) {}
...
std::shared_ptr<AbstractLQPNode> DropIndexNode::_on_shallow_copy(LQPNodeMapping& node_mapping) const {
  return DropIndexNode::make(index_name, if_exists, table_name);
}
