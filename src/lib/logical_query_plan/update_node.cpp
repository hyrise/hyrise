...
UpdateNode::UpdateNode(const std::string& table_name,
    ...
    : AbstractLQPNode(LQPNodeType::Update), table_name(table_name), update_column_expressions(update_column_expressions) {}
...
std::shared_ptr<AbstractLQPNode> UpdateNode::_shallow_copy_impl(LQPNodeMapping & node_mapping) const {
  return UpdateNode::make(table_name, expressions_copy_and_adapt_to_different_lqp(update_column_expressions, node_mapping));
}
...
bool UpdateNode::_shallow_equals_impl(const AbstractLQPNode& rhs, const LQPNodeMapping & node_mapping) const {
  ...
  return table_name == update_node_rhs.table_name && expressions_equal_to_expressions_in_different_lqp(update_column_expressions, update_node_rhs.update_column_expressions, node_mapping);
}
