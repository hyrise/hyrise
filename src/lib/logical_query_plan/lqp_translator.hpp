...
class LQPTranslator {
 ...
 private:
  ...
  std::vector<OperatorJoinPredicate> _create_set_predicates(const std::shared_ptr<AbstractLQPNode>& node, const std::vector<std::shared_ptr<AbstractExpression>>& join_expressions) const;
