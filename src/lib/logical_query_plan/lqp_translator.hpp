...
class LQPTranslator {
 ...
 private:
  ...
  std::shared_ptr<AbstractOperator> _translate_build_node(const std::shared_ptr<AbstractLQPNode>& node) const;
  std::shared_ptr<AbstractOperator> _translate_probe_node(const std::shared_ptr<AbstractLQPNode>& node) const;
