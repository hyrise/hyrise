...
class LQPExpression : public AbstractExpression<LQPExpression> {
 public:
  static LQPExpressionSPtr create_column(const LQPColumnReference& column_reference,
                                                      const std::optional<std::string>& alias = std::nullopt);

  static std::vector<LQPExpressionSPtr> create_columns(
   ...
   */
  static LQPExpressionSPtr create_subselect(AbstractLQPNodeSPtr root_node = nullptr,
                                                         const std::optional<std::string>& alias = std::nullopt);
  ...
  AbstractLQPNodeSPtr subselect_node() const;
 ...
 protected:
  void _deep_copy_impl(const LQPExpressionSPtr& copy) const override;

 private:
  std::optional<LQPColumnReference> _column_reference;
  AbstractLQPNodeSPtr _subselect_node;
};
