...
class LQPExpression : public AbstractExpression<LQPExpression> {
 public:
  static std::shared_ptr<LQPExpression> create_column(const LQPColumnReference& column_origin,
                                                      const std::optional<std::string>& alias = std::nullopt);

  static std::vector<std::shared_ptr<LQPExpression>> create_columns(
      const std::vector<LQPColumnReference>& column_origins,
  ...
  void set_column_origin(const LQPColumnReference& column_origin);
 ...
 private:
  std::optional<LQPColumnReference> _column_origin;
};
}  // namespace opossum
