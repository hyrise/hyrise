...
AlterDropColumnNode::AlterDropColumnNode(const std::string& init_table_name, const std::string& init_column_name)
    : AbstractNonQueryNode(LQPNodeType::AlterDropColumn), table_name(init_table_name), column_name(init_column_name) {}
...
bool AlterDropColumnNode::_on_shallow_equals(const AbstractLQPNode& rhs, const LQPNodeMapping& node_mapping) const {
  ...
  return table_name == alter_drop_column_node.table_name && column_name == alter_drop_column_node.column_name;
}
