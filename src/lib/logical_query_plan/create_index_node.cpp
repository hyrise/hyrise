...
CreateIndexNode::CreateIndexNode(const std::string& init_index_name, const bool init_if_not_exists, std::shared_ptr<std::vector<ColumnID>> init_column_ids)
    : AbstractNonQueryNode(LQPNodeType::CreateIndex), index_name(init_index_name),
      if_not_exists(init_if_not_exists), column_ids(init_column_ids) {}
...
std::shared_ptr<AbstractLQPNode> CreateIndexNode::_on_shallow_copy(LQPNodeMapping& node_mapping) const {
  return CreateIndexNode::make(index_name, if_not_exists, column_ids, left_input());
}
