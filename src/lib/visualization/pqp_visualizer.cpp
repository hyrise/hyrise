  ...
  switch (op->type()) {
    case OperatorType::Projection: {
      ...
      for (const auto& column_expression : projection->expressions) {
        _visualize_sub_querys(op, column_expression, visualized_ops);
      }
    } break;

    case OperatorType::TableScan: {
      const auto table_scan = std::dynamic_pointer_cast<const TableScan>(op);
      _visualize_sub_querys(op, table_scan->predicate(), visualized_ops);
    } break;

    case OperatorType::Limit: {
      const auto limit = std::dynamic_pointer_cast<const Limit>(op);
      _visualize_sub_querys(op, limit->row_count_expression(), visualized_ops);
    } break;
...
void PQPVisualizer::_visualize_sub_querys(const std::shared_ptr<const AbstractOperator>& op,
                                          const std::shared_ptr<AbstractExpression>& expression,
                                          std::unordered_set<std::shared_ptr<const AbstractOperator>>& visualized_ops) {
  visit_expression(expression, [&](const auto& sub_expression) {
    const auto pqp_sub_query_expression = std::dynamic_pointer_cast<PQPSubQueryExpression>(sub_expression);
    if (!pqp_sub_query_expression) return ExpressionVisitation::VisitArguments;

    _build_subtree(pqp_sub_query_expression->pqp, visualized_ops);

    auto edge_info = _default_edge;
    auto correlated_str = std::string(pqp_sub_query_expression->is_correlated() ? "correlated" : "uncorrelated");
    edge_info.label = correlated_str + " subquery";
    edge_info.style = "dashed";
    _add_edge(pqp_sub_query_expression->pqp, op, edge_info);
