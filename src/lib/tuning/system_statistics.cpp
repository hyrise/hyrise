...
const std::vector<SystemStatistics::SQLQueryCacheEntry>& SystemStatistics::recent_queries() const {
  ...
  auto gdfs_cache_ptr =
      dynamic_cast<const GDFSCache<std::string, std::shared_ptr<SQLQueryPlan>>*>(&query_plan_cache.cache());
  ...
  for (; cache_iterator != cache_end; ++cache_iterator) {
    const GDFSCache<std::string, std::shared_ptr<SQLQueryPlan>>::GDFSCacheEntry& query_plan = *cache_iterator;
