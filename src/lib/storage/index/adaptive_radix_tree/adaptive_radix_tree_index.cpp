...
AdaptiveRadixTreeIndex::AdaptiveRadixTreeIndex(
    ...
    : AbstractRangeIndex{get_index_type_of<AdaptiveRadixTreeIndex>()},
...
AbstractRangeIndex::Iterator AdaptiveRadixTreeIndex::_lower_bound(const std::vector<AllTypeVariant>& values) const {
...
AbstractRangeIndex::Iterator AdaptiveRadixTreeIndex::_upper_bound(const std::vector<AllTypeVariant>& values) const {
  Assert((values.size() == 1), "Adaptive Radix Tree Index expects exactly one input value");
  // the caller is responsible for not passing a NULL value
...
AbstractRangeIndex::Iterator AdaptiveRadixTreeIndex::_cbegin() const { return _chunk_offsets.cbegin(); }

AbstractRangeIndex::Iterator AdaptiveRadixTreeIndex::_cend() const { return _chunk_offsets.cend(); }
...
std::shared_ptr<ARTNode> AdaptiveRadixTreeIndex::_bulk_insert(
    ...
    AbstractRangeIndex::Iterator& it) {
