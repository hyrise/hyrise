...
VariableLengthKey::VariableLengthKey(CompositeKeyLength bytes_per_key)
    : _owned_data(std::make_unique<VariableLengthKeyWord[]>(bytes_per_key)),
      _impl(_owned_data.get(), bytes_per_key) {}  // NOLINT

VariableLengthKey::VariableLengthKey(const VariableLengthKeyBase& other)
    : _owned_data(std::make_unique<VariableLengthKeyWord[]>(other._size)),
      _impl(_owned_data.get(), other._size) {  // NOLINT
