...
size_t GroupKeyIndex2::estimate_memory_consumption(ChunkOffset row_count, ChunkOffset distinct_count,
                                                  uint32_t value_bytes) {
  return row_count * sizeof(ChunkOffset) + distinct_count * sizeof(std::size_t);
}

[[ noreturn ]] GroupKeyIndex2::GroupKeyIndex2(const std::vector<std::shared_ptr<const BaseSegment>>& segments_to_index) : BaseIndex{get_index_type_of<GroupKeyIndex>()}, _indexed_segments(std::dynamic_pointer_cast<const BaseDictionarySegment>(segments_to_index[0])) {
...
GroupKeyIndex2::GroupKeyIndex2(const std::vector<std::shared_ptr<const BaseSegment>>& segments_to_index, const ChunkID chunk_id)
  ...
  Assert(static_cast<bool>(_indexed_segments), "GroupKeyIndex2 only works with dictionary segments_to_index.");
  Assert((segments_to_index.size() == 1), "GroupKeyIndex2 only works with a single segment.");
  ...
  _index_postings_extended = std::vector<RowID>(_indexed_segments->size());
  ...
  resolve_compressed_vector_type(*_indexed_segments->attribute_vector(), [&](auto& attribute_vector) {
    ...
    for (; value_id_it != attribute_vector.cend(); ++value_id_it, ++position) {
      ...
      _index_postings_extended[index_offset_copy[value_id]] = RowID{chunk_id, position};
...
GroupKeyIndex2::Iterator GroupKeyIndex2::_lower_bound(const std::vector<AllTypeVariant>& values) const {
...
GroupKeyIndex2::Iterator GroupKeyIndex2::_upper_bound(const std::vector<AllTypeVariant>& values) const {
  DebugAssert((values.size() == 1), "Group Key Index expects only one input value");
...
const RowID* GroupKeyIndex2::lower_bound_cpy(const std::vector<AllTypeVariant>& values) const {
  DebugAssert((values.size() == 1), "Group Key Index expects only one input value");

  ValueID value_id = _indexed_segments->lower_bound(*values.begin());

  return _get_postings_iterator_extended_at(value_id);
}

const RowID* GroupKeyIndex2::upper_bound_cpy(const std::vector<AllTypeVariant>& values) const {
  DebugAssert((values.size() == 1), "Group Key Index expects only one input value");
...
GroupKeyIndex2::Iterator GroupKeyIndex2::_cbegin() const { return _index_postings.cbegin(); }

GroupKeyIndex2::Iterator GroupKeyIndex2::_cend() const { return _index_postings.cend(); }
...
/**
   ...
   */
GroupKeyIndex2::Iterator GroupKeyIndex2::_get_postings_iterator_at(ValueID value_id) const {
...
const RowID* GroupKeyIndex2::_get_postings_iterator_extended_at(ValueID value_id) const {
  if (value_id == INVALID_VALUE_ID) return &(*_index_postings_extended.cend());

  // get the start position in the position-vector, ie the offset, by looking up the index_offset at value_id
  auto start_pos = _index_offsets[value_id];

  // get an iterator pointing to start_pos
  auto iter = _index_postings_extended.cbegin();
  std::advance(iter, start_pos);
  return &(*iter);
}

std::vector<std::shared_ptr<const BaseSegment>> GroupKeyIndex2::_get_indexed_segments() const {
  return {_indexed_segments};
}

size_t GroupKeyIndex2::_memory_consumption() const {
