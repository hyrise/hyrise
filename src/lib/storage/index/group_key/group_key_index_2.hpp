 ...
 */
class GroupKeyIndex2 : public BaseIndex {
 ...
 public:
  ...
  GroupKeyIndex2() = delete;

  GroupKeyIndex2(const GroupKeyIndex2&) = delete;
  GroupKeyIndex2& operator=(const GroupKeyIndex2&) = delete;
  ...
  const RowID* lower_bound_cpy(const std::vector<AllTypeVariant>& values) const;

  const RowID* upper_bound_cpy(const std::vector<AllTypeVariant>& values) const;
 ...
 private:
  ...
  const RowID* _get_postings_iterator_extended_at(ValueID value_id) const;
 ...
 private:
  ...
  std::vector<RowID> _index_postings_extended;
};
}  // namespace opossum
