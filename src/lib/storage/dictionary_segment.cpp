...
template <typename T>
DictionarySegment<T>::DictionarySegment(pmr_vector<T>&& dictionary,
...
template <typename T>
const pmr_vector<T>& DictionarySegment<T>::dictionary() const {
...
std::shared_ptr<AbstractSegment> DictionarySegment<T>::copy_using_allocator(
  ...
  auto copy = std::make_shared<DictionarySegment<T>>(pmr_vector<T>(_dictionary, alloc), _attribute_vector->copy_using_allocator(alloc));
...
size_t DictionarySegment<T>::memory_usage(const MemoryUsageCalculationMode mode) const {
  ...
  return common_elements_size + _dictionary.capacity() * sizeof(typename decltype(_dictionary)::value_type);
}
...
template <typename T>
const std::unique_ptr<const BaseCompressedVector>& DictionarySegment<T>::attribute_vector() const {
