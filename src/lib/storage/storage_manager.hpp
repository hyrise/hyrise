...
class StorageManager : private Noncopyable {
 public:
  ...
  void add_view(const std::string& name, std::shared_ptr<AbstractASTNode> view);
  ...
  std::shared_ptr<AbstractASTNode> get_view(const std::string& name) const;
 ...
 protected:
  ...
  std::map<std::string, std::shared_ptr<const AbstractASTNode>> _views;
};
}  // namespace opossum
