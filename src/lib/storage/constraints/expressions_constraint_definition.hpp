...
struct ExpressionsConstraintDefinition final {
  ExpressionsConstraintDefinition(std::vector<std::shared_ptr<AbstractExpression>> init_column_expressions,
                                  const IsPrimaryKey is_primary_key)
      : column_expressions(std::move(init_column_expressions)), is_primary_key(is_primary_key) {
    Assert(std::unique(column_expressions.begin(), column_expressions.end()) == column_expressions.end(),
           "Duplicate column expressions unexpected.");
  }
