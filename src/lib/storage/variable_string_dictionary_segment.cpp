...
requires(std::is_same_v<T, pmr_string>) const pmr_vector<char>& VariableStringDictionarySegment<T>::dictionary() const {
  return _dictionary;
}

template <typename T>
requires(std::is_same_v<T, pmr_string>)
    VariableStringVector VariableStringDictionarySegment<T>::variable_string_dictionary()
const {
...
requires(std::is_same_v<T, pmr_string>) const
    pmr_vector<uint32_t>& VariableStringDictionarySegment<T>::offset_vector() const {
