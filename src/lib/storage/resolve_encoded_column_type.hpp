...
/**
 * @brief Mapping of encoding types to segment types
 *
  ...
  hana::fold(encoded_segment_for_type, false, [&](auto match_found, auto encoded_segment_pair) {
    ...
    if (!match_found && (encoding_type == segment.encoding_type())) {
      ...
      if constexpr(hana::value(data_type_supported)) {
        using ColumnTemplateType = typename decltype(segment_template_c)::type;
        using ColumnType = typename ColumnTemplateType::template _template<CxlumnDataType>;
        functor(static_cast<const ColumnType&>(segment));
      }
