...
class LZ4SegmentIterable : public PointAccessibleSegmentIterable<LZ4SegmentIterable<T>> {
 public:
  ...
  void _on_with_iterators(const Functor& functor) const {
    ...
    _segment.access_counter.get(SegmentAccessCounter::AccessType::Sequential) += decompressed_segment.size();
  ...
  void _on_with_iterators(const std::shared_ptr<const PosList>& position_filter, const Functor& functor) const {
    ...
    _segment.access_counter.get(SegmentAccessCounter::access_type(*position_filter)) += position_filter_size;
