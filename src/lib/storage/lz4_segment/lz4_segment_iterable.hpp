 ...
 public:
  ...
  void _on_with_iterators(const Functor& functor) const {
    ...
    if (_segment.null_values()) {
      ...
      auto end = Iterator<ValueIterator>{decompressed_segment.cend(), _segment.null_values()->cend(), static_cast<ChunkOffset>(decompressed_segment.size())};
      ...
      auto end = Iterator<ValueIterator>{decompressed_segment.cend(), std::nullopt, static_cast<ChunkOffset>(decompressed_segment.size())};
 ...
 private:
  ...
  class Iterator : public BaseSegmentIterator<Iterator<ValueIterator>, SegmentPosition<T>> {
   ...
   public:
    ...
    explicit Iterator(ValueIterator data_it, std::optional<NullValueIterator> null_value_it, ChunkOffset chunk_offset = 0u)
        : _chunk_offset{chunk_offset}, _data_it{data_it}, _null_value_it{null_value_it} {}
