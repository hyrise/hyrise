...
ReferenceColumn::ReferenceColumn(const TableCSPtr referenced_table,
                                 const ColumnID referenced_column_id, const PosListCSPtr pos)
...
const PosListCSPtr ReferenceColumn::pos_list() const { return _pos_list; }
const TableCSPtr ReferenceColumn::referenced_table() const { return _referenced_table; }
...
void ReferenceColumn::visit(ColumnVisitable& visitable, ColumnVisitableContextSPtr context) const {
  visitable.handle_column(*this, std::move(context));
}

BaseColumnSPtr ReferenceColumn::copy_using_allocator(const PolymorphicAllocator<size_t>& alloc) const {
