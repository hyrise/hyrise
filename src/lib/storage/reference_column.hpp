...
class reference_column : public base_column {
 ...
 public:
  reference_column(const std::shared_ptr<table> referenced_table, const size_t referenced_column_id,
  ...
  virtual void append(const all_type_variant &) { throw std::logic_error("reference_column is immutable"); }

  virtual size_t size() const { return _pos_list->size(); }

  const std::shared_ptr<pos_list> get_pos_list() const { return _pos_list; }
};

// TODO(Anyone): Dokumentieren, dass nicht alle Chunks einer Tabelle gleich groß sind.
// Wenn man einen Union aus zwei Tabellen mit jeweils einem Chunk macht, entstehenden zwei reference_columns, die auf
