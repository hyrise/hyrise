...
void expression_bind_placeholders_impl(
  ...
  visit_expression(expression, [&](auto& sub_expression) {
    ...
    } else if (const auto sub_query_expression = std::dynamic_pointer_cast<LQPSubQueryExpression>(sub_expression)) {
      lqp_bind_placeholders_impl(sub_query_expression->lqp, parameters, visited_nodes);
    }
