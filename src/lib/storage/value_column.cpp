...
template <typename T>
ValueColumn<T>::ValueColumn(const PolymorphicAllocator<T>& alloc, bool nullable) : BaseValueColumn(data_type_from_type<T>()), _values(alloc) {
  if (nullable) _null_values = pmr_concurrent_vector<bool>(alloc);
}

template <typename T>
ValueColumn<T>::ValueColumn(pmr_concurrent_vector<T>&& values): BaseValueColumn(data_type_from_type<T>()), _values(std::move(values)) {}
