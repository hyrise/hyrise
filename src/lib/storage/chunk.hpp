...
class Chunk : private Noncopyable {
 public:
  ...
  // Migrates segments, segments[column_ids[i]] is migrated via memory_resources[i]
  // Not thread-safe. Assuming that migration is done before workload execution.
  void migrate(const std::vector<MemoryResource*>& column_memory_resources);

  void migrate(const std::vector<ColumnID>& column_ids, MemoryResource* column_resource, MemoryResource* remainer_resource);
