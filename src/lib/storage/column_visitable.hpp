...
class ColumnVisitable {
 public:
  ...
  virtual void handle_column(const BaseValueColumn& column, ColumnVisitableContextSPtr context) = 0;
  virtual void handle_column(const BaseDictionaryColumn& column, ColumnVisitableContextSPtr context) = 0;
  virtual void handle_column(const ReferenceColumn& column, ColumnVisitableContextSPtr context) = 0;

  /**
   * This method is going to be called for all encoded column classes
   * that do not overload visit() themselves.
   */
  virtual void handle_column(const BaseEncodedColumn& column, ColumnVisitableContextSPtr context) = 0;
};
