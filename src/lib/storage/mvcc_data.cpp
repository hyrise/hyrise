...
MvccData::MvccData(const size_t size, CommitID begin_commit_id) {
  ...
  _begin_cids.resize(size, CopyableAtomic<CommitID>{begin_commit_id});
  _end_cids.resize(size, CopyableAtomic<CommitID>{MAX_COMMIT_ID});
  _tids.resize(size, CopyableAtomic<TransactionID>{INVALID_TRANSACTION_ID});
}
