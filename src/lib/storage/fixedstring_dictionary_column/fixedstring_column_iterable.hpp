...
template <typename T>
class FixedStringColumnIterable : public PointAccessibleColumnIterable<FixedStringColumnIterable<T>> {
 public:
  explicit FixedStringColumnIterable(const FixedStringColumn<T>& column) : _column{column} {}
 ...
 private:
  const FixedStringColumn<T>& _column;
