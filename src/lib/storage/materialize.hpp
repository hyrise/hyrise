...
void materialize_values(const BaseSegment& segment, Container& container) {
  ...
  resolve_segment_type<ContainerValueType>(segment, [&](const auto& resolved_segment) {
    create_iterable_from_segment<ContainerValueType>(resolved_segment).materialize_values(container);
  });
}
...
void materialize_values_and_nulls(const BaseSegment& segment, Container& container) {
  ...
  resolve_segment_type<ContainerValueType>(segment, [&](const auto& resolved_segment) {
    create_iterable_from_segment<ContainerValueType>(resolved_segment).materialize_values_and_nulls(container);
  });
}
...
void materialize_nulls(const BaseSegment& segment, Container& container) {
  resolve_segment_type<SegmentValueType>(segment, [&](const auto& resolved_segment) {
    create_iterable_from_segment<SegmentValueType>(resolved_segment).materialize_nulls(container);
  });
}
