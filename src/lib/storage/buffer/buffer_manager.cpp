...
void BufferManager::_make_resident(const PageID page_id, const Frame::StateVersionType state_before_exclusive) {
  ...
  retry_with_backoff([&, page_id = page_id]() { return _buffer_pool.ensure_free_pages(page_id.byte_count()); });
  _persistence_manager->read_page(page_id, region->get_page(page_id));
  _total_misses.fetch_add(1, std::memory_order_relaxed);

  // increment_metric_counter(_metrics->total_bytes_copied_from_ssd_to_dram, page_id.byte_count());
  return;
}
