...
std::byte* BufferManager::create_mapped_region() {
  ...
  const auto mapped_memory = static_cast<std::byte*>(
      mmap(NULL, BufferManagerConfig::DEFAULT_RESERVED_VIRTUAL_MEMORY, PROT_READ | PROT_WRITE, flags, -1, 0));
...
std::array<std::shared_ptr<VolatileRegion>, NUM_PAGE_SIZE_TYPES> BufferManager::create_volatile_regions(
    const BufferManagerConfig config, std::byte* mapped_region, std::shared_ptr<BufferManagerMetrics> metrics) {
  ...
  const auto approximate_pool_size = std::max(config.dram_buffer_pool_size, config.numa_buffer_pool_size) * 2;
  ...
  for (auto i = size_t{0}; i < NUM_PAGE_SIZE_TYPES; i++) {
    array[i] =
        std::make_shared<VolatileRegion>(magic_enum::enum_value<PageSizeType>(i), approximate_pool_size,
...
void BufferManager::unmap_region(std::byte* region) {
  if (munmap(region, BufferManagerConfig::DEFAULT_RESERVED_VIRTUAL_MEMORY) < 0) {
    const auto error = errno;
...
void BufferManager::make_resident(const PageID page_id, const AccessIntent access_intent,
  ...
  if (is_evicted) {
    ...
    retry_with_backoff([&]() {
      ...
      if (bypass_numa) {
        ...
        if (!_buffer_pools[0]->ensure_free_pages(page_id.size_type())) {
          return false;
        }
        region->mbind_to_numa_node(page_id, _buffer_pools[0]->node_id);
        increment_metric_counter(_metrics->total_bytes_copied_from_ssd_to_dram, page_id.num_bytes());
      } else {
        // Case 4.2: We bypass load the page into NUMA
        if (!_buffer_pools[1]->ensure_free_pages(page_id.size_type())) {
          return false;
        }
        region->mbind_to_numa_node(page_id, _buffer_pools[1]->node_id);
        increment_metric_counter(_metrics->total_bytes_copied_from_ssd_to_numa, page_id.num_bytes());
      }
      _ssd_region->read_page(page_id, region->get_page(page_id));
      increment_metric_counter(_metrics->total_misses);
      return true;
  ...
  DebugAssert(Frame::node_id(state_before_exclusive) == _buffer_pools[1]->node_id, "Should be on NUMA");
  retry_with_backoff([&]() {
    ...
    if (bypass_dram) {
      ...
      increment_metric_counter(_metrics->total_hits);
      return true;
    } else {
      ...
      _buffer_pools[1]->free_reserved_bytes(page_id.num_bytes());
      region->mbind_to_numa_node(page_id, _buffer_pools[0]->node_id);
      increment_metric_counter(_metrics->total_hits);
      increment_metric_counter(_metrics->total_bytes_copied_from_numa_to_dram, page_id.num_bytes());
      return true;
    }
  });
}
...
BufferManagerConfig BufferManager::config() const {
  return _config;
}
...
void* BufferManager::do_allocate(std::size_t bytes, std::size_t alignment) {
  ...
  auto buffer_pool =
      _buffer_pools[1] && _config.migration_policy.bypass_dram_during_write() ? _buffer_pools[1] : _buffer_pools[0];
