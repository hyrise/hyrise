...
VolatileRegion::VolatileRegion(const PageSizeType size_type, const size_t approximate_size_bytes,
                               std::byte* region_start, std::byte* region_end,
                               std::shared_ptr<BufferManagerMetrics> metrics)
    : _size_type(size_type),
      _region_start(region_start),
      _region_end(region_end),
      _frames(std::min(approximate_size_bytes / bytes_for_size_type(size_type),
                       (region_end - region_start) / bytes_for_size_type(size_type))),
      _free_slots(std::min(approximate_size_bytes / bytes_for_size_type(size_type),
                           (region_end - region_start) / bytes_for_size_type(size_type))),
      _metrics(metrics) {
  ...
  _free_slots.set();
  if constexpr (BufferManagerConfig::ENABLE_MPROTECT) {
...
void VolatileRegion::move_page_to_numa_node(PageID page_id, const NodeID target_memory_node) {
  ...
  _metrics->num_numa_page_movements.fetch_add(1, std::memory_order_relaxed);
