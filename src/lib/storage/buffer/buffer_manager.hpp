 ...
 * This basic implementation was presented in the SIGMOD'23 paper "Virtual-Memory Assisted Buffer Management" by Leis et. al. We further provide a multi-tier mechanism that can
 * offload pages to the SSD or to another NUMA node like CXl-attached memory. We further support explicitly defined variable sized pages to handle differently sized segments. For this, 
 * we employ a Second-Chance FIFO for page replacement
 * 
 * 
 * Usage:
 ...
 * Future Work:
 * The low-level API presented here can be used to implement a variety of buffer management techniques. We see the following extensions to improve the performance:
...
class BufferManager final : public boost::container::pmr::memory_resource, public Noncopyable {
 public:
  // One buffer pool per node. Currently we support a primary execution node called "cpu_node" and secondary "memory_node"
  using BufferPools = std::array<std::shared_ptr<BufferPool>, 2>;

  // Create a buffer manager with a JSON configuration provided by the environment variable HYRISE_BUFFER_MANAGER_CONFIG_JSON_PATH; or with the default configuration
  BufferManager();

  // Create a buffer manager with a given configuration
  ...
  void pin_shared(const PageID page_id, const AccessIntent accessIntent = AccessIntent::Read);
  ...
  BufferManagerConfig config() const;
  ...
  const BufferPools buffer_pools() const;
 ...
 private:
  ...
  BufferPools create_buffer_pools(
