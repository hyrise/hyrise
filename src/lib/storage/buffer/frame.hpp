...
class Frame final : private Noncopyable {
 ...
 private:
  ...
  static constexpr auto DIRTY_MASK       = uint64_t{0x0000800000000000};

  // The NUMA is encoded 7 bits between the state an the the dirty mask.
  static constexpr auto NODE_ID_MASK     = uint64_t{0x00007F0000000000};

  // The state is encoded in the upper 16 bits.
  static constexpr auto STATE_MASK       = uint64_t{0xFFFF000000000000};

  // The version is encoded in the lower 40 bits.
  static constexpr auto VERSION_MASK     = uint64_t{0x000000FFFFFFFFFF};
  ...
  static constexpr uint64_t BIT_WIDTH = std::numeric_limits<StateVersionType>::digits;
...
std::ostream& operator<<(std::ostream& ostream [[clang::lifetimebound]], const Frame& frame);
}  // namespace hyrise
