...
#include "storage/buffer/allocation_observer.hpp"
...
class BufferPoolAllocator {
 public:
  ...
  BufferPoolAllocator(boost::container::pmr::memory_resource* memory_resource,
                      std::shared_ptr<BufferManagerAllocationObserver> observer = nullptr)
  ...
  void register_observer(std::shared_ptr<BufferManagerAllocationObserver> observer) {
  ...
  std::weak_ptr<BufferManagerAllocationObserver> current_observer() const {
    return _observer;
  }

 private:
  boost::container::pmr::memory_resource* _memory_resource;
  std::weak_ptr<BufferManagerAllocationObserver> _observer;
};
