...
void DebugAssertPageAlignment(const std::byte* data) {
  DebugAssert(reinterpret_cast<std::uintptr_t>(data) % 512 == 0,
              "Data buffer is not aligned to 512 byte boundary as required for direct IO: " +
                  std::to_string(reinterpret_cast<std::uintptr_t>(data) % 512));
}

static StorageRegion::Mode get_mode(const std::filesystem::path& file_name) {
  if (std::filesystem::is_directory(file_name)) {
    return StorageRegion::Mode::FILE_PER_PAGE_SIZE;
  }

  if (std::filesystem::is_block_file(file_name)) {
...
int StorageRegion::_open_file_descriptor(const std::filesystem::path& file_name) {
  ...
  // On Linux, we use O_DIRECT to bypass the page cache. This is not supported on macOS, but we use the fcntl(F_NOCACHE)
  // call below. Files are opened in read-write mode and created if they do not exist. O_DSYNC ensures that all file
  // data and metadata is written to the disk before the syscall returns. This eliminates the need for explict fsync
  // calls after each write.
#ifdef __APPLE__
  const auto flags = O_RDWR | O_CREAT | O_DSYNC;
#elif __linux__
  const auto flags = O_RDWR | O_CREAT | O_DSYNC | O_DIRECT;
#endif
  const auto fd = open(file_name.string().c_str(), flags, 0666);
  if (fd < 0) {
...
// In comparison to O_DIRECT on Linux, we need to use fcntl(F_NOCACHE) for direct IO. See comment above.
  ...
  if (fcntl(fd, F_NOCACHE, 1) == -1) {
    const auto error = errno;
    close(fd);
    Fail("Error while setting F_NOCACHE on macOS: " + strerror(error));
  }
#endif
...
std::array<StorageRegion::FileHandle, PAGE_SIZE_TYPES_COUNT> StorageRegion::_open_file_handles_in_directory(
  ...
  auto file_handles = std::array<StorageRegion::FileHandle, PAGE_SIZE_TYPES_COUNT>{};
  ...
  for (auto page_size_type_index = size_t{0}; page_size_type_index < PAGE_SIZE_TYPES_COUNT; ++page_size_type_index) {
    ...
    file_handles[page_size_type_index] = {_open_file_descriptor(file_name), file_name};
...
std::array<StorageRegion::FileHandle, PAGE_SIZE_TYPES_COUNT> StorageRegion::_open_file_handles_block(
  ...
  // ioctl with the BLKGETSIZE64 allows us to get the size of the block device on Linux. It is not supported on macOS.
  auto block_size = uint64_t{0};
#ifdef __APPLE__
  Fail("Block device not supported on macOS");
#elif __linux__
  ...
  for (auto page_size_type_index = uint64_t{0}; page_size_type_index < PAGE_SIZE_TYPES_COUNT; ++page_size_type_index) {
    ...
    file_handles[page_size_type_index] = {fd, path, block_offset};
  }
