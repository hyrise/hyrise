...
class BufferPool final {
 public:
  ...
  std::shared_ptr<BufferPool> get_target_buffer_pool() const;
 ...
 private:
  ...
  BufferPool(const size_t pool_size,
             ...
             const MigrationPolicy migration_policy, const std::shared_ptr<SSDRegion> ssd_region,
             const std::shared_ptr<BufferPool> target_buffer_pool, const NodeID numa_node);
  ...
  const std::shared_ptr<BufferPool> target_buffer_pool;
