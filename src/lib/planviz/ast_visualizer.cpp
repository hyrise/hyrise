...
void ASTVisualizer::_build_graph(const std::vector<std::shared_ptr<AbstractASTNode>>& ast_roots) {
...
void ASTVisualizer::_build_subtree(const std::shared_ptr<AbstractASTNode>& node) {
  if (node->left_child()) {
