...
void Print::print(TableCSPtr table, uint32_t flags, std::ostream& out) {
  auto table_wrapper = std::make_shared<TableWrapper>(table);
  table_wrapper->execute();
  Print(table_wrapper, out, flags).execute();
}

void Print::print(AbstractOperatorCSPtr in, uint32_t flags, std::ostream& out) {
  Print(in, out, flags).execute();
}

TableCSPtr Print::_on_execute() {
...
std::vector<uint16_t> Print::_column_string_widths(uint16_t min, uint16_t max, TableCSPtr t) const {
