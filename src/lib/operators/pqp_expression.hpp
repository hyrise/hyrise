...
class PQPExpression : public AbstractExpression<PQPExpression> {
 public:
   ...
   */
  static std::shared_ptr<PQPExpression> create_subselect(const std::shared_ptr<AbstractOperator>& root_operator,
                                                         const std::optional<std::string>& alias = std::nullopt);
  ...
  void set_subselect_table(const std::shared_ptr<const Table>& table);
