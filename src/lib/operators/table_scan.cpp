...
std::shared_ptr<AbstractOperator> TableScan::_on_recreate(const std::vector<AllParameterVariant>& args,
  ...
  if (is_placeholder(_right_parameter)) {
    ...
    if (index < args.size()) {
      return std::make_shared<TableScan>(recreated_input_left, _left_column_id, _predicate_condition,
                                         args[index]);
    }
  }
  return std::make_shared<TableScan>(recreated_input_left, _left_column_id, _predicate_condition,
