...
typename Results::reference get_or_add_result(ResultIds& result_ids, Results& results, std::pair<AggregateKey, size_t>& key,
                                              const RowID& row_id) {
  if constexpr (std::is_same_v<AggregateKey, EmptyAggregateKey>) {
    ...
    return results[0];
  } else if (key.second == size_t{0} - 1) {
    // Get the result id for the current key or add it to the id map
    auto it = result_ids.find(key.first);
    ...
    key.second = result_id;
    return results[key.second];
  }

  if (key.second >= results.size()) {
    results.resize(key.second + 1);
    results[key.second].row_id = row_id;
  }
...
std::pair<AggregateKey, size_t>& get_aggregate_key([[maybe_unused]] KeysPerChunk<AggregateKey>& keys_per_chunk,
  ...
  if constexpr (!std::is_same_v<AggregateKey, EmptyAggregateKey>) {
    ...
    static std::pair<EmptyAggregateKey, size_t> empty_aggregate_key;
...
void AggregateHash::_aggregate_segment(ChunkID chunk_id, ColumnID column_index, const BaseSegment& base_segment,
  ...
  segment_with_iterators<ColumnDataType>(base_segment, [&](auto it, const auto end) {
    while (it != end) {
      ...
      */
      if (!position.is_null()) {
        ...
        if constexpr (function == AggregateFunction::Count || function == AggregateFunction::Avg || function == AggregateFunction::StandardDeviationSample) {
...
void AggregateHash::_aggregate() {
  ...
  KeysPerChunk<AggregateKey> keys_per_chunk;
  auto most_distinct_values = size_t{0};
    ...
    const auto chunk_count = input_table->chunk_count();
    {
      ...
      for (ChunkID chunk_id{0}; chunk_id < chunk_count; ++chunk_id) {
        ...
        if constexpr (std::is_same_v<AggregateKey, std::vector<AggregateKeyEntry>>) {
          keys_per_chunk.emplace_back(chunk->size(), std::make_pair(AggregateKey(_groupby_column_ids.size()), size_t{0} - 1));
        } else {
          keys_per_chunk.emplace_back(chunk->size(), std::make_pair(AggregateKey{}, size_t{0} - 1));
        }
      }
            ...
            for (ChunkID chunk_id{0}; chunk_id < chunk_count; ++chunk_id) {
              ...
              segment_iterate<ColumnDataType>(*base_segment, [&](const auto& position) {
                ...
                if constexpr (std::is_same_v<AggregateKey, AggregateKeyEntry>) {
                  if (position.is_null()) {
                    keys_per_chunk[chunk_id][chunk_offset].first = 0;
                  } else {
                    keys_per_chunk[chunk_id][chunk_offset].first = to_uint(position.value()) + 1;
                  }
                } else {
                  if (position.is_null()) {
                    keys_per_chunk[chunk_id][chunk_offset].first[group_column_index] = 0;
                  } else {
                    keys_per_chunk[chunk_id][chunk_offset].first[group_column_index] = to_uint(position.value()) + 1;
                  }
                }
                ++chunk_offset;
              });
            ...
            */
            ...
            for (ChunkID chunk_id{0}; chunk_id < chunk_count; ++chunk_id) {
              ...
              segment_iterate<ColumnDataType>(*base_segment, [&](const auto& position) {
                if (position.is_null()) {
                  if constexpr (std::is_same_v<AggregateKey, AggregateKeyEntry>) {
                    keys_per_chunk[chunk_id][chunk_offset].first = 0u;
                  } else {
                    keys_per_chunk[chunk_id][chunk_offset].first[group_column_index] = 0u;
                  }
                } else {
                  auto inserted = id_map.try_emplace(position.value(), id_counter);
                  if constexpr (std::is_same_v<AggregateKey, AggregateKeyEntry>) {
                    keys_per_chunk[chunk_id][chunk_offset].first = inserted.first->second;
                  } else {
                    keys_per_chunk[chunk_id][chunk_offset].first[group_column_index] = inserted.first->second;
                  }
  ...
  */
  ...
  for (ColumnID aggregate_idx{0}; aggregate_idx < _aggregates.size(); ++aggregate_idx) {
    ...
    if (input_column_id == INVALID_COLUMN_ID) {
      ...
      context->results.reserve(most_distinct_values);
