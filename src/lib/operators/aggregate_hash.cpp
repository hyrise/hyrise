...
std::shared_ptr<const Table> AggregateHash::_on_execute() {
  ...
  if (_aggregates.empty()) {
    auto context = std::static_pointer_cast<AggregateResultContext<DistinctColumnType, DistinctAggregateType>>(
        _contexts_per_column[0]);
    auto pos_list = std::make_shared<PosList>(context->results.size());
...
void AggregateHash::_write_groupby_output(const std::shared_ptr<PosList>& pos_list) {
  auto input_table = input_table_left();
  ...
  for (const auto& column_id : _groupby_column_ids) {
    ...
    resolve_data_type(input_table->column_data_type(column_id), [&](const auto typed_value) {
      ...
      for (const auto& row_id : *pos_list) {
        ...
        if (!accessor) {
          // We use a self-created pos_list here, not the one of the reference segment.
          accessor =
              create_segment_accessor<ColumnDataType>(input_table->get_chunk(row_id.chunk_id)->get_segment(column_id), pos_list);
        }
