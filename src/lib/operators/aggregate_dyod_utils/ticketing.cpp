...
RowFormat _create_row_format(const TableColumnDefinitions& column_definitions,
  ...
  const auto data_offset = sizeof(uint64_t) + sizeof(uint64_t);  // hash + null bitmap
    ...
    resolve_data_type(segment->data_type(), [&](auto type) {
      ...
      segment_iterate<ColumnDataType>(*segment, [&](const auto& position) {
        ...
        if constexpr (std::is_same_v<ColumnDataType, pmr_string>) {
          ...
          // Strings longer than the prefix additionally store the full, null-terminated value in the chunk's arena so
          // equality can resolve prefix collisions without re-reading the source segment.
