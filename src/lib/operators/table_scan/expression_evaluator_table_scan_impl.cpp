...
ExpressionEvaluatorTableScanImpl::ExpressionEvaluatorTableScanImpl(
  ...
  _uncorrelated_sub_query_results = ExpressionEvaluator::populate_uncorrelated_sub_query_results_cache({expression});
}
...
std::shared_ptr<PosList> ExpressionEvaluatorTableScanImpl::scan_chunk(ChunkID chunk_id) const {
  return std::make_shared<PosList>(
      ExpressionEvaluator{_in_table, chunk_id, _uncorrelated_sub_query_results}.evaluate_expression_to_pos_list(
          *_expression));
}
