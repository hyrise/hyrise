...
void BaseSingleColumnTableScanImpl::handle_column(const ReferenceColumn& base_column,
                                                  std::shared_ptr<ColumnVisitorContext> base_context) {
  auto context = std::static_pointer_cast<Context>(base_context);
  const ChunkID chunk_id = context->_chunk_id;
  auto& matches_out = context->_matches_out;

  auto chunk_offsets_by_chunk_id = split_pos_list_by_chunk_id(*base_column.pos_list());
  ...
  for (auto& pair : chunk_offsets_by_chunk_id) {
    ...
    const auto chunk = base_column.referenced_table()->get_chunk(referenced_chunk_id);
    auto referenced_column = chunk->get_column(base_column.referenced_column_id());
