...
BaseSingleCxlumnTableScanImpl::BaseSingleCxlumnTableScanImpl(const std::shared_ptr<const Table>& in_table,
                                                             const CxlumnID cxlumn_id,
                                                             const PredicateCondition predicate_condition)
    : BaseTableScanImpl{in_table, cxlumn_id, predicate_condition} {}

std::shared_ptr<PosList> BaseSingleCxlumnTableScanImpl::scan_chunk(ChunkID chunk_id) {
  const auto chunk = _in_table->get_chunk(chunk_id);
  const auto segment = chunk->get_segment(_left_cxlumn_id);
...
void BaseSingleCxlumnTableScanImpl::handle_segment(const ReferenceSegment& segment,
  ...
  for (auto& pair : chunk_offsets_by_chunk_id) {
    ...
    auto referenced_segment = chunk->get_segment(segment.referenced_cxlumn_id());
