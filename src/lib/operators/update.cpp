...
Update::Update(const ObjectID table_to_update_id, const std::shared_ptr<AbstractOperator>& fields_to_update_op,
    ...
    : AbstractReadWriteOperator(OperatorType::Update, fields_to_update_op, update_values_op),
      _table_to_update_id{table_to_update_id} {}
...
std::shared_ptr<const Table> Update::_on_execute(std::shared_ptr<TransactionContext> context) {
  const auto table_to_update = Hyrise::get().storage_manager.get_table(_table_to_update_id);
  ...
  _insert = std::make_shared<Insert>(_table_to_update_id, _right_input);
...
std::shared_ptr<AbstractOperator> Update::_on_deep_copy(
  ...
  return std::make_shared<Update>(_table_to_update_id, copied_left_input, copied_right_input);
}
