...
IndexScanCopy::IndexScanCopy(const std::shared_ptr<const AbstractOperator>& in, const SegmentIndexType index_type,
...
const std::string IndexScanCopy::name() const { return "IndexScanCopy"; }

void IndexScanCopy::set_included_chunk_ids(const std::vector<ChunkID>& chunk_ids) { _included_chunk_ids = chunk_ids; }

std::shared_ptr<const Table> IndexScanCopy::_on_execute() {
...
std::shared_ptr<AbstractOperator> IndexScanCopy::_on_deep_copy(
    const std::shared_ptr<AbstractOperator>& copied_input_left,
    const std::shared_ptr<AbstractOperator>& copied_input_right) const {
  return std::make_shared<IndexScanCopy>(copied_input_left, _index_type, _left_column_ids, _predicate_condition,
                                     _right_values, _right_values2);
}

void IndexScanCopy::_on_set_parameters(const std::unordered_map<ParameterID, AllTypeVariant>& parameters) {}

std::shared_ptr<AbstractTask> IndexScanCopy::_create_job_and_schedule(const ChunkID chunk_id, std::mutex& output_mutex) {
...
void IndexScanCopy::_validate_input() {
  ...
  Assert(_in_table->type() == TableType::Data, "IndexScanCopy only supports persistent tables right now.");
}

PosList IndexScanCopy::_scan_chunk(const ChunkID chunk_id) {
  ...
  if (auto casted = std::dynamic_pointer_cast<GroupKeyIndex2>(index)) {
    const RowID* a = casted->lower_bound_cpy(_right_values);
    const RowID* b = casted->upper_bound_cpy(_right_values2);
    ptrdiff_t size = b - a;
    matches_out.resize(size);
    std::memcpy(&matches_out[0], a, size);
  } else {
