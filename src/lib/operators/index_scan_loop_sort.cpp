#include "index_scan_loop_sort.hpp"
...
IndexScanLoopSort::IndexScanLoopSort(const std::shared_ptr<const AbstractOperator>& in, const SegmentIndexType index_type,
...
const std::string IndexScanLoopSort::name() const { return "IndexScanLoopSort"; }

void IndexScanLoopSort::set_included_chunk_ids(const std::vector<ChunkID>& chunk_ids) { _included_chunk_ids = chunk_ids; }

std::shared_ptr<const Table> IndexScanLoopSort::_on_execute() {
...
std::shared_ptr<AbstractOperator> IndexScanLoopSort::_on_deep_copy(
    const std::shared_ptr<AbstractOperator>& copied_input_left,
    const std::shared_ptr<AbstractOperator>& copied_input_right) const {
  return std::make_shared<IndexScanLoopSort>(copied_input_left, _index_type, _left_column_ids, _predicate_condition,
                                     _right_values, _right_values2);
}

void IndexScanLoopSort::_on_set_parameters(const std::unordered_map<ParameterID, AllTypeVariant>& parameters) {}

std::shared_ptr<AbstractTask> IndexScanLoopSort::_create_job_and_schedule(const ChunkID chunk_id, std::mutex& output_mutex) {
...
void IndexScanLoopSort::_validate_input() {
  ...
  Assert(_in_table->type() == TableType::Data, "IndexScanLoopSort only supports persistent tables right now.");
}

PosList IndexScanLoopSort::_scan_chunk(const ChunkID chunk_id) {
