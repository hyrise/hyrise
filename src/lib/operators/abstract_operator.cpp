...
AbstractOperator::AbstractOperator(const OperatorType type, const AbstractOperatorCSPtr left,
...
TableCSPtr AbstractOperator::get_output() const {
  DebugAssert(
      [&]() {
...
AbstractOperatorSPtr AbstractOperator::recreate(const std::vector<AllParameterVariant>& args) const {
  std::unordered_map<const AbstractOperator*, AbstractOperatorSPtr> recreated_ops;
  return _recreate_impl(recreated_ops, args);
}

TableCSPtr AbstractOperator::input_table_left() const { return _input_left->get_output(); }

TableCSPtr AbstractOperator::input_table_right() const { return _input_right->get_output(); }

bool AbstractOperator::transaction_context_is_set() const { return _transaction_context.has_value(); }

TransactionContextSPtr AbstractOperator::transaction_context() const {
...
void AbstractOperator::set_transaction_context(TransactionContextWPtr transaction_context) {
  _transaction_context = transaction_context;
}

void AbstractOperator::set_transaction_context_recursively(TransactionContextWPtr transaction_context) {
...
AbstractOperatorSPtr AbstractOperator::mutable_input_left() const {
  return std::const_pointer_cast<AbstractOperator>(_input_left);
}

AbstractOperatorSPtr AbstractOperator::mutable_input_right() const {
...
AbstractOperatorCSPtr AbstractOperator::input_left() const { return _input_left; }

AbstractOperatorCSPtr AbstractOperator::input_right() const { return _input_right; }

void AbstractOperator::print(std::ostream& stream) const {
  const auto get_children_fn = [](const auto& op) {
    std::vector<AbstractOperatorCSPtr> children;
...
AbstractOperatorSPtr AbstractOperator::_recreate_impl(
    std::unordered_map<const AbstractOperator*, AbstractOperatorSPtr>& recreated_ops,
  ...
  const auto recreated_input_left =
      input_left() ? input_left()->_recreate_impl(recreated_ops, args) : AbstractOperatorSPtr{};
  const auto recreated_input_right =
      input_right() ? input_right()->_recreate_impl(recreated_ops, args) : AbstractOperatorSPtr{};
