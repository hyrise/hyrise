...
MutateMetaTable::MutateMetaTable(const std::string& table_name, const MetaTableMutation& mutation_type,
    ...
    : AbstractReadOnlyOperator(OperatorType::MutateMetaTable, modification_values),
...
std::shared_ptr<const Table> MutateMetaTable::_on_execute() {
  ...
  switch (_mutation_type) {
    case MetaTableMutation::Insert:
      Hyrise::get().meta_table_manager.insert_into(_table_name, input_values);
      break;
    case MetaTableMutation::Update:
      Hyrise::get().meta_table_manager.update(_table_name, input_values);
      break;
    case MetaTableMutation::Delete:
      Hyrise::get().meta_table_manager.delete_from(_table_name, input_values);
      break;
  }
  return nullptr;
}
...
std::shared_ptr<AbstractOperator> MutateMetaTable::_on_deep_copy(
  ...
  return std::make_shared<MutateMetaTable>(_table_name, _mutation_type, copied_input_left);
}
