...
class GetTable : public AbstractReadOnlyOperator {
 public:
  ...
  void set_prunable_subquery_predicates(const std::vector<std::shared_ptr<AbstractExpression>>& subquery_scans) const;
 ...
 protected:
  ...
  mutable std::vector<std::shared_ptr<AbstractExpression>> _prunable_subquery_scans{};
