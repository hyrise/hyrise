...
std::shared_ptr<AbstractOperator> RowSort::_on_deep_copy(
  ...
  return std::make_shared<RowSort>(copied_left_input, _sort_definitions, _output_chunk_size, _force_materialization);
}
...
std::shared_ptr<const Table> RowSort::_on_execute() {
  ...
  if (input_table->row_count() == 0) {
    if (_force_materialization == Sort::ForceMaterialization::Yes && input_table->type() == TableType::References) {
...

  {
    ...
    // Functor to get the byte at a given offset from a pointer.
    struct GetChar {
      unsigned char operator()(const unsigned char* ptr, size_t offset) const {
        return ptr[offset];
      }
    };

    // Functor to get the total length of the key.
    // This is the missing piece the compiler was complaining about.
    struct GetLength {
      const size_t key_size;
      size_t operator()(const unsigned char* ptr) const {
        return key_size;
      }
    };

    // Functor to compare two full keys when needed.
    struct KeyComparator {
      const size_t key_size;
      bool operator()(const unsigned char* a, const unsigned char* b) const {
        return std::memcmp(a, b, key_size) < 0;
    ...
    /*boost::sort::spreadsort::string_sort(key_pointers.begin(), key_pointers.end(),
                                         GetChar{},
                                         GetLength{comparison_key_size},
                                         KeyComparator{comparison_key_size});*/
  }

  const auto row_id_offset = key_size - sizeof(RowID);
  auto sorted_pos_list = std::make_optional<RowIDPosList>();
  {
    sorted_pos_list.emplace();
  ...
  if (must_materialize) {
    ...
    sorted_table =
        write_materialized_output_table(input_table, std::move(*sorted_pos_list), _output_chunk_size);
  } else {
