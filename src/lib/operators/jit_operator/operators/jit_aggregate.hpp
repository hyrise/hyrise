...
// Each aggregate and groupby cxlumn stores its position in the output table. This allows the operator to maintain
// arbitrary orders of groupby and aggregate cxlumns.
struct JitAggregateCxlumn {
 ...
 * The operator groups tuples by a number of cxlumns and computes a set of aggregates for each group of tuples.
 * The operator uses two internal data structures to perform its work:
 * - A set of output vectors - one for each groupby cxlumn and one for each aggregate. The output table is later build
 *   from these vectors.
 ...
 * - A hash across all groupby cxlumns is computed.
 ...
 *   tuple. If any row matches across ALL groupby cxlumns, the aggregates in that row are updated with the values from
...
class JitAggregate : public AbstractJittableSink {
 public:
  ...
  // Creates the output table with appropriate cxlumn definitions
  ...
  // Adds a cxlumn to the operator that is to be considered when grouping tuples.
  void add_groupby_cxlumn(const std::string& cxlumn_name, const JitTupleValue& value);

  const std::vector<JitAggregateCxlumn> aggregate_cxlumns() const;
  const std::vector<JitGroupByCxlumn> groupby_cxlumns() const;

 private:
  void _consume(JitRuntimeContext& context) const final;

  uint32_t _num_hashmap_cxlumns{0};
  std::vector<JitAggregateCxlumn> _aggregate_cxlumns;
  std::vector<JitGroupByCxlumn> _groupby_cxlumns;
};
