...
// The optional hashmap_value_2 is used for computing averages. In this case the aggregate operator computes two
// aggregates (a SUM and a COUNT) and divides the two in a post-processing step.
...
// Represents a column that is considered when grouping tuples.
 ...
 * The operator groups tuples by a number of columns and computes a set of aggregates of each tuple group.
 ...
 * - In case of no match (i.e., this hash was encountered for the first time), a new tuple group is created by appending
 *   a value to each output vector and adding a mapping from the hash to the index in the output vectors.
 ...
 *   the current tuple. Otherwise, a new tuple group is created (see above).
