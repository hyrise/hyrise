...
void Difference::execute() {
  ...
  std::unordered_set<std::string> right_input_row_set(_input_right->row_count());

  // Iterating over all chunks and for each chunk over all columns
  for (ChunkID chunk_id = 0; chunk_id < _input_right->chunk_count(); chunk_id++) {
    const Chunk &chunk = _input_right->get_chunk(chunk_id);
  ...
  for (ChunkID chunk_id = 0; chunk_id < _input_left->chunk_count(); chunk_id++) {
    const Chunk &in_chunk = _input_left->get_chunk(chunk_id);
    Chunk out_chunk;
    ...
    for (size_t column_id = 0; column_id < _input_left->col_count(); column_id++) {
      ...
      const auto referenced_column =
          std::dynamic_pointer_cast<ReferenceColumn>(_input_left->get_chunk(chunk_id).get_column(column_id));
      auto out_column_id = column_id;
      auto out_referenced_table = _input_left;
    ...
    if (out_chunk.size() > 0) {
      _output->add_chunk(std::move(out_chunk));
    }
  }
}

std::shared_ptr<const Table> Difference::get_output() const { return _output; }
}  // namespace opossum
