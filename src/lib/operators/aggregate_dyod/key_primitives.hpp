...
KeyLayout compute_key_layout(const std::vector<ColumnID>& group_by_column_ids, const Table& input_table,
                             size_t length_field_width, std::optional<size_t> string_blob_bytes = std::nullopt);

NumericKeyLaneEntry make_numeric_lane(DataType data_type, ColumnID column_id, uint32_t field_offset,
                                      uint32_t null_bit_index);
