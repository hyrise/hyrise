...
class RadixClusterSort {
 ...
 public:
  RadixClusterOutput<T> execute() {
    ...
    auto materialized_left_segments = MaterializedSegmentList<T>{};
    auto samples_left = std::vector<T>{};
    auto materialized_right_segments = MaterializedSegmentList<T>{};
    ...
    auto materialize_left_side = [&](const auto& input_bloom_filter) {
      ...
      output.null_rows_left = std::move(std::get<1>(materialization_results));
    ...
    auto materialize_right_side = [&](const auto& input_bloom_filter) {
      ...
      output.null_rows_right = std::move(std::get<1>(materialization_results));
