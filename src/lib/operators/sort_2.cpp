...
Sort2::Sort2(const std::shared_ptr<const AbstractOperator>& in, const ColumnID column_id, const OrderByMode order_by_mode,
...
ColumnID Sort2::column_id() const { return _column_id; }

OrderByMode Sort2::order_by_mode() const { return _order_by_mode; }

const std::string Sort2::name() const { return "Sort"; }

std::shared_ptr<AbstractOperator> Sort2::_on_deep_copy(
    const std::shared_ptr<AbstractOperator>& copied_input_left,
    const std::shared_ptr<AbstractOperator>& copied_input_right) const {
  return std::make_shared<Sort2>(copied_input_left, _column_id, _order_by_mode, _output_chunk_size);
}

void Sort2::_on_set_parameters(const std::unordered_map<ParameterID, AllTypeVariant>& parameters) {}

std::shared_ptr<const Table> Sort2::_on_execute() {
...
class Sort2::SortImplMaterializeOutput {
 public:
  ...
  SortImplMaterializeOutput(const std::shared_ptr<const Table>& in,
                            ...
                            const size_t output_chunk_size, const ColumnID column_id)
...
template <typename SortColumnType>
class Sort2::SortImpl : public AbstractReadOnlyOperatorImpl {
 public:
