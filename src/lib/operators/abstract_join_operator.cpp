...
std::shared_ptr<Table> AbstractJoinOperator::_build_output_table(std::vector<std::shared_ptr<Chunk>>&& chunks) const {
  ...
  return std::make_shared<Table>(output_column_definitions, TableType::References, std::move(chunks));
}
