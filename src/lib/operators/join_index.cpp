...
void JoinIndex::_fallback_nested_loop(const ChunkID index_chunk_id, const bool track_probe_matches,
  ...
  for (ChunkID probe_chunk_id{0}; probe_chunk_id < chunk_count; ++probe_chunk_id) {
    ...
    const auto& probe_segment = chunk->get_segment(_adjusted_primary_predicate.column_ids.first);
    JoinNestedLoop::JoinParams params(*_probe_pos_list, *_index_pos_list, _probe_matches[probe_chunk_id],
                                      _index_matches[index_chunk_id], _mode,
                                      _adjusted_primary_predicate.predicate_condition, secondary_predicate_evaluator,
                                      track_probe_matches, track_index_matches, !is_semi_or_anti_join);
