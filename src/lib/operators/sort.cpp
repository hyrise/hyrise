        ...
        for (ColumnID column_id{0u}; column_id < output->column_count(); ++column_id) {
          ...
          resolve_data_type(column_data_type, [&](auto type) {
            ...
            for (auto row_index = 0u; row_index < row_count_out; ++row_index) {
              ...
              value_segment_value_vector[chunk_offset_out] =
                  is_null ? std::move(ColumnDataType{}) : std::move(*typed_value);
              ...
              if (chunk_offset_out >= _output_chunk_size) {
                chunk_offset_out = 0u;
                auto value_segment = std::make_shared<ValueSegment<ColumnDataType>>(
                    std::move(value_segment_value_vector), std::move(value_segment_null_vector));
