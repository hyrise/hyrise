...
inline std::shared_ptr<OperatorTask> OperatorTranslator::translate(
  ...
  for (const auto& column_name : column_names) {
    column_expressions.emplace_back(ExpressionNode::create_column_identifier(column_name));
  }
...
inline std::shared_ptr<OperatorTask> OperatorTranslator::translate(const proto::SortOperator& sort_operator) {
  ...
  auto sort = std::make_shared<Sort>(input_task->get_operator(), column_name, order_by_mode, output_chunk_size);
