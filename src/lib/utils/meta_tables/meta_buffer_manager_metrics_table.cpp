...
MetaBufferManagerMetricsTable::MetaBufferManagerMetricsTable()
    : AbstractMetaTable(TableColumnDefinitions{{"pool_size_dram", DataType::Long, false},
                                               ...
                                               {"num_numa_page_movements", DataType::Long, false},
...
std::shared_ptr<Table> MetaBufferManagerMetricsTable::_on_generate() const {
  ...
  const auto [dram_buffer_pool, numa_buffer_pool] = Hyrise::get().buffer_manager.buffer_pools();
  auto output_table = std::make_shared<Table>(_column_definitions, TableType::Data, std::nullopt, UseMvcc::No);
  output_table->append(
      {static_cast<int64_t>(Hyrise::get().buffer_manager.config().dram_buffer_pool_size),
       ...
       static_cast<int64_t>(Hyrise::get().topology.free_bytes(dram_buffer_pool->get_node_id())),
       static_cast<int64_t>(Hyrise::get().topology.free_bytes(numa_buffer_pool->get_node_id())),
       static_cast<int64_t>(Hyrise::get().topology.total_bytes(dram_buffer_pool->get_node_id())),
       static_cast<int64_t>(Hyrise::get().topology.total_bytes(dram_buffer_pool->get_node_id())),
       static_cast<int64_t>(dram_buffer_pool->get_reserved_bytes()),
       static_cast<int64_t>(numa_buffer_pool->get_reserved_bytes()),
       ...
       static_cast<int64_t>(dram_buffer_pool->get_num_eviction_queue_items_purged()),
       static_cast<int64_t>(numa_buffer_pool->get_num_eviction_queue_items_purged()),
       static_cast<int64_t>(dram_buffer_pool->get_num_eviction_queue_adds()),
       static_cast<int64_t>(numa_buffer_pool->get_num_eviction_queue_adds()),
       static_cast<int64_t>(dram_buffer_pool->get_num_eviction_queue_adds()),
       static_cast<int64_t>(numa_buffer_pool->get_num_eviction_queue_adds()),
       static_cast<int64_t>(metrics->num_madvice_free_calls.load(std::memory_order_relaxed)),
       static_cast<int64_t>(metrics->num_numa_page_movements.load(std::memory_order_relaxed)),
