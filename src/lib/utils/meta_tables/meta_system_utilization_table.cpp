...
uint64_t MetaSystemUtilizationTable::_get_system_cpu_time() {
  ...
  try {
    ...
    stat_file.close();
  } catch (std::ios_base::failure& fail) { Fail("Failed to read /proc/stat (" + fail.what() + ")"); }
...
MetaSystemUtilizationTable::SystemMemoryUsage MetaSystemUtilizationTable::_get_system_memory_usage() {
  ...
  try {
    ...
    meminfo_file.close();
  } catch (std::ios_base::failure& fail) { Fail("Failed to read /proc/meminfo (" + fail.what() + ")"); }
...
MetaSystemUtilizationTable::ProcessMemoryUsage MetaSystemUtilizationTable::_get_process_memory_usage() {
  ...
  } catch (std::ios_base::failure& fail) { Fail("Failed to read /proc/self/status (" + fail.what() + ")"); }
