...
MetaMockTable::MetaMockTable()
    : AbstractMetaTable(), _column_definitions(TableColumnDefinitions{{"mock", DataType::String, false}}), _insert_calls(0), _remove_calls(0), _update_calls(0)  {}
...
void MetaMockTable::_insert(const std::vector<AllTypeVariant>& values) {
  _insert_calls++;
}

void MetaMockTable::_remove(const AllTypeVariant& key) {
  _remove_calls++;
}

void MetaMockTable::_update(const AllTypeVariant& key, const std::vector<AllTypeVariant>& values) {
  _update_calls++;
}
