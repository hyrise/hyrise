...
// murmur hash for builtin number types (int, double)
template <typename T>
typename std::enable_if<std::is_arithmetic<T>::value, uint32_t>::type murmur3(T key, uint32_t seed) {
  return murmur_hash3(&key, sizeof(T), seed);
}

// murmur hash for std::string
template <typename T>
typename std::enable_if<std::is_same<T, std::string>::value, uint32_t>::type murmur3(T key, uint32_t seed) {
