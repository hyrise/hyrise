...
class MetaTableManager : public Noncopyable {
 public:
  ...
  static std::shared_ptr<Table> generate_tables_table() const;
  static std::shared_ptr<Table> generate_columns_table() const;
  static std::shared_ptr<Table> generate_chunks_table() const;
  static std::shared_ptr<Table> generate_segments_table() const;

  // Returns name.starts_with(META_PREFIX) as stdlibc++ does not support starts_with yet.
  static bool is_meta_table_name(const std::string& name) const;
