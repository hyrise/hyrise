 ...
 */
enum class OrderSensitivity : uint8_t { Yes, No };
 ...
 */
enum class TypeCmpMode : uint8_t { Strict, Lenient };
 ...
 */
enum class FloatComparisonMode : uint8_t { RelativeDifference, AbsoluteDifference };

/*
 * As SQLite has a weird type concept, we ignore (NOT) NULL constraints for tables retrieved from SQLite.
 */

enum class IgnoreNullable : uint8_t { Yes, No };
