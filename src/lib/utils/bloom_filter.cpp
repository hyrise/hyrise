...
template <uint8_t FilterSizeExponent, uint8_t BlockSizeExponent, uint8_t K>
  BlockBloomFilter<FilterSizeExponent, BlockSizeExponent, K>::BlockBloomFilter() : BaseBloomFilter(FilterSizeExponent, BlockSizeExponent, K) {
    _readonly_filter = reinterpret_cast<uint64_t*>(_filter.data());
  }
...
template <uint8_t FilterSizeExponent, uint8_t BlockSizeExponent, uint8_t K>
  uint32_t BlockBloomFilter<FilterSizeExponent, BlockSizeExponent, K>::_extract_bits(uint64_t hash, uint8_t hash_function_index) const {
