...
std::pair<SegmentEncodingSpec, bool> ColumnFeatureExtractor::_get_encoding_type_for_segment(
  ...
  if (encoded_scan_segment) {
    ...
    if (encoded_scan_segment->compressed_vector_type()) {
      // TODO(Bouncner): merge with encoding PR to get this functionality in place
      // segment_encoding_spec.vector_compression_type = *encoded_scan_segment->compressed_vector_type();
      // VectorCompressionType parent_vector_compression_type(const CompressedVectorType compressed_vector_type)
      std::cout << "Warning: vector compression data is not emitted." << std::endl;
    }
