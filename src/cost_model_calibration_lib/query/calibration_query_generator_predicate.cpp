...
const std::shared_ptr<AbstractExpression> CalibrationQueryGeneratorPredicate::generate_predicate_between_value_value(
  ...
  const auto scan_column_configuration =
      _find_column_for_configuration(generator_configuration.column_definitions, calibration_config.first_data_type,
  ...
  const auto first_value =
      _generate_value_expression(calibration_config.first_data_type, calibration_config.selectivity);
  const auto second_value = _generate_value_expression(calibration_config.first_data_type, 0.0f);
...
const std::shared_ptr<AbstractExpression> CalibrationQueryGeneratorPredicate::generate_predicate_between_column_column(
  ...
  const auto scan_column_configuration = _find_column_for_configuration(
      remaining_columns, calibration_config.first_data_type, calibration_config.first_encoding_type);
  ...
  const auto first_column_configuration = _find_column_for_configuration(
      remaining_columns, calibration_config.second_data_type, calibration_config.second_encoding_type);
  ...
  const auto second_column_configuration = _find_column_for_configuration(
      remaining_columns, calibration_config.third_data_type, calibration_config.third_encoding_type);
...
const std::shared_ptr<AbstractExpression> CalibrationQueryGeneratorPredicate::generate_predicate_column_value(
  ...
  const auto value = _generate_value_expression(calibration_config.first_data_type, calibration_config.selectivity);
...
const std::shared_ptr<AbstractExpression> CalibrationQueryGeneratorPredicate::generate_predicate_like(
  ...
  if (calibration_config.first_data_type != DataType::String) return {};
  ...
  const auto filter_column = _generate_column_expression(table, *filter_column_configuration);
  const auto value =
      _generate_value_expression(calibration_config.first_data_type, calibration_config.selectivity, true);
...
const std::shared_ptr<AbstractExpression> CalibrationQueryGeneratorPredicate::generate_predicate_or(
  ...
  const CalibrationQueryGeneratorPredicateConfiguration second_configuration{calibration_config.table_name,
                                                                             calibration_config.first_encoding_type,
                                                                             calibration_config.first_data_type,
                                                                             calibration_config.second_encoding_type,
                                                                             calibration_config.second_data_type,
                                                                             calibration_config.third_encoding_type,
                                                                             calibration_config.third_data_type,
                                                                             0.5f,
                                                                             calibration_config.reference_column,
                                                                             calibration_config.row_count};
