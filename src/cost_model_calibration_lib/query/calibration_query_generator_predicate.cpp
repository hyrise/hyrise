              ...
              for (const auto& third_encoding : configuration.encodings) {
                for (const auto& selectivity : configuration.selectivities) {
                  for (const auto& table : tables) {
                    output.push_back({table.first, data_type, first_encoding, second_encoding, third_encoding, selectivity, false, table.second});
                    output.push_back({table.first, data_type, first_encoding, second_encoding, third_encoding, selectivity, true, table.second});
                  }
                }
              }
            }
...
const std::shared_ptr<AbstractExpression> CalibrationQueryGeneratorPredicate::generate_predicate_between_value_value(
  ...
  const auto first_value =
      _generate_value_expression(calibration_config.data_type, calibration_config.selectivity);
...
const std::shared_ptr<AbstractExpression> CalibrationQueryGeneratorPredicate::generate_predicate_like(
  ...
  const auto filter_column = _generate_column_expression(table, *filter_column_configuration);
  const auto value =
      _generate_value_expression(calibration_config.data_type, calibration_config.selectivity, true);
