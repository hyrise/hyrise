...
const std::vector<std::string> CalibrationQueryGenerator::generate_queries(
  ...
  //  auto join_query = CalibrationQueryGenerator::_generate_join(table_definitions);
  //  if (join_query) {
...
const std::optional<std::string> CalibrationQueryGenerator::_generate_table_scan(
  ...
  auto predicates = CalibrationQueryGeneratorPredicates::generate_predicates(predicate_generator, column_definitions);
...
//  }
//
//  auto join_columns = _generate_join_columns(left_table->columns, right_table->columns);
//
//  auto left_predicate = CalibrationQueryGeneratorPredicates::generate_predicates(left_table->columns, "l.");
//  auto right_predicate = CalibrationQueryGeneratorPredicates::generate_predicates(right_table->columns, "r.");
//
...
//
//  auto column_definitions = table_definition.columns;
//  auto predicates = CalibrationQueryGeneratorPredicates::generate_predicates(column_definitions);
//
