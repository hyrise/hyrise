...
class UccDiscoveryPlugin : public AbstractPlugin {
 ...
 private:
  ...
  static bool _dictionary_segments_contain_duplicates(const std::shared_ptr<Table>& table, const ColumnID column_id);
  ...
  static void _ucc_candidates_from_removable_join_input(const std::shared_ptr<const AbstractLQPNode>& root_node,
                                                        const std::shared_ptr<LQPColumnExpression>& column_candidate,
                                                        UccCandidates& ucc_candidates);
};
