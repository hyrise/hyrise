...
class HistogramGetter {
 public:
  static std::shared_ptr<const AbstractHistogram<ColumnDataType>> get_histogram(
...
class AbstractClusteringAlgo {
 public:
  AbstractClusteringAlgo(ClusteringByTable clustering)
      : clustering_by_table(clustering), _original_table_sizes{}, _runtime_statistics{} {}
 ...
 protected:
  ...
  // helper functions
  std::shared_ptr<Chunk> _create_empty_chunk(const std::shared_ptr<const Table>& table,
  ...
  void _append_chunks_to_table(const std::vector<std::shared_ptr<Chunk>>& chunks, const std::shared_ptr<Table> table,
                               bool allow_mutable = true) const;
  void _append_sorted_chunks_to_table(const std::vector<std::shared_ptr<Chunk>>& chunks,
                                      const std::shared_ptr<Table> table, bool allow_mutable = true) const;
  std::shared_ptr<Chunk> _sort_chunk(std::shared_ptr<Chunk> chunk, const ColumnID sort_column,
