...
class DisjointClustersAlgo : public AbstractClusteringAlgo {
 ...
 protected:
  ...
  ClusterBoundaries _get_boundaries(const std::shared_ptr<const AbstractHistogram<ColumnDataType>>& histogram,
                                    const size_t row_count, const size_t num_clusters, const bool nullable) const;
  ...
  std::vector<std::shared_ptr<Chunk>> _sort_and_encode_chunks(const std::vector<std::shared_ptr<Chunk>>& chunks,
                                                              const ColumnID sort_column_id) const;
