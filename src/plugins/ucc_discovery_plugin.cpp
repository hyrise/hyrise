...
bool UccDiscoveryPlugin::_dictionary_segments_contain_duplicates(const std::shared_ptr<Table>& table,
...
template <typename ColumnDataType>
bool UccDiscoveryPlugin::_uniqueness_holds_across_segments(const std::shared_ptr<Table>& table,
                                                           const ColumnID column_id) {
  const auto chunk_count = table->chunk_count();
...
void UccDiscoveryPlugin::_ucc_candidates_from_removable_join_input(
    ...
    const std::shared_ptr<LQPColumnExpression>& column_candidate, UccCandidates& ucc_candidates) {
