...
class ReductionBenchmarks : public MicroBenchmarkBasicFixture {
 public:
  ...
  void generate_ssb_data(const float scale_factor, const uint32_t worker_count) {
    ...
    std::cout << "Generating Star Schema data set with scale factor " << scale_factor << ", automatic encoding and worker count " << worker_count << "\n";
...
//   setenv("HASH", std::to_string(hash_count).c_str(), 1);
  ...
  const auto build_reduce_dryrun = std::make_shared<Reduce<ReduceMode::Build, UseMinMax::Yes>>(_left_input, _right_input, predicate);
  const auto probe_reduce_dryrun = std::make_shared<Reduce<ReduceMode::Probe, UseMinMax::Yes>>(_left_input, build_reduce_dryrun, predicate);
  ...
  for (auto _ : state) {
    const auto build_reduce = std::make_shared<Reduce<ReduceMode::Build, UseMinMax::Yes>>(_left_input, _right_input, predicate);
    const auto probe_reduce = std::make_shared<Reduce<ReduceMode::Probe, UseMinMax::Yes>>(_left_input, build_reduce, predicate);
