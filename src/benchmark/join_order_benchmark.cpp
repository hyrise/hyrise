...
void add_key_constraints(std::unordered_map<std::string, BenchmarkTableInfo>& table_info_by_name) {
  ...
  const std::unordered_map<std::string, std::string> unique_constraints = {
      {"role_type", "role"}, {"keyword", "keyword"}, {"company_type", "kind"},
      {"info_type", "info"}, {"kind_type", "kind"},  {"comp_cast_type", "kind"}};
