...
int main(int argc, char* argv[]) {
  ...
  const auto title_table_constraint = TableKeyConstraint({{title_table->column_id_by_name("id")}, KeyConstraintType::PRIMARY_KEY});
  title_table->add_soft_key_constraint(title_table_constraint);

  const auto& name_table = Hyrise::get().storage_manager.get_table("name");
  const auto name_table_constraint = TableKeyConstraint({{name_table->column_id_by_name("id")}, KeyConstraintType::PRIMARY_KEY});
  name_table->add_soft_key_constraint(name_table_constraint);

  const auto& aka_title_table = Hyrise::get().storage_manager.get_table("aka_title");
  aka_title_table->add_soft_key_constraint({{aka_title_table->column_id_by_name("movie_id")}, KeyConstraintType::FOREIGN_KEY, std::make_shared<TableKeyConstraint>(title_table_constraint)});

  const auto& cast_info_table = Hyrise::get().storage_manager.get_table("cast_info");
  cast_info_table->add_soft_key_constraint({{cast_info_table->column_id_by_name("movie_id")}, KeyConstraintType::FOREIGN_KEY, std::make_shared<TableKeyConstraint>(title_table_constraint)});
  cast_info_table->add_soft_key_constraint({{cast_info_table->column_id_by_name("person_id")}, KeyConstraintType::FOREIGN_KEY, std::make_shared<TableKeyConstraint>(name_table_constraint)});

  const auto& complete_cast_table = Hyrise::get().storage_manager.get_table("complete_cast");
  complete_cast_table->add_soft_key_constraint({{complete_cast_table->column_id_by_name("movie_id")}, KeyConstraintType::FOREIGN_KEY, std::make_shared<TableKeyConstraint>(title_table_constraint)});

  const auto& movie_companies_table = Hyrise::get().storage_manager.get_table("movie_companies");
  movie_companies_table->add_soft_key_constraint({{movie_companies_table->column_id_by_name("movie_id")}, KeyConstraintType::FOREIGN_KEY, std::make_shared<TableKeyConstraint>(title_table_constraint)});

  const auto& movie_info_idx_table = Hyrise::get().storage_manager.get_table("movie_info_idx");
  movie_info_idx_table->add_soft_key_constraint({{movie_info_idx_table->column_id_by_name("movie_id")}, KeyConstraintType::FOREIGN_KEY, std::make_shared<TableKeyConstraint>(title_table_constraint)});

  const auto& movie_keyword_table = Hyrise::get().storage_manager.get_table("movie_keyword");
  movie_keyword_table->add_soft_key_constraint({{movie_keyword_table->column_id_by_name("movie_id")}, KeyConstraintType::FOREIGN_KEY, std::make_shared<TableKeyConstraint>(title_table_constraint)});

  const auto& movie_link_table = Hyrise::get().storage_manager.get_table("movie_link");
  movie_link_table->add_soft_key_constraint({{movie_link_table->column_id_by_name("movie_id")}, KeyConstraintType::FOREIGN_KEY, std::make_shared<TableKeyConstraint>(title_table_constraint)});

  const auto& movie_info_table = Hyrise::get().storage_manager.get_table("movie_info");
  // movie_info's primary key is missing
  movie_info_table->add_soft_key_constraint({{movie_info_table->column_id_by_name("movie_id")}, KeyConstraintType::FOREIGN_KEY, std::make_shared<TableKeyConstraint>(title_table_constraint)});

  const auto& person_info_table = Hyrise::get().storage_manager.get_table("person_info");
  person_info_table->add_soft_key_constraint({{person_info_table->column_id_by_name("person_id")}, KeyConstraintType::FOREIGN_KEY, std::make_shared<TableKeyConstraint>(name_table_constraint)});
