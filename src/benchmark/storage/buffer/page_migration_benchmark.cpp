...
#endif
    _mapped_region = static_cast<std::byte*>(
        mmap(NULL, BufferManagerConfig::DEFAULT_RESERVED_VIRTUAL_MEMORY, PROT_READ | PROT_WRITE, flags, -1, 0));

    Assert(_mapped_region != MAP_FAILED, "Failed to map region");
  }

  void TearDown(const ::benchmark::State& state) {
    Assert(munmap(_mapped_region, BufferManagerConfig::DEFAULT_RESERVED_VIRTUAL_MEMORY), "Failed to unmap region");
  }
