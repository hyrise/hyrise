...
void BenchmarkBasicFixture::SetUp(::benchmark::State& state) {
  ...
  _table_dict_wrapper = std::make_shared<TableWrapper>(table_generator->generate_table(_chunk_size, EncodingType::Dictionary));
  _table_new_dict_wrapper = std::make_shared<TableWrapper>(table_generator->generate_table(_chunk_size, EncodingType::NewDictionary));
  _table_wrapper_a->execute();
  _table_wrapper_b->execute();
  _table_dict_wrapper->execute();
  _table_new_dict_wrapper->execute();
}
