    ...
    const std::string CalibrationQueryGeneratorPredicates::generate_predicate(
        ...
        if (second_column) {
            for (const auto& column : column_definitions) {
                ...
                if (column.second.type != filter_column->second.type || column.second.type != second_column->second.type) continue;
        ...
        if (foo > 0.95 && second_column && third_column) {
            ...
            return boost::str(boost::format(between_predicate_template) % filter_column_name % second_column_name % third_column_name);
        } else if (foo > 0.75) {
            ...
            return boost::str(boost::format(between_predicate_template) % filter_column_name % first_filter_column_value % second_filter_column_value);
