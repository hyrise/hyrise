...
namespace opossum {
    ...
    const nlohmann::json CostModelFeatureExtractor::extract_features(const std::shared_ptr<const AbstractOperator> &op) {
        ...
        if (const auto& output = op->get_output()) {
            ...
            auto left_input_memory_usage = (op->input_left()) ? op->input_left()->get_output()->estimate_memory_usage()
                                                              : 0;
            ...
            auto right_input_memory_usage = (op->input_right())
                                            ? op->input_right()->get_output()->estimate_memory_usage() : 0;
    ...
    const nlohmann::json CostModelFeatureExtractor::_extract_features_for_operator(const std::shared_ptr<const TableScan> &op) {
        nlohmann::json operator_result{};
    ...
    const nlohmann::json CostModelFeatureExtractor::_extract_features_for_operator(const std::shared_ptr<const GetTable> &op) {
        nlohmann::json operator_result{};
        return operator_result;
    }

    const nlohmann::json CostModelFeatureExtractor::_extract_features_for_operator(const std::shared_ptr<const Projection> &op) {
        nlohmann::json operator_result{};
    ...
    const nlohmann::json CostModelFeatureExtractor::_extract_features_for_operator(const std::shared_ptr<const JoinHash> &op) {
        nlohmann::json operator_result{};
