...
BENCHMARK_DEFINE_F(BenchmarkJoinFixture, BM_JoinSortMerge)(benchmark::State& state) {
  ...
  auto warm_up =
      std::make_shared<JoinSortMerge>(_tw_small_uni1, _tw_small_uni2, JoinMode::Inner, std::pair<ColumnID, ColumnID>{ColumnID{0}, ColumnID{0}}, PredicateCondition::Equals);
  warm_up->execute();
  while (state.KeepRunning()) {
    auto table_scan =
        std::make_shared<JoinSortMerge>(_tw_small_uni1, _tw_small_uni2, JoinMode::Inner, std::pair<ColumnID, ColumnID>(ColumnID{0}, ColumnID{0}), PredicateCondition::Equals);
...
BENCHMARK_DEFINE_F(BenchmarkJoinFixture, BM_JoinNestedLoop)(benchmark::State& state) {
  ...
  auto warm_up =
      std::make_shared<JoinNestedLoop>(_tw_small_uni1, _tw_small_uni2, JoinMode::Inner, std::pair<ColumnID, ColumnID>{ColumnID{0}, ColumnID{0}}, PredicateCondition::Equals);
  warm_up->execute();
  while (state.KeepRunning()) {
    auto table_scan =
        std::make_shared<JoinNestedLoop>(_tw_small_uni1, _tw_small_uni2, JoinMode::Inner, std::pair<ColumnID, ColumnID>(ColumnID{0}, ColumnID{0}), PredicateCondition::Equals);
