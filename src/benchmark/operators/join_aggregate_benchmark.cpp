...
std::shared_ptr<TableWrapper> create_zip_table(const size_t table_size) {
  ...
  for (ChunkID chunk_index = ChunkID{0}; chunk_index < chunk_count; ++chunk_index) {
    ...
    chunk->finalize();
    chunk->set_ordered_by(std::make_pair<ColumnID, OrderByMode>(ColumnID{0}, OrderByMode::Ascending));
    chunk->set_ordered_by(std::make_pair<ColumnID, OrderByMode>(ColumnID{1}, OrderByMode::Ascending));
  }
