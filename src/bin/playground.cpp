...
public:
  ...
  TableBuilder(size_t chunk_size, const hana::tuple<ColumnDefinitions...>& column_definitions, UseMvcc use_mvcc,
    ...
    auto ccolumn_definitions = TableColumnDefinitions{};
    hana::fold_left(column_definitions, ccolumn_definitions,
                           [](auto& definitions, auto name_and_type_and_is_nullable) -> decltype(definitions) {
                             definitions.emplace_back(name_and_type_and_is_nullable[hana::llong_c<0>],
                                                      name_and_type_and_is_nullable[hana::llong_c<1>],
                                                      name_and_type_and_is_nullable[hana::llong_c<2>]);
                             return definitions;
                           });
    _table = std::make_shared<Table>(ccolumn_definitions, TableType::Data, chunk_size, use_mvcc);
  ...
  void append_row(std::optional<DataTypes>&&... column_values) {
    ...
    hana::for_each(vectors_and_values, [](auto&& data_vector_and_is_null_vector_and_value) {
      ...
      is_null_vector.emplace_back(is_null);
    });
...
private:
  ...
  // so we define a struct which defines an alias for boolean and consumes the template parameter, so we can use "..."
  template<typename T>
  struct ConsumeTemplateParameterAndDefineBoolean { using boolean = bool; };
  hana::tuple<std::vector<typename ConsumeTemplateParameterAndDefineBoolean<DataTypes>::boolean>...> _is_null_vectors;
  ...
  void _emit_chunk() {
    ...
    hana::for_each(_data_vectors_and_is_null_vectors, [&](auto&& data_vector_and_is_null_vector) {
      ...
      segments.push_back(std::make_shared<ValueSegment<T>>(std::move(data_vector), std::move(is_null_vector)));  // NOLINT

      data_vector = std::decay_t<decltype(data_vector)>();
      data_vector.reserve(_estimated_rows_per_chunk);

      is_null_vector = std::decay_t<decltype(is_null_vector)>();
      is_null_vector.reserve(_estimated_rows_per_chunk);
    });
...
const auto column_definitions = hana::make_tuple(
  hana::make_tuple("a", hana::type_c<int64_t>, false),
  hana::make_tuple("b", hana::type_c<int64_t>, true));

int main() {
  auto table_builder = TableBuilder<int64_t, int64_t>(2, column_definitions, UseMvcc::Yes, size_t{3});
