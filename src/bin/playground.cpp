...
void setup_table(std::string table_name) {
  ...
  auto output = std::make_shared<Table>(column_definitions, TableType::Data, ChunkOffset{5});
...
int main() {
  ...
  for (ChunkID chunk_id = ChunkID{0}; chunk_id < chunk_count; ++chunk_id) {
      ...
      for (size_t i = 0; i < sort_definitions.size(); ++i) {
        ...
        resolve_data_type(tab->column_data_type(sort_col), [&](auto type) {
          ...
          if constexpr (std::is_same_v<ColumnDataType, pmr_string>) {
            ...
            throw std::logic_error("Unsupported data type for sorting: " + std::string(typeid(ColumnDataType).name()));
          }
        });
    ...
    */
    ...
    // Print sorted keys
    std::cout << "Sorted row IDs:" << std::endl;
    auto column_count = chunk->column_count();
    for (ChunkOffset i = ChunkOffset{0}; i < row_count_for_chunk; ++i) {
