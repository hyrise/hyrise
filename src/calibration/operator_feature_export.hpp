 ...
 private:
  ...
  const std::map<OperatorType, std::shared_ptr<CSVWriter>> _csv_writers = [&](){
    auto csv_writers_per_operator = std::map<OperatorType, std::shared_ptr<CSVWriter>>();
      ...
      // TODO How to iterate over enums in a safe way? If we change order of OperatorTypes, we might miss some types
      for (int op_type_id = int(OperatorType::Projection); op_type_id != int(OperatorType::Validate); ++op_type_id )
      {
