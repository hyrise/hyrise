...
enum TableFeatureExportType {
    TABLE, COLUMN, SEGMENT
};

class TableFeatureExport{
 public:
    TableFeatureExport(const std::string& path_to_dir);
...
private:
  ...
  CSVWriter _table_csv_writer = CSVWriter(_path_to_dir + "/" + _table_meta_file_name + ".csv", _get_header(TableFeatureExportType::TABLE));
  CSVWriter _column_csv_writer = CSVWriter(_path_to_dir + "/" + _column_meta_file_name + ".csv", _get_header(TableFeatureExportType::COLUMN));
  CSVWriter _segment_csv_writer = CSVWriter(_path_to_dir + "/" + _segment_meta_file_name + ".csv", _get_header(TableFeatureExportType::SEGMENT));
