...
TPCCBenchmarkItemRunner::TPCCBenchmarkItemRunner(const std::shared_ptr<BenchmarkConfig>& config,
                                                 int num_warehouses)
...
const std::vector<BenchmarkItemID>& TPCCBenchmarkItemRunner::items() const {
  static const std::vector<BenchmarkItemID> items{BenchmarkItemID{0}, BenchmarkItemID{1}, BenchmarkItemID{2}, BenchmarkItemID{3}, BenchmarkItemID{4}};
  return items;
}
...
void TPCCBenchmarkItemRunner::_on_execute_item(const BenchmarkItemID item_id, BenchmarkSQLExecutor& sql_executor) {
  ...
  switch (item_id) {
    ...
    case 0: (void)TpccDelivery{_num_warehouses, sql_executor}.execute(); break;
    case 1: (void)TpccNewOrder{_num_warehouses, sql_executor}.execute(); break;
    case 2: (void)TpccOrderStatus{_num_warehouses, sql_executor}.execute(); break;
    case 3: (void)TpccPayment{_num_warehouses, sql_executor}.execute(); break;
    case 4: (void)TpccStockLevel{_num_warehouses, sql_executor}.execute(); break;
    default: Fail("Invalid item_id");
  }
}
