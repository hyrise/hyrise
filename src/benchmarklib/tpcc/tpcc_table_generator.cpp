...
std::shared_ptr<Table> TpccTableGenerator::generate_items_table() {
   ...
   * indices[0] = item
   */
  std::vector<ChunkSegments> columns_by_chunk;
  TableCxlumnDefinitions cxlumn_definitions;

  auto original_ids = _random_gen.select_unique_ids(NUM_ITEMS / 10, NUM_ITEMS);

  add_column<int>(columns_by_chunk, cxlumn_definitions, "I_ID", cardinalities,
                  [&](std::vector<size_t> indices) { return indices[0]; });
  add_column<int>(columns_by_chunk, cxlumn_definitions, "I_IM_ID", cardinalities,
                  [&](std::vector<size_t>) { return _random_gen.random_number(1, 10000); });
  add_column<std::string>(columns_by_chunk, cxlumn_definitions, "I_NAME", cardinalities,
                          [&](std::vector<size_t>) { return _random_gen.astring(14, 24); });
  add_column<float>(columns_by_chunk, cxlumn_definitions, "I_PRICE", cardinalities,
                    [&](std::vector<size_t>) { return _random_gen.random_number(100, 10000) / 100.f; });
  add_column<std::string>(
      columns_by_chunk, cxlumn_definitions, "I_DATA", cardinalities, [&](std::vector<size_t> indices) {
  ...
  auto table = std::make_shared<Table>(cxlumn_definitions, TableType::Data, _chunk_size, UseMvcc::Yes);
...
std::shared_ptr<Table> TpccTableGenerator::generate_warehouse_table() {
  ...
  add_column<int>(columns_by_chunk, cxlumn_definitions, "W_ID", cardinalities,
                  [&](std::vector<size_t> indices) { return indices[0]; });
  add_column<std::string>(columns_by_chunk, cxlumn_definitions, "W_NAME", cardinalities,
                          [&](std::vector<size_t>) { return _random_gen.astring(6, 10); });
  add_column<std::string>(columns_by_chunk, cxlumn_definitions, "W_STREET_1", cardinalities,
                          [&](std::vector<size_t>) { return _random_gen.astring(10, 20); });
  add_column<std::string>(columns_by_chunk, cxlumn_definitions, "W_STREET_2", cardinalities,
                          [&](std::vector<size_t>) { return _random_gen.astring(10, 20); });
  add_column<std::string>(columns_by_chunk, cxlumn_definitions, "W_CITY", cardinalities,
                          [&](std::vector<size_t>) { return _random_gen.astring(10, 20); });
  add_column<std::string>(columns_by_chunk, cxlumn_definitions, "W_STATE", cardinalities,
                          [&](std::vector<size_t>) { return _random_gen.astring(2, 2); });

  add_column<std::string>(columns_by_chunk, cxlumn_definitions, "W_ZIP", cardinalities,
                          [&](std::vector<size_t>) { return _random_gen.zip_code(); });
  add_column<float>(columns_by_chunk, cxlumn_definitions, "W_TAX", cardinalities,
                    [&](std::vector<size_t>) { return _random_gen.random_number(0, 2000) / 10000.f; });
  add_column<float>(columns_by_chunk, cxlumn_definitions, "W_YTD", cardinalities, [&](std::vector<size_t>) {
...
std::shared_ptr<Table> TpccTableGenerator::generate_stock_table() {
  ...
  add_column<int>(columns_by_chunk, cxlumn_definitions, "S_I_ID", cardinalities,
                  [&](std::vector<size_t> indices) { return indices[1]; });
  add_column<int>(columns_by_chunk, cxlumn_definitions, "S_W_ID", cardinalities,
                  [&](std::vector<size_t> indices) { return indices[0]; });
  add_column<int>(columns_by_chunk, cxlumn_definitions, "S_QUANTITY", cardinalities,
  ...
  for (int district_i = 1; district_i <= 10; district_i++) {
    ...
    add_column<std::string>(columns_by_chunk, cxlumn_definitions, "S_DIST_" + district_i_str.str(), cardinalities,
                            [&](std::vector<size_t>) { return _random_gen.astring(24, 24); });
  }
  add_column<int>(columns_by_chunk, cxlumn_definitions, "S_YTD", cardinalities, [&](std::vector<size_t>) { return 0; });
  add_column<int>(columns_by_chunk, cxlumn_definitions, "S_ORDER_CNT", cardinalities,
                  [&](std::vector<size_t>) { return 0; });
  add_column<int>(columns_by_chunk, cxlumn_definitions, "S_REMOTE_CNT", cardinalities,
                  [&](std::vector<size_t>) { return 0; });
  add_column<std::string>(
      columns_by_chunk, cxlumn_definitions, "S_DATA", cardinalities, [&](std::vector<size_t> indices) {
        std::string data = _random_gen.astring(26, 50);
...
std::shared_ptr<Table> TpccTableGenerator::generate_district_table() {
  ...
  add_column<int>(columns_by_chunk, cxlumn_definitions, "D_ID", cardinalities,
                  [&](std::vector<size_t> indices) { return indices[1]; });
  add_column<int>(columns_by_chunk, cxlumn_definitions, "D_W_ID", cardinalities,
                  [&](std::vector<size_t> indices) { return indices[0]; });
  add_column<std::string>(columns_by_chunk, cxlumn_definitions, "D_NAME", cardinalities,
                          [&](std::vector<size_t>) { return _random_gen.astring(6, 10); });
  add_column<std::string>(columns_by_chunk, cxlumn_definitions, "D_STREET_1", cardinalities,
                          [&](std::vector<size_t>) { return _random_gen.astring(10, 20); });
  add_column<std::string>(columns_by_chunk, cxlumn_definitions, "D_STREET_2", cardinalities,
                          [&](std::vector<size_t>) { return _random_gen.astring(10, 20); });
  add_column<std::string>(columns_by_chunk, cxlumn_definitions, "D_CITY", cardinalities,
                          [&](std::vector<size_t>) { return _random_gen.astring(10, 20); });
  add_column<std::string>(columns_by_chunk, cxlumn_definitions, "D_STATE", cardinalities,
                          [&](std::vector<size_t>) { return _random_gen.astring(2, 2); });

  add_column<std::string>(columns_by_chunk, cxlumn_definitions, "D_ZIP", cardinalities,
                          [&](std::vector<size_t>) { return _random_gen.zip_code(); });
  add_column<float>(columns_by_chunk, cxlumn_definitions, "D_TAX", cardinalities,
                    [&](std::vector<size_t>) { return _random_gen.random_number(0, 2000) / 10000.f; });
  add_column<float>(columns_by_chunk, cxlumn_definitions, "D_YTD", cardinalities,
                    [&](std::vector<size_t>) { return CUSTOMER_YTD * NUM_CUSTOMERS_PER_DISTRICT; });
  add_column<int>(columns_by_chunk, cxlumn_definitions, "D_NEXT_O_ID", cardinalities,
...
std::shared_ptr<Table> TpccTableGenerator::generate_customer_table() {
  ...
  add_column<int>(columns_by_chunk, cxlumn_definitions, "C_ID", cardinalities,
                  [&](std::vector<size_t> indices) { return indices[2]; });
  add_column<int>(columns_by_chunk, cxlumn_definitions, "C_D_ID", cardinalities,
                  [&](std::vector<size_t> indices) { return indices[1]; });
  add_column<int>(columns_by_chunk, cxlumn_definitions, "C_W_ID", cardinalities,
                  [&](std::vector<size_t> indices) { return indices[0]; });
  add_column<std::string>(columns_by_chunk, cxlumn_definitions, "C_FIRST", cardinalities,
                          [&](std::vector<size_t>) { return _random_gen.astring(8, 16); });
  add_column<std::string>(columns_by_chunk, cxlumn_definitions, "C_MIDDLE", cardinalities,
                          [&](std::vector<size_t>) { return "OE"; });
  add_column<std::string>(columns_by_chunk, cxlumn_definitions, "C_LAST", cardinalities,
                          [&](std::vector<size_t> indices) { return _random_gen.last_name(indices[2]); });
  add_column<std::string>(columns_by_chunk, cxlumn_definitions, "C_STREET_1", cardinalities,
                          [&](std::vector<size_t>) { return _random_gen.astring(10, 20); });
  add_column<std::string>(columns_by_chunk, cxlumn_definitions, "C_STREET_2", cardinalities,
                          [&](std::vector<size_t>) { return _random_gen.astring(10, 20); });
  add_column<std::string>(columns_by_chunk, cxlumn_definitions, "C_CITY", cardinalities,
                          [&](std::vector<size_t>) { return _random_gen.astring(10, 20); });
  add_column<std::string>(columns_by_chunk, cxlumn_definitions, "C_STATE", cardinalities,
                          [&](std::vector<size_t>) { return _random_gen.astring(2, 2); });
  add_column<std::string>(columns_by_chunk, cxlumn_definitions, "C_ZIP", cardinalities,
                          [&](std::vector<size_t>) { return _random_gen.zip_code(); });
  add_column<std::string>(columns_by_chunk, cxlumn_definitions, "C_PHONE", cardinalities,
                          [&](std::vector<size_t>) { return _random_gen.nstring(16, 16); });
  add_column<int>(columns_by_chunk, cxlumn_definitions, "C_SINCE", cardinalities,
                  [&](std::vector<size_t>) { return _current_date; });
  add_column<std::string>(columns_by_chunk, cxlumn_definitions, "C_CREDIT", cardinalities,
                          [&](std::vector<size_t> indices) {
                            bool is_original = original_ids.find(indices[2]) != original_ids.end();
                            return is_original ? "BC" : "GC";
                          });
  add_column<int>(columns_by_chunk, cxlumn_definitions, "C_CREDIT_LIM", cardinalities,
                  [&](std::vector<size_t>) { return 50000; });
  add_column<float>(columns_by_chunk, cxlumn_definitions, "C_DISCOUNT", cardinalities,
                    [&](std::vector<size_t>) { return _random_gen.random_number(0, 5000) / 10000.f; });
  add_column<float>(columns_by_chunk, cxlumn_definitions, "C_BALANCE", cardinalities,
                    [&](std::vector<size_t>) { return -CUSTOMER_YTD; });
  add_column<float>(columns_by_chunk, cxlumn_definitions, "C_YTD_PAYMENT", cardinalities,
                    [&](std::vector<size_t>) { return CUSTOMER_YTD; });
  add_column<int>(columns_by_chunk, cxlumn_definitions, "C_PAYMENT_CNT", cardinalities,
                  [&](std::vector<size_t>) { return 1; });
  add_column<int>(columns_by_chunk, cxlumn_definitions, "C_DELIVERY_CNT", cardinalities,
                  [&](std::vector<size_t>) { return 0; });
  add_column<std::string>(columns_by_chunk, cxlumn_definitions, "C_DATA", cardinalities,
...
std::shared_ptr<Table> TpccTableGenerator::generate_history_table() {
  ...
  add_column<int>(columns_by_chunk, cxlumn_definitions, "H_C_ID", cardinalities,
                  [&](std::vector<size_t> indices) { return indices[2]; });
  add_column<int>(columns_by_chunk, cxlumn_definitions, "H_C_D_ID", cardinalities,
                  [&](std::vector<size_t> indices) { return indices[1]; });
  add_column<int>(columns_by_chunk, cxlumn_definitions, "H_C_W_ID", cardinalities,
                  [&](std::vector<size_t> indices) { return indices[0]; });
  add_column<int>(columns_by_chunk, cxlumn_definitions, "H_DATE", cardinalities,
                  [&](std::vector<size_t>) { return _current_date; });
  add_column<float>(columns_by_chunk, cxlumn_definitions, "H_AMOUNT", cardinalities,
                    [&](std::vector<size_t>) { return 10.f; });
  add_column<std::string>(columns_by_chunk, cxlumn_definitions, "H_DATA", cardinalities,
...
std::shared_ptr<Table> TpccTableGenerator::generate_order_table(
  ...
  add_column<int>(columns_by_chunk, cxlumn_definitions, "O_ID", cardinalities,
                  [&](std::vector<size_t> indices) { return indices[2]; });
  add_column<int>(columns_by_chunk, cxlumn_definitions, "O_D_ID", cardinalities,
                  [&](std::vector<size_t> indices) { return indices[1]; });
  add_column<int>(columns_by_chunk, cxlumn_definitions, "O_W_ID", cardinalities,
                  [&](std::vector<size_t> indices) { return indices[0]; });
  add_column<int>(columns_by_chunk, cxlumn_definitions, "O_C_ID", cardinalities,
                  [&](std::vector<size_t> indices) { return customer_permutation[indices[2]]; });
  add_column<int>(columns_by_chunk, cxlumn_definitions, "O_ENTRY_D", cardinalities,
                  [&](std::vector<size_t>) { return _current_date; });
  // TODO(anybody) -1 should be null

  add_column<int>(columns_by_chunk, cxlumn_definitions, "O_CARRIER_ID", cardinalities,
                  [&](std::vector<size_t> indices) {
                    return indices[2] <= NUM_ORDERS - NUM_NEW_ORDERS ? _random_gen.random_number(1, 10) : -1;
                  });
  add_column<int>(columns_by_chunk, cxlumn_definitions, "O_OL_CNT", cardinalities,
                  [&](std::vector<size_t> indices) { return order_line_counts[indices[0]][indices[1]][indices[2]]; });
  add_column<int>(columns_by_chunk, cxlumn_definitions, "O_ALL_LOCAL", cardinalities,
                  [&](std::vector<size_t>) { return 1; });
...
template <typename T>
void TpccTableGenerator::_add_order_line_column(std::vector<ChunkSegments>& columns_by_chunk,
...
std::shared_ptr<Table> TpccTableGenerator::generate_order_line_table(
  ...
  _add_order_line_column<int>(columns_by_chunk, cxlumn_definitions, "OL_O_ID", cardinalities, order_line_counts,
                              [&](std::vector<size_t> indices) { return indices[2]; });
  _add_order_line_column<int>(columns_by_chunk, cxlumn_definitions, "OL_D_ID", cardinalities, order_line_counts,
                              [&](std::vector<size_t> indices) { return indices[1]; });
  _add_order_line_column<int>(columns_by_chunk, cxlumn_definitions, "OL_W_ID", cardinalities, order_line_counts,
                              [&](std::vector<size_t> indices) { return indices[0]; });
  _add_order_line_column<int>(columns_by_chunk, cxlumn_definitions, "OL_NUMBER", cardinalities, order_line_counts,
                              [&](std::vector<size_t> indices) { return indices[3]; });
  _add_order_line_column<int>(columns_by_chunk, cxlumn_definitions, "OL_I_ID", cardinalities, order_line_counts,
                              [&](std::vector<size_t>) { return _random_gen.random_number(1, NUM_ITEMS); });
  _add_order_line_column<int>(columns_by_chunk, cxlumn_definitions, "OL_SUPPLY_W_ID", cardinalities, order_line_counts,
                              [&](std::vector<size_t> indices) { return indices[0]; });
  // TODO(anybody) -1 should be null
  _add_order_line_column<int>(
      columns_by_chunk, cxlumn_definitions, "OL_DELIVERY_D", cardinalities, order_line_counts,
      [&](std::vector<size_t> indices) { return indices[2] <= NUM_ORDERS - NUM_NEW_ORDERS ? _current_date : -1; });
  _add_order_line_column<int>(columns_by_chunk, cxlumn_definitions, "OL_QUANTITY", cardinalities, order_line_counts,
                              [&](std::vector<size_t>) { return 5; });

  _add_order_line_column<float>(
      columns_by_chunk, cxlumn_definitions, "OL_AMOUNT", cardinalities, order_line_counts,
      [&](std::vector<size_t> indices) {
        return indices[2] <= NUM_ORDERS - NUM_NEW_ORDERS ? 0.f : _random_gen.random_number(1, 999999) / 100.f;
      });
  _add_order_line_column<std::string>(columns_by_chunk, cxlumn_definitions, "OL_DIST_INFO", cardinalities,
                                      order_line_counts,
                                      [&](std::vector<size_t>) { return _random_gen.astring(24, 24); });
...
std::shared_ptr<Table> TpccTableGenerator::generate_new_order_table() {
  ...
  add_column<int>(columns_by_chunk, cxlumn_definitions, "NO_O_ID", cardinalities,
                  [&](std::vector<size_t> indices) { return indices[2] + NUM_ORDERS + 1 - NUM_NEW_ORDERS; });
  add_column<int>(columns_by_chunk, cxlumn_definitions, "NO_D_ID", cardinalities,
                  [&](std::vector<size_t> indices) { return indices[1]; });
  add_column<int>(columns_by_chunk, cxlumn_definitions, "NO_W_ID", cardinalities,
                  [&](std::vector<size_t> indices) { return indices[0]; });
