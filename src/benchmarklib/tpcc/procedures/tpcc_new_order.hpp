...
class TPCCNewOrder : public AbstractTPCCProcedure {
 public:
  ...
  static constexpr auto PREPARED_STATEMENTS = std::to_array({
      "PREPARE new_order_select_warehouse_tax FROM 'SELECT W_TAX FROM WAREHOUSE WHERE W_ID = ?'",
      ...
      "PREPARE new_order_select_stock_with_dist_03 FROM 'SELECT S_QUANTITY, S_DIST_03, S_DATA, S_YTD, S_ORDER_CNT, S_REMOTE_CNT FROM STOCK WHERE S_I_ID = ? AND S_W_ID = ?'",
      "PREPARE new_order_select_stock_with_dist_04 FROM 'SELECT S_QUANTITY, S_DIST_04, S_DATA, S_YTD, S_ORDER_CNT, S_REMOTE_CNT FROM STOCK WHERE S_I_ID = ? AND S_W_ID = ?'",
      "PREPARE new_order_select_stock_with_dist_05 FROM 'SELECT S_QUANTITY, S_DIST_05, S_DATA, S_YTD, S_ORDER_CNT, S_REMOTE_CNT FROM STOCK WHERE S_I_ID = ? AND S_W_ID = ?'",
      "PREPARE new_order_select_stock_with_dist_06 FROM 'SELECT S_QUANTITY, S_DIST_06, S_DATA, S_YTD, S_ORDER_CNT, S_REMOTE_CNT FROM STOCK WHERE S_I_ID = ? AND S_W_ID = ?'",
      "PREPARE new_order_select_stock_with_dist_07 FROM 'SELECT S_QUANTITY, S_DIST_07, S_DATA, S_YTD, S_ORDER_CNT, S_REMOTE_CNT FROM STOCK WHERE S_I_ID = ? AND S_W_ID = ?'",
      "PREPARE new_order_select_stock_with_dist_08 FROM 'SELECT S_QUANTITY, S_DIST_08, S_DATA, S_YTD, S_ORDER_CNT, S_REMOTE_CNT FROM STOCK WHERE S_I_ID = ? AND S_W_ID = ?'",
