  ...
  for (auto d_id = 1; d_id <= 10; ++d_id) {
    ...
    const auto new_order_select_pair =
        _sql_executor.execute(std::string{"SELECT COUNT(*), MIN(NO_O_ID) FROM NEW_ORDER WHERE NO_W_ID = "} +
                              std::to_string(_w_id) + " AND NO_D_ID = " + std::to_string(d_id));
    ...
    const auto new_order_update_pair =
        _sql_executor.execute(std::string{"DELETE FROM NEW_ORDER WHERE NO_W_ID = "} + std::to_string(_w_id) +
    ...
    const auto order_select_pair =
        _sql_executor.execute(std::string{"SELECT O_C_ID FROM \"ORDER\" WHERE O_W_ID = "} + std::to_string(_w_id) +
    ...
    const auto order_update_pair =
        _sql_executor.execute(std::string{"UPDATE \"ORDER\" SET O_CARRIER_ID = "} + std::to_string(_o_carrier_id) +
                              " WHERE O_W_ID = " + std::to_string(_w_id) + " AND O_D_ID = " + std::to_string(d_id) +
    ...
    const auto order_line_select_pair = _sql_executor.execute(
        std::string{"SELECT SUM(OL_AMOUNT) FROM ORDER_LINE WHERE OL_W_ID = "} + std::to_string(_w_id) +
    ...
    const auto order_line_update_pair =
        _sql_executor.execute(std::string{"UPDATE ORDER_LINE SET OL_DELIVERY_D = "} + std::to_string(_ol_delivery_d) +
                              " WHERE OL_W_ID = " + std::to_string(_w_id) + " AND OL_D_ID = " + std::to_string(d_id) +
    ...
    const auto customer_update_pair =
        _sql_executor.execute(std::string{"UPDATE CUSTOMER SET C_BALANCE = C_BALANCE + "} + std::to_string(amount) +
                              ", C_DELIVERY_CNT = C_DELIVERY_CNT + 1 WHERE C_W_ID = " + std::to_string(_w_id) +
