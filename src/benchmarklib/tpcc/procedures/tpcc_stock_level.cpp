...
bool TpccStockLevel::execute() {
  const auto district_table_pair = _sql_executor.execute(std::string{"SELECT D_NEXT_O_ID FROM DISTRICT WHERE D_W_ID = "} + std::to_string(_w_id) + " AND D_ID = " + std::to_string(_d_id));
  ...
  const auto order_line_table_pair = _sql_executor.execute(std::string{"SELECT OL_I_ID FROM ORDER_LINE WHERE OL_W_ID = "} + std::to_string(_w_id) + " AND OL_D_ID = " + std::to_string(_d_id) + " AND OL_O_ID >= " + std::to_string(first_o_id));
  ...
  _sql_executor.execute(std::string{"SELECT COUNT(*) FROM STOCK WHERE S_I_ID IN ("} + ol_i_ids + ") AND S_W_ID = " + std::to_string(_w_id) + " AND S_QUANTITY < " + std::to_string(_threshold));
