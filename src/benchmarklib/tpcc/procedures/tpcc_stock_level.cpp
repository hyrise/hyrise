...
TPCCStockLevel::TPCCStockLevel(const int num_warehouses, BenchmarkSQLExecutor& init_sql_executor)
  ...
  _threshold = threshold_dist(random_engine);
}
...
bool TPCCStockLevel::_on_execute() {
  ...
  const auto& [_, district_table] =
      _sql_executor.execute("SELECT D_NEXT_O_ID FROM DISTRICT WHERE D_W_ID = " + std::to_string(_w_id) +
  ...
  _sql_executor.execute(
      "SELECT COUNT(DISTINCT(S_I_ID)) item_count FROM ORDER_LINE, STOCK WHERE OL_W_ID = " + std::to_string(_w_id) +
      " AND OL_D_ID = " + std::to_string(_d_id) + " AND OL_O_ID < " + std::to_string(next_o_id) +
      " AND OL_O_ID >= " + std::to_string(next_o_id - 20) + " AND S_W_ID = " + std::to_string(_w_id) +
      " AND S_I_ID = OL_I_ID AND S_QUANTITY < " + std::to_string(_threshold));
