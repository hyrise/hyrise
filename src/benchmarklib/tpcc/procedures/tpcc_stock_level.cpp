...
bool TPCCStockLevel::_on_execute() {
  ...
  const auto& [_, district_table] =
      _sql_executor.execute(std::format("EXECUTE stock_level_next_o_id({}, {})", _w_id, _d_id));
  ...
  // differently.
  _sql_executor.execute(std::format("EXECUTE stock_level_count_items({0}, {1}, {2}, {3}, {0}, {4})", _w_id, _d_id,
                                    next_o_id, next_o_id - 20, _threshold));
