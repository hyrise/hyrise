...
bool TpccPayment::execute() {
  ...
  const auto customer_update_balance_pair = _sql_executor.execute(std::string{"UPDATE CUSTOMER SET C_BALANCE = C_BALANCE - "} + std::to_string(_h_amount) + ", C_YTD_PAYMENT = C_YTD_PAYMENT + " + std::to_string(_h_amount) + ", C_PAYMENT_CNT = C_PAYMENT_CNT + 1 WHERE C_W_ID = " + std::to_string(_w_id) + " AND C_D_ID = " + std::to_string(_c_d_id) + " AND C_ID = " + std::to_string(customer_id));
  if(customer_update_balance_pair.first != SQLPipelineStatus::Success)  {_sql_executor.rollback(); return false;}
  ...
  if (customer_table->get_value<pmr_string>(ColumnID{11}, customer_offset) == "BC") {
    ...
    const auto customer_update_data_pair = _sql_executor.execute(std::string{"UPDATE CUSTOMER SET C_DATA = '"} + new_c_data + "' WHERE C_W_ID = " + std::to_string(_w_id) + " AND C_D_ID = " + std::to_string(_c_d_id) + " AND C_ID = " + std::to_string(customer_id));
    if(customer_update_data_pair.first != SQLPipelineStatus::Success)  {_sql_executor.rollback(); return false;}
  }
