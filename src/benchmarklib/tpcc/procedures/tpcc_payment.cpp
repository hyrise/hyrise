...
bool TPCCPayment::_on_execute() {
  ...
  const auto warehouse_select_pair = _sql_executor.execute(std::format("EXECUTE payment_select_warehouse({})", w_id));
  ...
  const auto district_select_pair =
      _sql_executor.execute(std::format("EXECUTE payment_select_district({}, {})", w_id, d_id));
  ...
  if (!select_customer_by_name) {
    ...
    std::tie(std::ignore, customer_table) = _sql_executor.execute(
        std::format("EXECUTE payment_select_customer_by_id({}, {}, {})", w_id, c_d_id, std::get<int32_t>(customer)));
    ...
    std::tie(std::ignore, customer_table) = _sql_executor.execute(std::format(
        "EXECUTE payment_select_customer_by_name({}, {}, '{}')", w_id, c_d_id, std::get<pmr_string>(customer)));
  ...
  if (*customer_table->get_value<pmr_string>(ColumnID{11}, customer_offset) == "BC") {
    ...
    const auto customer_update_data_pair = _sql_executor.execute(
        std::format("EXECUTE payment_update_customer_c_data('{}', {}, {}, {})", new_c_data, w_id, c_d_id, c_id));
