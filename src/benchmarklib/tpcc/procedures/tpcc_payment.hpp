...
class TPCCPayment : public AbstractTPCCProcedure {
 public:
  ...
  static constexpr auto PREPARED_STATEMENTS = std::to_array({
    "PREPARE payment_select_warehouse FROM 'SELECT W_NAME, W_STREET_1, W_STREET_2, W_CITY, W_STATE, W_ZIP, W_YTD FROM WAREHOUSE WHERE W_ID = ?'",
    "PREPARE payment_update_warehouse_ytd FROM 'UPDATE WAREHOUSE SET W_YTD = ? WHERE W_ID = ?'",
    "PREPARE payment_select_district FROM 'SELECT D_NAME, D_STREET_1, D_STREET_2, D_CITY, D_STATE, D_ZIP, D_YTD FROM DISTRICT WHERE D_W_ID = ? AND D_ID = ?'",
    "PREPARE payment_update_district_ytd FROM 'UPDATE DISTRICT SET D_YTD = ? WHERE D_W_ID = ? AND D_ID = ?'",
    "PREPARE payment_select_customer_by_id FROM 'SELECT C_ID, C_FIRST, C_MIDDLE, C_LAST, C_STREET_1, C_STREET_2, C_CITY, C_STATE, C_ZIP, C_PHONE, C_SINCE, C_CREDIT, C_CREDIT_LIM, C_DISCOUNT, C_BALANCE, C_DATA FROM CUSTOMER WHERE C_W_ID = ? AND C_D_ID = ? AND C_ID = ?'",
    "PREPARE payment_select_customer_by_name FROM 'SELECT C_ID, C_FIRST, C_MIDDLE, C_LAST, C_STREET_1, C_STREET_2, C_CITY, C_STATE, C_ZIP, C_PHONE, C_SINCE, C_CREDIT, C_CREDIT_LIM, C_DISCOUNT, C_BALANCE, C_DATA FROM CUSTOMER WHERE C_W_ID = ? AND C_D_ID = ? AND C_LAST = ? ORDER BY C_FIRST'",
