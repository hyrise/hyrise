...
class TPCCStockLevel : public AbstractTPCCProcedure {
 public:
  ...
  static constexpr auto PREPARED_STATEMENTS = std::to_array({
    "PREPARE stock_level_next_o_id FROM 'SELECT D_NEXT_O_ID FROM DISTRICT WHERE D_W_ID = ? AND D_ID = ?'",
    ...
    "PREPARE stock_level_count_items FROM 'SELECT COUNT(DISTINCT(S_I_ID)) item_count FROM ORDER_LINE, STOCK WHERE OL_W_ID = ? AND OL_D_ID = ? AND OL_O_ID < ? AND OL_O_ID >= ? AND S_W_ID = ? AND S_I_ID = OL_I_ID AND S_QUANTITY < ?'"
  });
  // clang-format on
 protected:
