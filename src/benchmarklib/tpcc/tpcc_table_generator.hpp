...
class TpccTableGenerator : public opossum::AbstractBenchmarkTableGenerator {
 ...
 protected:
  ...
  template <typename T>
  std::vector<T> _generate_inner_order_line_cxlumn(std::vector<size_t> indices,
                                                   order_line_counts_type order_line_counts,
                                                   const std::function<T(std::vector<size_t>)>& generator_function);

  template <typename T>
  void _add_order_line_cxlumn(std::vector<Segments>& segments, TableCxlumnDefinitions& cxlumn_definitions,
