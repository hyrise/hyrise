...
auto prepare_for_table(int table_id){
  ...
  const auto& tdefs = *getSimpleTdefsByNumber(table_id);  // TODO const?
...
// clang-format off
// TODO add cc_address(ds_addr_t) and cc_tax_percentage(decimal_t)
...
std::unordered_map<std::string, BenchmarkTableInfo> TpcdsTableGenerator::generate() {
  ...
  auto call_center_builder = TableBuilder{_benchmark_config->chunk_size,
                                          call_center_column_types,
                                          call_center_column_names,
                                          UseMvcc::Yes,
