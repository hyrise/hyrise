...
cxxopts::Options BenchmarkRunner::get_basic_cli_options(const std::string& benchmark_name) {
  ...
  cli_options.add_options()
    ...
    ("table_indexes", "Create table indexes (index per table column; columns defined by benchmark)", cxxopts::value<bool>()->default_value("true"))  // NOLINT(whitespace/line_length)
    ("index_everything", "Create indexes on every column of every table of the benchmark", cxxopts::value<bool>()->default_value("true"))  // NOLINT(whitespace/line_length)
