          ...
          for (const auto& sql_statement_metrics : pipeline_metrics.statement_metrics) {
            ...
            for (const auto& operator_cardinality_metrics : sql_statement_metrics->operator_cardinality_metrics) {
              ...
              sql_statement_metrics_json["operator_cardinality_metrics"].push_back(nlohmann::json{
                  ...
                  {"operator_type", magic_enum::enum_name(operator_cardinality_metrics.operator_type)},
            ...
            std::cout << "Predicate column datasizes: " << sql_statement_metrics->predicate_column_datatype.size()
                      << std::endl;

            for (const auto [datatype, count] : sql_statement_metrics->join_column_datatype) {
              sql_statement_metrics_json["join_column_datatypes"].push_back(
                  nlohmann::json{{"datatype", data_type_to_string.left.at(datatype)}, {"count", count}});
            }

            for (const auto [datatype, count] : sql_statement_metrics->predicate_column_datatype) {
              sql_statement_metrics_json["predicate_column_datatypes"].push_back(
                  nlohmann::json{{"datatype", data_type_to_string.left.at(datatype)}, {"count", count}});
            }
