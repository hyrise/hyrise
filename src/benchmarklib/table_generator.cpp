...
std::shared_ptr<Table> TableGenerator::generate_table(const ChunkID chunk_size,
  ...
  std::default_random_engine engine;  // NOLINT(cert-msc32-c) clang-tidy warns that default argument is predictable
...
std::shared_ptr<Table> TableGenerator::generate_table(
  ...
  auto pseudorandom_engine = std::mt19937{};  // NOLINT(cert-msc32-c) clang-tidy warns that no argument is predictable
