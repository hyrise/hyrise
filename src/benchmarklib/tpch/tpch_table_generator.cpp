...
void TPCHTableGenerator::_add_constraints(
  ...
  customer_table->add_soft_unique_constraint(TableConstraintDefinition{{customer_table->column_id_by_name("c_custkey")}, IsPrimaryKey::Yes});

  const auto& orders_table = table_info_by_name.at("orders").table;
  const auto orders_pk_constraint = TableConstraintDefinition{{orders_table->column_id_by_name("o_orderkey")}, IsPrimaryKey::Yes};
  ...
  const auto lineitem_pk_constraint = TableConstraintDefinition{
      {lineitem_table->column_id_by_name("l_orderkey"), lineitem_table->column_id_by_name("l_linenumber")}, IsPrimaryKey::Yes};
  lineitem_table->add_soft_unique_constraint(lineitem_pk_constraint);

  const auto& part_table = table_info_by_name.at("part").table;
  const auto part_table_pk_constraint = TableConstraintDefinition{{part_table->column_id_by_name("p_partkey")}, IsPrimaryKey::Yes};
  ...
  const auto partsupp_pk_constraint = TableConstraintDefinition{
      {partsupp_table->column_id_by_name("ps_partkey"), partsupp_table->column_id_by_name("ps_suppkey")}, IsPrimaryKey::Yes};
  ...
  const auto supplier_pk_constraint = TableConstraintDefinition{{supplier_table->column_id_by_name("s_suppkey")}, IsPrimaryKey::Yes};
  ...
  const auto nation_pk_constraint = TableConstraintDefinition{{nation_table->column_id_by_name("n_nationkey")}, IsPrimaryKey::Yes};
  nation_table->add_soft_unique_constraint(nation_pk_constraint);

  const auto& region_table = table_info_by_name.at("region").table;
  const auto region_pk_constraint = TableConstraintDefinition{{region_table->column_id_by_name("r_regionkey")}, IsPrimaryKey::Yes};
