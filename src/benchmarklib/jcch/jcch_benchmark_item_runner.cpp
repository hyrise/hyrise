...
bool JCCHBenchmarkItemRunner::_on_execute_item(const BenchmarkItemID item_id, BenchmarkSQLExecutor& sql_executor) {
  ...
  switch (item_id) {
    ...
    case 1 - 1: {
      // In some cases, we still need to do the date calculations that SQLite (used for verification) does not support yet.
      // When parsing a date, we expect the generator to provide sound date strings and omit checks.
