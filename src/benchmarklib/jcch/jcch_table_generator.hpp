...
class JCCHTableGenerator : virtual public AbstractTableGenerator, private TPCHTableGenerator, private FileBasedTableGenerator {
 public:
  ...
  explicit JCCHTableGenerator(const std::string& path, float scale_factor, uint32_t chunk_size = Chunk::DEFAULT_SIZE);

  // Constructor for creating a JCCHTableGenerator in a benchmark
  explicit JCCHTableGenerator(const std::string& path, float scale_factor, const std::shared_ptr<BenchmarkConfig>& benchmark_config);
