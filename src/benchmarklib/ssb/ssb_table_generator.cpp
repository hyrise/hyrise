...
std::unordered_map<std::string, BenchmarkTableInfo> SSBTableGenerator::generate() {
  ...
  if (!std::filesystem::exists(tables_path + "/customer.bin")) {
    ...
    Assert(std::filesystem::exists(tables_path), "Creating SSB tables folder failed");
    ...
    for (const auto& table_name : ssb_table_names) {
      ...
      {
        auto cmd = std::stringstream{};
        cmd << "cp resources/benchmark/ssb/schema/" << table_name << ".csv.json " << tables_path << table_name
            << ".csv.json";
        auto ret = system(cmd.str().c_str());
