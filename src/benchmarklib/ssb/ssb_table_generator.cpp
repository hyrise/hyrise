...
std::unordered_map<std::string, BenchmarkTableInfo> SSBTableGenerator::generate() {
  ...
  if (!std::filesystem::exists(data_path + "customer.bin")) {
    ...
    for (const auto& table_name : ssb_table_names) {
      // Rename tbl files generated by dbgen to csv so that the correct importer is used
      std::filesystem::rename(data_path + table_name + ".tbl", data_path + table_name + ".csv");
      ...
      // Remove the trailing separator from each line as the CSVReader does not like them
      {
        ...
        auto cmd = std::stringstream{};
        cmd << "sed -Ee 's/\\|$//' " << sed_inplace << " " << data_path << table_name << ".csv";
        auto ret = system(cmd.str().c_str());
      ...
      {
        auto cmd = std::stringstream{};
        cmd << "cp resources/benchmark/ssb/schema/" << table_name << ".csv.json " << data_path << table_name
            << ".csv.json";
        auto ret = system(cmd.str().c_str());
  ...
  if (std::filesystem::exists(data_path + "/customer.csv")) {
    auto cmd = std::stringstream{};
    cmd << "rm " << data_path << "*.csv*";
    auto ret = system(cmd.str().c_str());
