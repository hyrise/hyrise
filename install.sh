...
if echo $REPLY | grep -E '^[Yy]$' > /dev/null; then
    ...
    if [[ "$unamestr" == 'Darwin' ]]; then
        ...
        if brew update >/dev/null; then
            ...
            for formula in boost cmake gcc clang-format@3.8 gcovr tbb pkg-config readline ncurses sqlite3 parallel libpq libpqxx; do
        ...
        if cat /etc/lsb-release | grep DISTRIB_ID | grep Ubuntu >/dev/null; then
            ...
            if sudo apt-get update >/dev/null; then
                ...
                sudo apt-get install --no-install-recommends -y clang-$requiredclang clang-format-3.8 gcovr python2.7 gcc-${requiredgccmajor} g++-${requiredgccmajor} llvm libnuma-dev libnuma1 libtbb-dev build-essential cmake libreadline-dev libncurses5-dev libsqlite3-dev parallel $boostall libpq-dev libpqxx-dev &
