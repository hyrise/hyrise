    ...
    if [[ "$unamestr" == 'Darwin' ]]; then
        ...
        if [ -f /etc/lsb-release ] && cat /etc/lsb-release | grep DISTRIB_ID | grep Ubuntu >/dev/null; then
            ...
            if sudo apt-get update >/dev/null; then
                ...
                sudo update-alternatives --install /usr/bin/clang clang /usr/bin/clang-9 80 --slave /usr/bin/clang++ clang++ /usr/bin/clang++-9 --slave /usr/bin/clang-tidy clang-tidy /usr/bin/clang-tidy-9 --slave /usr/bin/llvm-profdata llvm-profdata /usr/bin/llvm-profdata-9
            else
                echo "Error during installation."
                exit 1
            fi
