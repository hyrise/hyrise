  ...
  oppossumCI.inside("-u 0:0 -v /mnt/ccache:/ccache -e \"CCACHE_DIR=/ccache\" -e \"CCACHE_CPP2=yes\" -e \"CCACHE_MAXSIZE=10GB\" -e \"CCACHE_SLOPPINESS=file_macro\"") {
    try {
      stage("Setup") {
        checkout scm
        sh "./install.sh"
        sh "mkdir clang-debug && cd clang-debug && cmake -GNinja -DCI_BUILD=ON -DCMAKE_CXX_COMPILER_LAUNCHER=ccache -DCMAKE_BUILD_TYPE=Debug -DCMAKE_C_COMPILER=clang-6.0 -DCMAKE_CXX_COMPILER=clang++-6.0 .. &\
        mkdir clang-debug-tidy && cd clang-debug-tidy && cmake -GNinja -DCI_BUILD=ON -DCMAKE_CXX_COMPILER_LAUNCHER=ccache -DCMAKE_BUILD_TYPE=Debug -DCMAKE_C_COMPILER=clang-6.0 -DCMAKE_CXX_COMPILER=clang++-6.0 -DENABLE_CLANG_TIDY=ON .. &\
        mkdir clang-debug-addr-ub-sanitizers && cd clang-debug-addr-ub-sanitizers && cmake -GNinja -DCI_BUILD=ON -DCMAKE_CXX_COMPILER_LAUNCHER=ccache -DCMAKE_BUILD_TYPE=Debug -DCMAKE_C_COMPILER=clang-6.0 -DCMAKE_CXX_COMPILER=clang++-6.0 -DENABLE_ADDR_UB_SANITIZATION=ON .. &\
        mkdir clang-release-addr-ub-sanitizers && cd clang-release-addr-ub-sanitizers && cmake -GNinja -DCI_BUILD=ON -DCMAKE_CXX_COMPILER_LAUNCHER=ccache -DCMAKE_BUILD_TYPE=Release -DCMAKE_C_COMPILER=clang-6.0 -DCMAKE_CXX_COMPILER=clang++-6.0 -DENABLE_ADDR_UB_SANITIZATION=ON .. &\
        mkdir clang-release && cd clang-release && cmake -GNinja -DCI_BUILD=ON -DCMAKE_CXX_COMPILER_LAUNCHER=ccache -DCMAKE_BUILD_TYPE=Release -DCMAKE_C_COMPILER=clang-6.0 -DCMAKE_CXX_COMPILER=clang++-6.0 .. &\
        mkdir clang-release-addr-ub-sanitizers-no-numa && cd clang-release-addr-ub-sanitizers-no-numa && cmake -GNinja -DCI_BUILD=ON -DCMAKE_CXX_COMPILER_LAUNCHER=ccache -DCMAKE_BUILD_TYPE=Release -DCMAKE_C_COMPILER=clang-6.0 -DCMAKE_CXX_COMPILER=clang++-6.0 -DENABLE_ADDR_UB_SANITIZATION=ON -DENABLE_NUMA_SUPPORT=OFF .. &\
        mkdir clang-release-thread-sanitizer-no-numa && cd clang-release-thread-sanitizer-no-numa && cmake -GNinja -DCI_BUILD=ON -DCMAKE_CXX_COMPILER_LAUNCHER=ccache -DCMAKE_C_COMPILER=clang-6.0 -DCMAKE_CXX_COMPILER=clang++-6.0 -DENABLE_THREAD_SANITIZATION=ON -DENABLE_NUMA_SUPPORT=OFF .. &\
        mkdir gcc-debug && cd gcc-debug && cmake -GNinja -DCI_BUILD=ON -DCMAKE_CXX_COMPILER_LAUNCHER=ccache -DCMAKE_BUILD_TYPE=Debug -DCMAKE_C_COMPILER=gcc -DCMAKE_CXX_COMPILER=g++ .. &\
        mkdir gcc-release && cd gcc-release && cmake -GNinja -DCI_BUILD=ON -DCMAKE_CXX_COMPILER_LAUNCHER=ccache -DCMAKE_BUILD_TYPE=Release -DCMAKE_C_COMPILER=gcc -DCMAKE_CXX_COMPILER=g++ .. &\
        wait"
      ...
      parallel clangDebug: {
        stage("clang-debug") {
          sh "export CCACHE_BASEDIR=`pwd`; cd clang-debug && ninja all -j \$(( \$(cat /proc/cpuinfo | grep processor | wc -l) / 3 + 2))"
          sh "./clang-debug/hyriseTest clang-debug"
        }
      }, gccDebug: {
        stage("gcc-debug") {
          sh "export CCACHE_BASEDIR=`pwd`; cd gcc-debug && ninja all -j \$(( \$(cat /proc/cpuinfo | grep processor | wc -l) / 3 + 2))"
      ...
      parallel clangRelease: {
        stage("clang-release") {
          if (env.BRANCH_NAME == 'master' || full_ci) {
            sh "export CCACHE_BASEDIR=`pwd`; cd clang-release && ninja all -j \$(( \$(cat /proc/cpuinfo | grep processor | wc -l) / 3 + 2))"
        ...
        stage("clang-debug:tidy") {
          if (env.BRANCH_NAME == 'master' || full_ci) {
            sh "export CCACHE_BASEDIR=`pwd`; cd clang-debug-tidy && ninja hyriseTest hyriseSystemTest -j \$(( \$(cat /proc/cpuinfo | grep processor | wc -l) / 3 + 2))"
          } else {
        ...
        stage("clang-debug:addr-ub-sanitizers") {
          if (env.BRANCH_NAME == 'master' || full_ci) {
            sh "export CCACHE_BASEDIR=`pwd`; cd clang-debug-addr-ub-sanitizers && ninja hyriseTest hyriseSystemTest -j \$(( \$(cat /proc/cpuinfo | grep processor | wc -l) / 3 + 2))"
        ...
        if (env.BRANCH_NAME == 'master' || full_ci) {
          stage("gcc-release") {
            sh "export CCACHE_BASEDIR=`pwd`; cd gcc-release && ninja all -j \$(( \$(cat /proc/cpuinfo | grep processor | wc -l) / 3 + 2))"
        ...
        stage("clang-release:addr-ub-sanitizers") {
          if (env.BRANCH_NAME == 'master' || full_ci) {
            sh "export CCACHE_BASEDIR=`pwd`; cd clang-release-addr-ub-sanitizers && ninja hyriseTest hyriseSystemTest -j \$(( \$(cat /proc/cpuinfo | grep processor | wc -l) / 3 + 2))"
        ...
        stage("clang-release:addr-ub-sanitizers w/o NUMA") {
          if (env.BRANCH_NAME == 'master' || full_ci) {
            sh "export CCACHE_BASEDIR=`pwd`; cd clang-release-addr-ub-sanitizers-no-numa && ninja hyriseTest hyriseSystemTest -j \$(( \$(cat /proc/cpuinfo | grep processor | wc -l) / 3 + 2))"
        ...
        stage("clang-release:thread-sanitizer w/o NUMA") {
          if (env.BRANCH_NAME == 'master' || full_ci) {
            sh "export CCACHE_BASEDIR=`pwd`; cd clang-release-thread-sanitizer-no-numa && ninja hyriseTest hyriseSystemTest -j \$(( \$(cat /proc/cpuinfo | grep processor | wc -l) / 3 + 2))"
      ...
      parallel memcheckReleaseTest: {
        stage("memcheck-release-test") {
          if (env.BRANCH_NAME == 'master' || full_ci) {
            ...
            sh "valgrind --tool=memcheck --error-exitcode=1 --leak-check=full --gen-suppressions=all --suppressions=.valgrind-ignore.txt ./clang-release/hyriseTest clang-release-memcheck-test --gtest_filter=-NUMAMemoryResourceTest.BasicAllocate"
          } else {
        ...
        stage("memcheck-release-systemtest1") {
          if (env.BRANCH_NAME == 'master' || full_ci) {
            ...
            sh "valgrind --tool=memcheck --error-exitcode=1 --leak-check=full --gen-suppressions=all --suppressions=.valgrind-ignore.txt ./clang-release/hyriseSystemTest clang-release-memcheck-systemtest1 --gtest_filter=TPCHTestInstances/TPCHTest.TPCHQueryTest/20"
          } else {
        ...
        stage("memcheck-release-systemtest2") {
          if (env.BRANCH_NAME == 'master' || full_ci) {
            ...
            sh "valgrind --tool=memcheck --error-exitcode=1 --leak-check=full --gen-suppressions=all --suppressions=.valgrind-ignore.txt ./clang-release/hyriseSystemTest clang-release-memcheck-systemtest2 --gtest_filter=-TPCHTestInstances/TPCHTest.TPCHQueryTest/20"
          } else {
