      ...
      stage("Build") {
        stage("Build gcc") {
          stage("Build gcc Release") {
            sh "cd gcc-release && make opossumConsole opossumTest opossumAsan -j \$(cat /proc/cpuinfo | grep processor | wc -l)"
          }
          stage("Build gcc Debug") {
            sh "cd gcc-debug && make opossumConsole opossumTest opossumAsan -j \$(cat /proc/cpuinfo | grep processor | wc -l)"
          }
        }

        stage("Build clang") {
          stage("Build clang release") {
            sh "cd clang-release && make opossumConsole opossumTest opossumAsan -j \$(cat /proc/cpuinfo | grep processor | wc -l)"
          }
          stage("Build clang debug") {
            sh "cd clang-debug && make opossumConsole opossumTest opossumAsan -j \$(cat /proc/cpuinfo | grep processor | wc -l)"
          }
        }
      }
