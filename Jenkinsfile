          ...
          parallel memcheckReleaseTest: {
            ...
            stage('nixSetup') {
              if (env.BRANCH_NAME == 'master' || full_ci) {
                ...
                sh "/nix/var/nix/profiles/default/bin/nix --extra-experimental-features 'nix-command flakes' develop --command bash -c \"mkdir nix-debug && cd nix-debug && cmake ${debug} ${clang} ${unity} ${ninja} .. && ninja all -j \$(( \$(nproc) / 7)) && ./hyriseTest\""
                ...
                sh "/nix/var/nix/profiles/default/bin/nix --extra-experimental-features 'nix-command flakes' develop --command bash -c \"mkdir nix-release && cd nix-release && cmake ${release} ${clang} ${unity} ${ninja} .. && ninja all -j \$(( \$(nproc) / 3)) && ./hyriseTest\""
              } else {
