          ...
          parallel memcheckReleaseTest: {
            stage("memcheckReleaseTest") {
              ...
              if (env.BRANCH_NAME == 'master' || full_ci) {
                ...
                sh "valgrind --tool=memcheck --error-exitcode=1 --gen-suppressions=all --num-callers=25 --fair-sched=yes --suppressions=resources/.valgrind-ignore.txt ./clang-release/hyriseTest clang-release-memcheck-test --gtest_filter=-NUMAMemoryResourceTest.BasicAllocate:SQLiteTestRunner*:SchedulerTest.CheckScheduledTasksAreGrouped"
