  ...
  stage ("Start") {
    ...
    try {
      withCredentials([usernamePassword(credentialsId: '5fe8ede9-bbdb-4803-a307-6924d4b4d9b5', usernameVariable: 'GITHUB_USERNAME', passwordVariable: 'GITHUB_TOKEN')]) {
        ...
        sh '''
          curl -s -I -H "Authorization: token ${GITHUB_TOKEN}" https://api.github.com/orgs/hyrise/memberships/${PR_CREATED_BY} 2>/dev/null | head -n 1 | grep "HTTP/1.1 204 No Content"
        '''
      }
    } catch (error) {
      stage ("User unknown") {
