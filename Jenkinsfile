...
try {
  ...
  node('linux') {
    ...
    oppossumCI.inside("-u 0:0 -v /mnt/ccache:/ccache -e \"CCACHE_DIR=/ccache\" -e \"CCACHE_CPP2=yes\" -e \"CCACHE_MAXSIZE=50GB\" -e \"CCACHE_SLOPPINESS=file_macro,pch_defines,time_macros\" -e\"CCACHE_DEPEND_MODE=yes\" --privileged=true") {
      try {
        stage("Setup") {
          checkout scm
          sh "./install.sh"
    ...
    stage("clangDebugMac") {
      if (env.BRANCH_NAME == 'master' || full_ci) {
        try {
          ...
          sh "cd clang-debug && CCACHE_CPP2=yes CCACHE_SLOPPINESS=file_macro,pch_defines,time_macros CCACHE_DEPEND_MODE=yes CCACHE_BASEDIR=`pwd` make -j8"
