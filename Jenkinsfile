    ...
    docker.withRegistry('', 'docker') {
      ...
      hyriseCI.inside("--cap-add SYS_PTRACE -u 0:0") {
	    ...
	    stage('nixSetup') {
              ...
              sh "/nix/var/nix/profiles/default/bin/nix-shell resources/nix --pure --run \"mkdir nix-debug && cd nix-debug && cmake ${clang} ${debug} ${unity} .. && ninja all -j \$(( \$(nproc) / 7)) && ./hyriseTest && ./hyriseSystemTest\""
              ...
              sh "/nix/var/nix/profiles/default/bin/nix-shell resources/nix --pure --run \"mkdir nix-release && cd nix-release && cmake ${clang} ${release} ${unity} .. && ninja all -j \$(( \$(nproc) / 3)) && ./hyriseTest && ./hyriseSystemTest\""
	    }
          }
        } finally {
